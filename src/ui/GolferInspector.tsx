...
export default function GolferInspector() {
  ...
  useFloatingPanelFocus(!!golfer, panelRef, close);
  ...
  const membership = regular?.membership?.tier === 'lifetime'
    ? 'Lifetime member'
  ...
  return (
    <section className="golferInspector" data-ui="people-inspector" ref={panelRef} role="dialog" aria-modal="false" aria-labelledby="golfer-inspector-title" tabIndex={-1}>
      <header className="golferInspectorHead">
        <span className="peopleSeal" aria-hidden="true"><Icon name="regulars" size={19} /></span>
        <CharacterPortrait name={golfer.name} shirt={golfer.shirt} skin={golfer.skin} cap={golfer.cap} expression={model.expression} variant="profile" className="inspectorPortrait" />
        <div>
          <small>PEOPLE · LIVE SIMFOTO</small>
          <h2 id="golfer-inspector-title">{golfer.name}</h2>
          <p>{membership}{model.visits == null ? '' : ` · Visit ${model.visits}`}</p>
        </div>
        <button className="iconButton golferInspectorClose" aria-label={`Close ${golfer.name} profile`} onClick={close}><Icon name="close" size={15} /></button>
      </header>

      <div className="golferSituation" aria-label={`${golfer.name} current situation`}>
      ...
      <div className="golferInspectorMeters">
        <section aria-labelledby="golfer-needs-title">
          ...
          {NEEDS.map(([key, label]) => <Meter key={key} label={label} ariaLabel={`${golfer.name} ${label}`} value={model.needs[key]} tone="need" />)}
        </section>
        <section aria-labelledby="golfer-skills-title">
          <h3 id="golfer-skills-title">Skills</h3>
          {SKILLS.map(([key, label]) => <Meter key={key} label={label} ariaLabel={`${golfer.name} ${label}`} value={model.skills[key]} tone="skill" />)}
        </section>
      </div>

      <blockquote className="golferLatestComment">
        <small>LATEST COMMENT</small>
        <p>{model.latestComment ? `“${model.latestComment}”` : `${golfer.name} is concentrating on the round.`}</p>
      </blockquote>
