import { golferAppearance, type GolferAppearance } from './sprites';
 ...
 */
export function golferPortraitRecipe(identity: string, expression: PortraitExpression = 'neutral'): GolferPortraitRecipe {
  const normalized = cleanIdentity(identity);
  const appearance = golferAppearance(normalized);
  ...
  return {
    ...
    hairTone: HAIR_TONES[(appearance.face + appearance.pants) % HAIR_TONES.length],
    accentTone: ACCENT_TONES[(appearance.outfit + appearance.bag) % ACCENT_TONES.length],
  };
}
 ...
 */
export function golferPortraitSprite(identity: string, shirt: string, skin: string, cap: string, expression: PortraitExpression = 'neutral'): HTMLCanvasElement {
  const recipe = golferPortraitRecipe(identity, expression);
  const key = `simfoto-v1|${recipe.identity}|${shirt}|${skin}|${cap}|${expression}`;
  ...
  line(ctx, [[20, 46], [24, 51], [29, 46]], '#f4f0dc', 2);
