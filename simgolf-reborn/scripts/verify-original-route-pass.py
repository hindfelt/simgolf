...
for a,n in [(0x4227b1,0xab0),(0x466ba0,0x10e),(0x40bc50,0x33),(0x40a9f0,0x81),(0x40c1a0,0x42),(0x4a57a0,0x27),(0x4c1870,64)]:
 o=p.get_offset_from_rva(a-0x400000);u.mem_write(a,p.__data__[o:o+n])
def write(a,v,size=4):u.mem_write(a,(v&((1<<(size*8))-1)).to_bytes(size,'little'))
def read(a):return struct.unpack('<i',u.mem_read(a,4))[0]
remaining=0;lie=0;calls=[];current={};sampleIndex=0
def hook(u,a,s,d):
...
def run(q):
 global current,calls,sampleIndex
 current=q;calls=[];sampleIndex=0
 ...
 u.emu_start(0x4227b1,0x423261,count=2000000)
 assert u.reg_read(UC_X86_REG_EIP)==0x423261
...
module=(root/'simgolf-reborn/scene/src/simulation/original-route-pass.js').as_uri()
script='''import {readFileSync} from 'node:fs';const {originalRoutePass}=await import(MODULE);
...
for(const [q,e,expectedCalls] of rows){const cells=new Map(q.cells.map(([x,z,code,shotClass,flags])=>[`${x},${z}`,{code,shotClass,flags}]));
 let index=0;const calls=[];const a=originalRoutePass({...q,assessShot:c=>{calls.push({type:"assess",...c});return q.costs[c.shape+1]+c.flag*3;},terrainAt:p=>cells.get(`${p.x},${p.z}`)},c=>{calls.push({type:"simulate",...c});return {landing:q.landings[(index++)%q.landings.length]};});
 if(JSON.stringify(a)!==JSON.stringify(e)||JSON.stringify(calls)!==JSON.stringify(expectedCalls))throw Error(JSON.stringify({a,e}));
}console.log(`${rows.length} full-grid route passes and ordered calls match original x86.`);
'''.replace('MODULE',json.dumps(module))
subprocess.run(['node','--input-type=module','-e',script],input=json.dumps(rows),text=True,check=True)

if '--write-fixture' in __import__('sys').argv:
 (root/'simgolf-reborn/scene/tests/fixtures/original-route-pass.json').write_text(json.dumps(rows[:3],separators=(',',':'))+'\n')
