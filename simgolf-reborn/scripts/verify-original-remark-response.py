"""Execute contiguous reaction stages, with an explicit controlled sound boundary."""
...
for a,n in [(0x467d72,0x2f8),(0x466a20,0x19),(0x466a00,0x20),(0x45ba70,0x60),(0x4b9800,8),(0x4a57a0,0x27)]:
...
for i in range(1600):
 ...
 u.emu_start(0x467d72,0x46806a,count=1500);end=u.reg_read(UC_X86_REG_EIP);assert end in (0x46806a,0x469075)
...
module=(root/'simgolf-reborn/scene/src/simulation/original-remark-response.js').as_uri()
script="""import {readFileSync} from 'node:fs';import {isDeepStrictEqual} from 'node:util';import {originalRemarkResponse} from MODULE;const rows=JSON.parse(readFileSync(0,'utf8'));for(const [q,expected] of rows){q.state.actor=Uint8Array.from(q.state.actor);q.before=Uint8Array.from(q.before);const got=originalRemarkResponse(q,(event,state)=>{if(q.effectMutation){state.actor[0x18]=0x40;state.seed=777;}return state;});got.state.actor=[...got.state.actor];if(!isDeepStrictEqual(got,expected))throw Error(JSON.stringify({q,expected,got}));}console.log(`${rows.length} native contiguous reaction-response cases matched`);""".replace('MODULE',json.dumps(module))
subprocess.run(['node','--input-type=module','-e',script],input=json.dumps(rows),text=True,check=True)
(root/'simgolf-reborn/scene/tests/fixtures/original-remark-response.json').write_text(json.dumps(rows[:48],separators=(',',':'))+'\n')
