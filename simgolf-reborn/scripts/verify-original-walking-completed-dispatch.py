...
for address in [0x466fb0,0x40c7f0,0x447a30,0x4acb95,0x42a019,0x429f4c,0x4295ef,0x42bdb5,0x42d23c,0x42b825,0x425b50,0x40daa0,0x40db60,0x46c140,0x4672d0,0x40c1f0,0x40c140,0x42def0,0x40c580,0x4071d0]:u.hook_add(UC_HOOK_CODE,hook,begin=address,end=address)
...
for i in range(500):
 ...
 u.reg_write(UC_X86_REG_ESP,0x102000);u.reg_write(UC_X86_REG_EBP,id*256);u.reg_write(UC_X86_REG_EAX,b[id*256+0x29]);u.reg_write(UC_X86_REG_FPCW,0x37f);end=None;calls=[];arrived=None;arrivalBranch=False;u.emu_start(0x4290ca,0x400fff,count=10000000);assert end
 e=dict(holes=list(u.mem_read(0x574500,len(h))),records=list(u.mem_read(0x583430,44)),sourceText=bytes(u.mem_read(0x518f78,512)).split(b"\0")[0].decode(),messageFlag=u.mem_read(0x568148,1)[0],actors=list(u.mem_read(0x577f00,len(b))),seed=get(0x820454)&0xffffffff,calls=calls,next=end,cashUnits=get(0x570a24),serviceIncome=struct.unpack('<h',u.mem_read(0x582c64,2))[0])
 rows.append([q,list(b),list(h),list(f),list(t),marks,widths,metadata,e])
script="""import {readFileSync} from 'node:fs';import {isDeepStrictEqual} from 'node:util';const {originalWalkingCompletedDispatch}=await import(MODULE);const rows=JSON.parse(readFileSync(0,'utf8'));const coverage={};for(const [q,b,h,f,t,m,w,metadata,e] of rows){const r=originalWalkingCompletedDispatch({...q,actors:Array.from({length:152},(_,i)=>new Uint8Array(b.slice(i*256,(i+1)*256))),holes:Array.from({length:20},(_,i)=>new Uint8Array(h.slice(i*520,(i+1)*520))),facilityRecords:new Uint8Array(f),facilityPrefix:new Uint8Array(32),facilityWidths:w,terrain:new Uint8Array(t),tileFlags:new Uint16Array(m),metadata,traversalCosts:new Int8Array(2500).fill(2),metadataClass:new Uint8Array(128),serviceIncome:new Int16Array(1),tileServiceState:new Uint8Array(2500),restDecorations:new Uint8Array(2500),completionRecords:[new Uint8Array(44)]},(event,state)=>{if(event.address===0x466fb0)state.sourceText+='Golfer '+event.args[0];if(event.address===0x425b50){state.actors[event.args[0]][0x29]=0;state.queueClock=(state.queueClock+17)|0;}if(event.address===0x4672d0)state.actors[event.args[0]][0x78]=36;return {state,value:1};});const a={holes:r.state.holes.flatMap(b=>Array.from(b)),records:Array.from(r.state.completionRecords[0]),sourceText:r.state.sourceText,messageFlag:r.state.messageFlag,actors:r.state.actors.flatMap(b=>Array.from(b)),seed:r.state.seed,calls:r.calls,next:r.next,cashUnits:r.state.cashUnits,serviceIncome:r.state.serviceIncome[0]};coverage[r.next]=(coverage[r.next]??0)+1;if(r.calls.some(c=>c.address===0x40c7f0))coverage.departures=(coverage.departures??0)+1;if(r.calls.some(c=>c.address===0x4071d0))coverage.restVisits=(coverage.restVisits??0)+1;if(r.calls.some(c=>c.address===0x40c580))coverage.paidServiceVisits=(coverage.paidServiceVisits??0)+1;if(!isDeepStrictEqual(b.slice(q.actorId*256+8,q.actorId*256+16),Array.from(r.state.actors[q.actorId].slice(8,16))))coverage.positionChanges=(coverage.positionChanges??0)+1;if(!isDeepStrictEqual(a,e))throw Error(JSON.stringify({q,fields:Object.keys(e).filter(k=>!isDeepStrictEqual(a[k],e[k])),actual:{...a,actors:undefined,calls:undefined},expected:{...e,actors:undefined,calls:undefined}}));}console.log(`${rows.length} continuous native walking dispatches match. ${JSON.stringify(coverage)}`);""".replace('MODULE',json.dumps((root/'simgolf-reborn/scene/src/simulation/original-walking-completed-dispatch.js').as_uri()))
