...
script="""import {readFileSync} from 'node:fs';const {originalObjectIndex}=await import(MODULE);const {originalPlannerWorldRecords}=await import(READER);
for(const [q,e] of JSON.parse(readFileSync(0,'utf8'))){const facilityRecords=new Uint8Array(4096),v=new DataView(facilityRecords.buffer);q.records.forEach((r,i)=>{v.setInt16(i*16,r.type,true);v.setInt16(i*16+2,r.x,true);v.setInt16(i*16+4,r.z,true);});const a=originalObjectIndex(q.x,q.z,originalPlannerWorldRecords({facilityRecords,objectBaseSizes:Int8Array.from(q.sizes),objectExpansions:Int32Array.from(q.expansions)}));
