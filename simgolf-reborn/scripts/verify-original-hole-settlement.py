...
for i in range(1200):
 ...
 h=rng.randbytes(520);s=rng.randbytes(184);record=bytearray(44);record[2]=rng.randrange(256);mode=rng.choice([0,2]);bonus=rng.choice([-2,0,4,2147483647]);mutate=i%2==0
 q=dict(actor=list(b),hole=list(h),stat=list(s),record=list(record),mode=mode,bonus=bonus)
 u.mem_write(0x577f00,bytes(b));u.mem_write(0x574500,h*20);u.mem_write(0x5698c0,s*32);u.mem_write(0x583430,bytes(record));put(0x542c04,mode);put(0x542be8,bonus);put(0x102004,0)
