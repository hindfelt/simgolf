"""Animation release through swing, ball motion and actual ordinary hole scoring."""
...
counts={0x405e80:2,0x40c1f0:4,0x466fb0:2,0x40c7f0:3,0x40c580:4,0x4672d0:3,0x45b2c0:1,0x45b180:1,0x4093b0:1,0x40c140:3,0x42f110:2,0x4096e0:1,0x409820:1,0x409780:1}
...
u.reg_write(UC_X86_REG_FPCW,0x37f);u.hook_add(UC_HOOK_CODE,hook);u.reg_write(UC_X86_REG_ESP,0x102000);u.emu_start(0x491380,0x4913bc,count=20000);calls=[];rng=random.Random(9912);rows=[]
for i in range(1200):
 ...
 struct.pack_into('<i',b,0xdc,x);struct.pack_into('<i',b,0xe0,z);struct.pack_into('<i',b,0xec,160);b[0x24]=13;b[0x25]=16;b[0x28]=1;struct.pack_into('<I',b,0x18,struct.unpack_from('<I',b,0x18)[0]&~0x40000);q['actor']=list(b)
 ...
 put(0x4c1848,0);put(0x102010,0);put(0x102018,q['ballTile']['x']);put(0x102030,q['visualSlot']);calls=[];branch=None;u.reg_write(UC_X86_REG_ESP,0x102000);u.reg_write(UC_X86_REG_EBP,0);u.reg_write(UC_X86_REG_EDI,q['ballTile']['z']);put(0x102014,1);put(0x102020,q['ballTile']['z']);put(0x102028,0);put(0x10201c,0);put(0x102050,0);put(0x102074,8);u.reg_write(UC_X86_REG_EAX,x);u.reg_write(UC_X86_REG_ECX,z);u.reg_write(UC_X86_REG_EBX,8);u.mem_write(0x53e2f8,struct.pack('<160i',*([8]*160)));put(0x10204c,0)
 animation_ticks=0
 while u.mem_read(0x577f28,1)[0]!=2:
  animation_ticks+=1;assert animation_ticks<=10
  put(0x102038,u.mem_read(0x577f26,1)[0]);u.reg_write(UC_X86_REG_ESP,0x102000);u.reg_write(UC_X86_REG_EBP,0);u.reg_write(UC_X86_REG_EDI,0);u.emu_start(0x414f79,0x41503b,count=1000)
 for swing_ticks in range(1,10):
  u.reg_write(UC_X86_REG_ESP,0x102000);u.reg_write(UC_X86_REG_EBP,0);u.reg_write(UC_X86_REG_EAX,u.mem_read(0x577f28,1)[0]);u.emu_start(0x42bb3b,0x400fff,count=50000)
  if any(e['address']==0x426b00 for e in calls):break
 assert any(e['address']==0x426b00 for e in calls), 'Cup completion was not reached'

 def h(addr,n):return hashlib.sha256(bytes(u.mem_read(addr,n))).hexdigest()
 rows.append(dict(q=q,mutate=mutate,expected=dict(animationTicks=animation_ticks,swingTicks=swing_ticks,actor=list(u.mem_read(0x577f00,256)),record=list(u.mem_read(0x583430,44)),holes=h(0x574500,520*21),stats=h(0x5698c0,184*32),periods=h(0x582c60,40),notices=h(0x5842b2,76*64),value=read(0x4c1848),cashTotal=read(0x570a24),periodIndex=struct.unpack('<h',u.mem_read(0x5a5784,2))[0],phaseCounter=read(0x831828),recordHolder=read(0x4c1dfc),presentationMode=read(0x5a8714),sourceText=text(),calls=calls,next=branch,selectionState=read(0x5a4440),scoreList=list(struct.unpack('<10i',u.mem_read(0x568f74,40))))))
module=(root/'simgolf-reborn/scene/src/simulation/original-actor-motion-continuation.js').as_uri()
script="""import {readFileSync} from 'node:fs';import {createHash} from 'node:crypto';import {isDeepStrictEqual} from 'node:util';const {originalActorMotionContinuation}=await import(MODULE);const {originalActorSwingAnimation}=await import(ANIMATION),{originalSwingProgress}=await import(SWING);function complete(snapshot,resolve){let state=snapshot,frameIndex=0,animationTicks=0;const calls=[];while(state.actors[0][0x28]!==2){if(++animationTicks>10)throw Error("Animation timeout");const a=originalActorSwingAnimation(state,{animationDirection:0,frameCounts:Array(160).fill(8),previousFrame:state.actors[0][0x26],frameIndex});state=a.state;frameIndex=a.frameIndex;}for(let swingTicks=1;swingTicks<10;swingTicks++){const a=originalSwingProgress(state,resolve);state=a.state;if(a.next==="motion"){const m=originalActorMotionContinuation({...a,ballTile:snapshot.ballTile,ballTerrain:snapshot.ballTerrain,visualSlot:snapshot.visualSlot},resolve);return {...m,calls:[...calls,...m.calls],animationTicks,swingTicks};}calls.push(...a.calls);}throw Error("Swing timeout");}const hash=rows=>createHash('sha256').update(Buffer.concat(rows.map(b=>Buffer.from(b)))).digest('hex');for(const [i,r] of JSON.parse(readFileSync(0,'utf8')).entries()){const q=r.q;const a=complete({...q,terrain:new Uint8Array(2500).fill(1),edgeMasks:new Uint8Array(2500),metadata:Array.from({length:23},()=>({bounceCoefficient:0,rollCoefficient:4,scatterCoefficient:0})),tileFlags:new Uint16Array(2500).fill(128),actorId:0,actors:[Uint8Array.from(q.actor)],scoreList:Int32Array.from(q.scoreList),holeRecords:Array.from({length:21},(_,i)=>{const b=i===20?new Uint8Array(520):Uint8Array.from(q.hole);if(i===19)b[0]=0;return b;}),statRecords:Array.from({length:32},()=>Uint8Array.from(q.stat)),completionRecords:[Uint8Array.from(q.record)],financialPeriods:Array.from({length:2},(_,i)=>Uint8Array.from(q.periods.slice(i*20,i*20+20))),completionNotices:Array.from({length:64},(_,i)=>Uint8Array.from(q.notices.slice(i*76,i*76+76)))},(e,state)=>{if(e.address===0x466fb0)state.sourceText='Gary';if(r.mutate){if(e.address===0x405e80){state.actors[0][0x21]=2;state.actors[0][0x29]=2;state.actors[0][0xc2]=1;}if(e.address===0x40c1f0)state.settlementValue=(state.settlementValue+1)|0;if(e.address===0x40c7f0){state.settlementValue=-9;state.periodIndex=1;}if(e.address===0x4672d0){state.actors[0][0x29]=3;state.actors[0][0x2a]=6;state.phaseCounter=120;}}return {state,value:0};});const s=a.state,actual={animationTicks:a.animationTicks,swingTicks:a.swingTicks,actor:Array.from(s.actors[0]),record:Array.from(s.completionRecords[0]),holes:hash(s.holeRecords),stats:hash(s.statRecords),periods:hash(s.financialPeriods),notices:hash(s.completionNotices),value:s.settlementValue,cashTotal:s.cashTotal,periodIndex:s.periodIndex,phaseCounter:s.phaseCounter,recordHolder:s.recordHolder,presentationMode:s.presentationMode,sourceText:s.sourceText,calls:a.calls,next:a.next,selectionState:s.selectionState,scoreList:Array.from(s.scoreList)};if(!isDeepStrictEqual(actual,r.expected))throw Error(JSON.stringify({i,keys:Object.keys(actual).filter(k=>!isDeepStrictEqual(actual[k],r.expected[k])),actorDiff:actual.actor.flatMap((v,j)=>v===r.expected.actor[j]?[]:[[j,v,r.expected.actor[j]]]),calls:actual.calls,expectedCalls:r.expected.calls}));}console.log('1200 continuous native animation-to-scored-cup cases match.');""".replace('MODULE',json.dumps(module)).replace('ANIMATION',json.dumps((root/'simgolf-reborn/scene/src/simulation/original-swing-animation.js').as_uri())).replace('SWING',json.dumps((root/'simgolf-reborn/scene/src/simulation/original-swing-progress.js').as_uri()))
