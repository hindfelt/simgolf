...
for a,n in [(0x426b10,0x420),(0x405e80,0xfd),(0x40c580,0x76),(0x40c1f0,0x1e7),(0x42f270,0x485),(0x4c1f94,36),(0x4672d0,0xe90),(0x469080,0xdd),(0x4a0000,1),(0x466a20,0x19),(0x466a00,0x20),(0x406b20,0x1c9),(0x466fb0,0x320),(0x40c7f0,0xf7),(0x45ba70,0x60),(0x4b9800,8),(0x4a57a0,0x27),(0x46806a,0x102b),(0x469160,0xe9),(0x469330,0x3008),(0x46c104,58),(0x45b090,0xbd),(0x46c140,0x2c),(0x4c0000,0x30000)]:u.mem_write(a,p.get_data(a-0x400000,n))
...
for kind in [19]:
 for i in range(384):
  ...
  q['globalFlags']=(0x200000 if i%7==0 else 0)|(0x1000000 if i%11==0 else 0);q['state']['globalFlags']=q['globalFlags'];put(0x59d208,q['globalFlags'])
  q['reactionMode']=2 if i%5==0 else 0;q['feeBonus']=i%3;q['profileTiers']={'0':0}
  q['state'].update(performance=[0]*1600,feeUnits=0,cashUnits=100,feeLedgerIndex=i%2,feeLedger={'0':0,'1':1},moneyNoticeIndex=i%8,moneyNotices=[dict(x=0,z=0,units=0,ticks=0) for j in range(8)],feeMessageActor=0,feeTutorialStage=0)
  put(0x542c04,q['reactionMode']);q['state']['reactionMode']=q['reactionMode'];put(0x542be8,q['feeBonus']);put(0x4c1848,0);put(0x570a24,100);u.mem_write(0x5a5784,struct.pack('<h',i%2));u.mem_write(0x582c60,bytes(2));u.mem_write(0x582c74,struct.pack('<h',1));put(0x599600,i%8);put(0x4c1dfc,0);put(0x5a8714,0);u.mem_write(0x5698e0,bytes(6400))
  for addr in [0x541ee8,0x541f08,0x541ce8,0x541e10]:u.mem_write(addr,bytes(32))
  u.reg_write(UC_X86_REG_ESP,sp);u.reg_write(UC_X86_REG_EBX,0);u.reg_write(UC_X86_REG_EBP,0)
  try:u.emu_start(0x426b10,0x426f30,count=60000)
  except Exception:
  ...
  s['performance']=list(struct.unpack('<1600i',u.mem_read(0x5698e0,6400)))
  for key,addr in [('cashUnits',0x570a24),('feeUnits',0x4c1848),('feeMessageActor',0x4c1dfc),('feeTutorialStage',0x5a8714),('moneyNoticeIndex',0x599600)]:s[key]=struct.unpack('<i',u.mem_read(addr,4))[0]
  s['feeLedger']={str(j):struct.unpack('<h',u.mem_read(0x582c60+20*j,2))[0] for j in range(2)}
  s['moneyNotices']=[{key:struct.unpack('<i',u.mem_read(addr+j*4,4))[0] for key,addr in [('x',0x541ee8),('z',0x541f08),('units',0x541ce8),('ticks',0x541e10)]} for j in range(8)]
  rows.append([q,dict(state=s)])
module=(root/'simgolf-reborn/scene/src/simulation/original-audible-hole-completion-prefix.js').as_uri()
script="""import {readFileSync} from 'node:fs';import {isDeepStrictEqual} from 'node:util';import {originalAudibleHoleCompletionPrefix} from MODULE;import {originalActorName} from NAMES;const rows=JSON.parse(readFileSync(0,'utf8'));for(const [q,expected] of rows){for(const s of [q.state,expected.state]){for(const field of ['actors','holeRecords','completionProfiles'])for(const id in s[field])s[field][id]=Uint8Array.from(s[field][id]);for(const field of ['completionMarkers','terrain','tileGrowth','positive','negative'])s[field]=Uint8Array.from(s[field]);s.tileFlags=Uint16Array.from(s.tileFlags);s.performance=Int32Array.from(s.performance);}for(const field of ['profileRecords','holeRecords','profileHistory'])for(const id in q[field])q[field][id]=Uint8Array.from(q[field][id]);const result=originalAudibleHoleCompletionPrefix(q,{resolvePhrase:(e,s)=>{if(e.address===0x466fb0){s.sourceText=originalActorName({...q.names,actorId:e.args[0],actor:s.actors[e.args[0]],sourceText:s.sourceText,appendComma:!!e.args[1]});return s;}return {...s,remarkStyle:91,sourceText:s.sourceText.split('\\0',1)[0]+'Place'+e.args[0]+','+e.args[1]};},audioContext:()=>({camera:q.camera,zoom:q.zoom,map:{flagsAt:()=>q.terrain.flags,storedHeight:()=>q.terrain.stored,objectHeight:()=>q.terrain.object,cornerHeight:(c,r,d)=>q.terrain.corners[d]}}),playback:(e,s)=>s});const got={state:result.state};if(!isDeepStrictEqual(got,expected))throw Error(JSON.stringify({q,expected,got}));}console.log(`${rows.length} native full audible completion prefixes matched`);""".replace('MODULE',json.dumps(module)).replace('NAMES',json.dumps((root/'simgolf-reborn/scene/src/simulation/original-actor-name.js').as_uri()))
subprocess.run(['node','--input-type=module','-e',script],input=json.dumps(rows),text=True,check=True)
(root/'simgolf-reborn/scene/tests/fixtures/original-audible-hole-completion-prefix.json').write_text(json.dumps(rows,separators=(',',':'))+'\n')
