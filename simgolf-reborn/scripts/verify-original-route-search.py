"""Verify original search setup, passes, pruning and final publication together."""
...
for a,n in [(0x42252c,0x108c),(0x466a00,0x50),(0x491380,0x3c),(0x4913e0,0x10b),(0x466b40,0x59),(0x4baa48,16),(0x466ba0,0x10e),(0x40bc50,0x33),(0x40a9f0,0x81),(0x40c1a0,0x42),(0x4a57a0,0x27),(0x4c1870,64)]:
...
def run(q):
 ...
 u.emu_start(0x42252c,0x4235b8,count=5000000)
 assert u.reg_read(UC_X86_REG_EIP)==0x423582
...
for _ in range(30):
 ...
 if _==29:
  q['scores']=[[100000]*6 for i in range(441)];q['winner']['target']['x']=-1;q['winner']['cornerTarget']=1
 rows.append([q,run(q),calls])
module=(root/'simgolf-reborn/scene/src/simulation/original-route-search.js').as_uri()
script='''import {readFileSync} from 'node:fs';import {isDeepStrictEqual} from 'node:util';const {originalRouteSearch}=await import(MODULE);
...
for(const [q,e,expectedCalls] of rows){const cells=new Map(q.cells.map(([x,z,code,shotClass,flags])=>[`${x},${z}`,{code,shotClass,flags}]));
 let index=0;const calls=[];const a=originalRouteSearch({...q,assessShot:c=>{calls.push({type:"assess",...c});return q.costs[c.shape+1]+c.flag*3;},terrainAt:p=>cells.get(`${p.x},${p.z}`)},c=>{calls.push({type:'range',...c});return q.followingRange;},c=>{calls.push({type:"simulate",...c});return {landing:q.landings[(index++)%q.landings.length]};});
 if(!isDeepStrictEqual(a,e)||JSON.stringify(calls)!==JSON.stringify(expectedCalls))throw Error(JSON.stringify({a,e}));
}console.log(`${rows.length} combined route searches and ordered calls match original x86.`);
...
if '--write-fixture' in __import__('sys').argv:
 ...
 (root/'simgolf-reborn/scene/tests/fixtures/original-route-search.json').write_text(json.dumps(fixtures,separators=(',',':'))+'\n')
