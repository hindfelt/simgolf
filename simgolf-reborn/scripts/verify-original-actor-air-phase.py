...
current=None;draws=0;sound=None;calls=[]
def hook(u,a,size,data):
 global draws,sound
 if a==0x45ba70:draws+=1
 if a not in [0x42f110,0x40c1f0,0x4672d0]:return
 count={0x42f110:2,0x40c1f0:4,0x4672d0:3}[a]
...
for i in range(3000):
 ...
 e=dict(actor=list(u.mem_read(0x577f00,256)),seed=read(0x820454),calls=calls,hit=sound is not None,randomDraws=draws)
 rows.append([q,before,e])
module=(root/'simgolf-reborn/scene/src/simulation/original-actor-air-phase.js').as_uri()
script="""import {readFileSync} from 'node:fs';import {isDeepStrictEqual} from 'node:util';
const {originalActorAirPhase}=await import(MODULE);const rows=JSON.parse(readFileSync(0,'utf8'));let effects=0,hits=0;
for(const [q,before,e] of rows){
 const state={actors:[new Uint8Array(before)],actorId:0,previousTerrainHeight:q.previousTerrainHeight,ballTile:{x:q.cellX,z:q.cellZ},ballTerrain:q.terrainCode,terrainFlags:q.terrainFlags,variant:q.variant,luck:q.luck,seed:q.seed};
 const r=originalActorAirPhase(state,(event,state)=>{
  ...
  return {state,value:q.terrainHeight};
 });
 const a={actor:Array.from(r.state.actors[0]),seed:r.state.seed,calls:r.calls,hit:r.hit,randomDraws:r.randomDraws};
 if(!isDeepStrictEqual(a,e))throw Error(JSON.stringify({q,a,e}));effects+=r.calls.length;hits+=Number(r.hit);
}
console.log(`${rows.length} native airborne actor phases match; ${effects} ordered effects, ${hits} collisions.`);
