"""Continuous native actor decisions through walking or a shot continuation.
...
for a in [0x40dc70,0x4672d0,0x466ea0,0x406dd0,0x465c40,0x406450,0x426b00,0x42f270,0x42f020,0x425b50,0x40c140]:u.mem_write(a,b'\xc3')
...
def hook(u,a,size,data):
 ...
 if a in [0x42b55c,0x42b825,0x42d23c,0x42bdb5,0x4295ef]:
...
for i in range(1500):
 ...
 trace=[];calls=[];draws=0;sample=None;next_branch=None;point_index=0;turn_seen=False;put(0x102050,0);u.reg_write(UC_X86_REG_FPCW,0x37f);u.reg_write(UC_X86_REG_ESP,0x102000);u.reg_write(UC_X86_REG_EBP,slot*256);u.reg_write(UC_X86_REG_EBX,slot);u.emu_start(0x42819c,0x400fff,count=10000000)
 ...
 rows[-1]['expected'].update(globalFlags=read(0x59d208),actors=list(u.mem_read(0x577f00,152*256)),holes=list(u.mem_read(0x574500,20*520)),visitorAssignments=list(u.mem_read(0x567698,800)),avoidanceCursor=read(0x5a446c),selectionState=struct.unpack('<i',u.mem_read(0x5a4440,4))[0])
for row in rows:
