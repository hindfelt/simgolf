...
def hook(u,a,size,data):
 ...
 if a in [0x425b50,0x40daa0,0x40db60,0x46c140,0x4672d0,0x40c1f0,0x40c140,0x42def0]:
  sp=u.reg_read(UC_X86_REG_ESP);args=[get(sp+4*j) for j in range(1,2 if a in [0x425b50,0x46c140] else 6 if a==0x42def0 else 5 if a==0x40c1f0 else 4)];calls.append(dict(address=a,args=args))
  if a==0x425b50:
  ...
  if a in [0x46c140,0x4672d0,0x40c1f0,0x40c140]:
...
for address in [0x429f4c,0x4295ef,0x42bdb5,0x42a019,0x42adac,0x42d23c,0x42b825,0x425b50,0x40daa0,0x40db60,0x46c140,0x4672d0,0x40c1f0,0x40c140,0x42def0]:u.hook_add(UC_HOOK_CODE,hook,begin=address,end=address)
