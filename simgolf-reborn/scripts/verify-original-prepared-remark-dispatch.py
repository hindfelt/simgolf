...
for a,n in [(0x4672d0,0x2a4),(0x406b20,0x1c9),(0x466fb0,0x320),(0x46c104,58),(0x45b090,0xbd),(0x466440,0x592),(0x469250,0xd4),(0x407050,0xc0),(0x45b2c0,0x55),(0x40a6c0,0xdc),(0x46c140,0x2c),(0x466e30,0x6c),(0x469330,0x3008),(0x4c0000,0x30000)]:u.mem_write(a,p.get_data(a-0x400000,n))
...
for a in [0x40c1f0,0x4a5c60,0x4a58bd,0x4a5d5e,0x4a580f,0x4acb95,0x4074d0]:u.mem_write(a,b'\xc3')
...
for kind in kinds:
 for i in range(2048 if kind==50 else 4096 if kind in [19,23] else 640 if kind==59 else 384):
  ...
  dispatchEvents=[];phraseEvents=[];displayEvents=[];preparationEvents=[];put(0x102120,0x401000);put(0x102124,id);put(0x102128,kind);put(0x10212c,value);u.reg_write(UC_X86_REG_ESP,0x102120);u.emu_start(0x4672d0,0x467502,count=30000);assert u.reg_read(UC_X86_REG_EIP) in [0x467502,0x469075];advanced=u.reg_read(UC_X86_REG_EIP)==0x467502;allowed=bool(dispatchEvents)
  ...
  rows.append([q,dict(state=expected,events=dispatchEvents,phraseEvents=phraseEvents,displayEvents=displayEvents,receiver=receiver if allowed else None,allowed=allowed,kind=dispatchEvents[0]['args'][0] if allowed else kind,next='adjustment' if advanced else 'return',before=list(u.mem_read(0x102020,256)) if advanced else None,voiceOffset=u.reg_read(UC_X86_REG_ESI) if advanced else None,preparationEvents=preparationEvents)])
module=(root/'simgolf-reborn/scene/src/simulation/original-remark-dispatch.js').as_uri()
script="""import {readFileSync} from 'node:fs';import {isDeepStrictEqual} from 'node:util';import {originalPreparedRemarkDispatch} from MODULE;import {originalActorName} from NAMES;const rows=JSON.parse(readFileSync(0,'utf8'));for(const [q,expected] of rows){if(q.profileHistory)for(const id in q.profileHistory)q.profileHistory[id]=Uint8Array.from(q.profileHistory[id]);if(q.holeRecords)for(const id in q.holeRecords)q.holeRecords[id]=Uint8Array.from(q.holeRecords[id]);if(q.profileRecords)for(const id in q.profileRecords)q.profileRecords[id]=Uint8Array.from(q.profileRecords[id]);for(const s of [q.state,expected.state])for(const id in s.actors)s.actors[id]=Uint8Array.from(s.actors[id]);if(expected.before)expected.before=Uint8Array.from(expected.before);const got=originalPreparedRemarkDispatch(q,(e,s)=>{if(e.address===0x466fb0){s.sourceText=originalActorName({...q.names,actorId:e.args[0],actor:s.actors[e.args[0]],sourceText:s.sourceText,appendComma:!!e.args[1]});return s;}return {...s,remarkStyle:12345,sourceText:s.sourceText.split('\\0',1)[0]+(e.address===0x466fb0?'Name'+e.args[0]:'Place'+e.args[2])};},()=>q.resourceLines,(e,s)=>s);if(!isDeepStrictEqual(got,expected))throw Error(JSON.stringify({kind:q.kind,value:q.value,expected:{...expected,state:{...expected.state,actors:null}},got:{...got,state:{...got.state,actors:null}}}));}console.log(`${rows.length} native prepared remark-dispatch cases matched`);""".replace('MODULE',json.dumps(module)).replace('NAMES',json.dumps((root/'simgolf-reborn/scene/src/simulation/original-actor-name.js').as_uri()))
...
(root/'simgolf-reborn/scene/tests/fixtures/original-prepared-remark-dispatch.json').write_text(json.dumps(fixture,separators=(',',':'))+'\n')
