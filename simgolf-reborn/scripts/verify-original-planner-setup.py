"""Compare reconstructed range with original x86, planner entry and range, supplying only map surface lookup."""
...
for address,size in [(0x4235c0,0x9d),(0x4219e0,0x168),(0x466a00,0x1d)]:
...
def run(q):
 ...
 for c,v in enumerate(q['classes']):write(0x576dc2+48*c,v,1)
 ...
 write(base+0xdc,q['x'],4);write(base+0xe0,q['z'],4);write(base+0x18,q['actorFlags'],4)
 write(0x59d208,q['worldFlags'],4);write(0x53ce64,q['obstacleCount'],4)
 index=(q['x']>>10)*50+(q['z']>>10);write(0x570d38+index,q['surface'],1)
 u.mem_write(0x102000,struct.pack('<6I',0x400fff,q['actorId'],0,0,0,0))
 u.reg_write(UC_X86_REG_ESP,0x102000);u.emu_start(0x4235c0,0x42365d,count=1000)
 sp=u.reg_read(UC_X86_REG_ESP)
 def read(a):return struct.unpack('<i',u.mem_read(a,4))[0]
 patches=[dict(code=c,shotClass=u.mem_read(0x576dc2+48*c,1)[0]) for c in [17,20]] if q['actorFlags']&1 else []
 return dict(range=read(sp+0x30),originTile=dict(x=read(sp+0x34),z=read(sp+0x40)),originIndex=read(sp+0x50),terrainCode=read(sp+0x14),obstacleIndex=read(0x4c1fbc),shotClassOverrides=patches)
...
for _ in range(5000):
 ...
 q.update(x=rng.randrange(51200),z=rng.randrange(51200),actorFlags=rng.randrange(4),worldFlags=rng.randrange(256),obstacleCount=rng.randrange(-2,20),classes=[rng.randrange(-2,6) for _ in range(23)])
 rows.append([q,run(q)])
if '--write-fixture' in __import__('sys').argv:
 fixture=root/'simgolf-reborn/scene/tests/fixtures/original-planner-setup.json'
 fixture.write_text(json.dumps(rows[:40],indent=2)+'\n')
module=(root/'simgolf-reborn/scene/src/simulation/original-planner-setup.js').as_uri()
script='''import {readFileSync} from 'node:fs';
const {originalPlannerSetup}=await import(MODULE);
const rows=JSON.parse(readFileSync(0,'utf8'));
for(const [q,expected] of rows) {
 const actual=originalPlannerSetup({...q,rangeInput:q},{terrainAt:()=>q.surface,shotClassAt:c=>q.classes[c]});
 if(JSON.stringify(actual)!==JSON.stringify(expected))throw Error(JSON.stringify({q,expected,actual}));
}
console.log(`${rows.length} planner entry states and ranges match original x86 (surface lookup supplied).`);
