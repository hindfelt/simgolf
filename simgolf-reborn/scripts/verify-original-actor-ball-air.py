...
for i in range(3000):
 b=dict(x=20992+rng.randrange(-450,451),z=20992+rng.randrange(-450,451),height=rng.randrange(50,500),speed=rng.choice([rng.randrange(4000),rng.randrange(65536)]),verticalSpeed=rng.randrange(-200,500),heading=rng.randrange(2**32),angularOffset=rng.randrange(-100000000,100000000),seed=0)
 ...
 write(sp+0x18,20);u.mem_write(0x570d38,bytes([q['terrainCode']])*2500)
 ...
 end=None;u.emu_start(0x42bdc3,0x400fff,count=30000);assert end
...
const {originalActorBallMotion}=await import(MODULE);const rows=JSON.parse(readFileSync(0,'utf8'));let effects=0,hits=0;
for(const [q,before,e] of rows){
 ...
 const state={actors,terrain:new Uint8Array(2500).fill(q.terrainCode),holes:[null,h],bounceCoefficient:q.bounceCoefficient,scatterCoefficient:q.scatterCoefficient,visualSlot:-1,direction:0,boundaryFlags:0,difficulty:0,actorId:0,previousTerrainHeight:q.previousTerrainHeight,ballTile:{x:q.cellX,z:q.cellZ},ballTerrain:q.terrainCode,terrainFlags:q.terrainFlags,variant:q.variant,luck:q.luck,seed:q.seed};
 const r=originalActorBallMotion(state,(event,state)=>{
...
console.log(`${rows.length} continuous native moved airborne paths match; ${effects} ordered effects, ${hits} collisions.`);
