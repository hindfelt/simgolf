"""Verify prepared putt output through clearance, animation and first movement."""
...
motion_effects={0x42f110:2,0x4096e0:1,0x4672d0:3,0x40c1f0:4,0x409820:1,0x409780:1}
...
def motion_hook(v,a,n,d):
 global motion_done
 if a==0x42beb0:motion_done=True;v.emu_stop()
...
for q,e in rows:
 ...
 assert motion_done
 q['motionExpected']=dict(actors=list(v.mem_read(0x577f00,512)),seed=vr(0x820454)&0xffffffff,calls=motion_calls,animationTicks=animation_ticks,swingTicks=swing_ticks)
...
function verifyMovement(snapshot,q){let state={...structuredClone(snapshot),ballTerrain:q.startTerrain,visualSlot:-1},frameIndex=0,animationTicks=0,swingTicks=0;state.actors=Array.from({length:152},(_,i)=>state.actors[i]||new Uint8Array(256));state=originalSwingClearance({...state,swingOverride:0,roundClock:0}).state;const calls=[],resolve=(e,state)=>{calls.push(e);return {state,value:0};};while(state.actors[0][0x28]!==2){if(++animationTicks>10)throw Error('Animation timeout');const a=originalActorSwingAnimation(state,{animationDirection:0,frameCounts:Array(160).fill(8),previousFrame:state.actors[0][0x26],frameIndex});state=a.state;frameIndex=a.frameIndex;}for(swingTicks=1;swingTicks<10;swingTicks++){const a=originalSwingProgress(state,resolve);state=a.state;if(a.next==='motion'){state=originalActorPositionStep(state,resolve).state;break;}}const actual={actors:state.actors.slice(0,2).flatMap(b=>Array.from(b)),seed:state.seed,calls,animationTicks,swingTicks};if(!isDeepStrictEqual(actual,q.motionExpected))throw Error(JSON.stringify({scenario:q.scenario,actual,expected:q.motionExpected}));}
...
console.log('36 native planned putts match preparation, animation, impact and first movement.');
