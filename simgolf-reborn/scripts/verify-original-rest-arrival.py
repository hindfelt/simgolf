"""Native rest arrivals with controlled adjacency and reaction effects."""
...
def hook(u,a,size,data):
 ...
 if a==0x4071d0:attempts+=1;value=int(attempts>=q['acceptAfter'])
...
for i in range(1200):
 b=bytearray(rng.randbytes(256));struct.pack_into('<h',b,0xb2,rng.choice([-32768,-1,0,1,2,20,100,32767]));q=dict(actorId=0,actorTile=dict(x=23,z=24),restDecoration=rng.randrange(256),seed=rng.getrandbits(32),acceptAfter=rng.randrange(1,10))
 ...
 rows.append([q,list(b),dict(actor=list(u.mem_read(0x577f00,256)),seed=get(0x820454)&0xffffffff,calls=calls)])
script="""import {readFileSync} from 'node:fs';import {isDeepStrictEqual} from 'node:util';const {originalRestArrival}=await import(MODULE);const rows=JSON.parse(readFileSync(0,'utf8'));for(const [q,b,e] of rows){let attempts=0;const r=originalRestArrival({...q,actors:[new Uint8Array(b)]},(event,state)=>{if(event.address===0x4672d0)state.actors[0][0x8d]=event.args[1];return {state,value:event.address===0x4071d0?Number(++attempts>=q.acceptAfter):0};});const a={actor:Array.from(r.state.actors[0]),seed:r.state.seed,calls:r.calls};if(!isDeepStrictEqual(a,e))throw Error(JSON.stringify({q,fields:Object.keys(e).filter(k=>!isDeepStrictEqual(a[k],e[k]))}));}console.log(`${rows.length} native rest arrivals match actors, RNG and ordered effects.`);""".replace('MODULE',json.dumps((root/'simgolf-reborn/scene/src/simulation/original-rest-arrival.js').as_uri()))
