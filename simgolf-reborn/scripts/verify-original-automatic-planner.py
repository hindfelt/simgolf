...
for q,_ in json.loads((root/'simgolf-reborn/scene/tests/fixtures/original-search-launch.json').read_text()):
 ...
 rows.append([q,final]);print('Native outer planner finished',q['scenario'],q['mode'],flush=True)
...
for(const [q,e] of JSON.parse(readFileSync(0,'utf8'))){const {searched,...a}=originalAutomaticPlanner(q.automatic,{map:searchLaunchMap(q),physical:{luck:5},searchState:q.outerSearchState,score:0,scoreAt:(x,z)=>q.aimScores[x*50+z]},{emit:(_event,state)=>state});if(!isDeepStrictEqual(a,e)){
...
console.log('Six complete automatic planners match uninterrupted native outer execution.');
""".replace('PLANNER',json.dumps((root/'simgolf-reborn/scene/src/simulation/original-automatic-planner.js').as_uri())).replace('MAP',json.dumps((root/'simgolf-reborn/scene/tests/helpers/original-search-launch.js').as_uri()))
