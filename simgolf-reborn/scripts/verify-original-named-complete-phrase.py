"""Contiguous phrase entry, standard selection and substitution; actual actor naming; controlled location/CRT."""
...
for a,n in [(0x466fb0,0x320),(0x46c104,58),(0x45b090,0xbd),(0x466440,0x592),(0x469250,0xd4),(0x407050,0xc0),(0x45b2c0,0x55),(0x40a6c0,0xdc),(0x46c140,0x2c),(0x466e30,0x6c),(0x469330,0x3008),(0x4c0000,0x30000)]:u.mem_write(a,p.get_data(a-0x400000,n))
...
script="""import {readFileSync} from 'node:fs';import {isDeepStrictEqual} from 'node:util';import {originalCompletePhrase} from MODULE;import {originalActorName} from NAMES;const rows=JSON.parse(readFileSync(0,'utf8'));for(const [q,expected] of rows){if(q.profileHistory)for(const id in q.profileHistory)q.profileHistory[id]=Uint8Array.from(q.profileHistory[id]);if(q.holeRecords)for(const id in q.holeRecords)q.holeRecords[id]=Uint8Array.from(q.holeRecords[id]);if(q.profileRecords)for(const id in q.profileRecords)q.profileRecords[id]=Uint8Array.from(q.profileRecords[id]);for(const s of [q.state,expected.state])for(const id in s.actors)s.actors[id]=Uint8Array.from(s.actors[id]);const got=originalCompletePhrase(q,(e,s)=>{if(e.address===0x466fb0){s.sourceText=originalActorName({...q.names,actorId:e.args[0],actor:s.actors[e.args[0]],sourceText:s.sourceText,appendComma:!!e.args[1]});return s;}return {...s,remarkStyle:12345,sourceText:s.sourceText.split('\\0',1)[0]+(e.address===0x466fb0?'Name'+e.args[0]:'Place'+e.args[2])};},()=>q.resourceLines);if(!isDeepStrictEqual(got,expected))throw Error(JSON.stringify({kind:q.kind,value:q.value,expected:{...expected,state:{...expected.state,actors:null}},got:{...got,state:{...got.state,actors:null}}}));}console.log(`${rows.length} native named complete-phrase cases matched`);""".replace('MODULE',json.dumps(module)).replace('NAMES',json.dumps((root/'simgolf-reborn/scene/src/simulation/original-actor-name.js').as_uri()))
...
(root/'simgolf-reborn/scene/tests/fixtures/original-named-complete-phrase.json').write_text(json.dumps(fixture,separators=(',',':'))+'\n')
