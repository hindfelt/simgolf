"""Verify original route search from entry initialization through publication."""
...
for a,n in [(0x42245e,0x115a),(0x466a00,0x50),(0x491380,0x3c),(0x4913e0,0x10b),(0x466b40,0x59),(0x4baa48,16),(0x466ba0,0x10e),(0x40bc50,0x33),(0x40a9f0,0x81),(0x40c1a0,0x42),(0x4a57a0,0x27),(0x4c1870,64)]:
...
def hook(u,a,s,d):
 ...
 if a==0x4219e0:
  ...
  if ret==0x4224d0:
   calls.append(dict(type='initialRange',actorId=read(sp+4),worldFlags=read(0x59d208)&0xffffffff,candidateSkillMask=read(0x4c1e0c),searchActive=read(0x59a188),searchFlag=read(0x5a872c),cornerTarget=read(0x5a8730)))
   u.reg_write(UC_X86_REG_EAX,current['range']);u.reg_write(UC_X86_REG_ESP,sp+4);u.reg_write(UC_X86_REG_EIP,ret);return
  calls.append(dict(type='range',shotCounter=u.mem_read(0x577f2a,1)[0],target=dict(x=read(0x577fd4),z=read(0x577fd8)),heading=read(0x577fe8)&0xffffffff,mode=read(0x5a870c)))
  u.reg_write(UC_X86_REG_EAX,current['followingRange']);u.reg_write(UC_X86_REG_ESP,sp+4);u.reg_write(UC_X86_REG_EIP,ret)
...
script='''import {readFileSync} from 'node:fs';import {isDeepStrictEqual} from 'node:util';const {originalEnteredRouteSearch}=await import(MODULE);
...
for(const [q,e,expectedCalls] of rows){const cells=new Map(q.cells.map(([x,z,code,shotClass,flags])=>[`${x},${z}`,{code,shotClass,flags}]));
 let index=0;const calls=[];const a=originalEnteredRouteSearch({...q,assessShot:c=>{calls.push({type:"assess",...c});return q.costs[c.shape+1]+c.flag*3;},terrainAt:p=>cells.get(`${p.x},${p.z}`)},c=>{calls.push({type:'initialRange',...c});return q.range;},c=>{calls.push({type:'range',...c});return q.followingRange;},c=>{calls.push({type:"simulate",...c});return {landing:q.landings[(index++)%q.landings.length]};});
 if(!isDeepStrictEqual(a,e)||JSON.stringify(calls)!==JSON.stringify(expectedCalls))throw Error(JSON.stringify({a,e}));
}console.log(`${rows.length} entered route searches and ordered calls match original x86.`);
...
if '--write-fixture' in __import__('sys').argv:
 ...
 (root/'simgolf-reborn/scene/tests/fixtures/original-entered-route-search.json').write_text(json.dumps(fixtures,separators=(',',':'))+'\n')
