...
a=0x46c140;o=p.get_offset_from_rva(a-0x400000);u.mem_write(a,p.__data__[o:o+0x2c])
...
def run(q):
 ...
 for a,v in [(0x577fb4+s,actor['stateCode']),(0x577fbe+s,actor['profileIndex']),(0x577faa+s,q['actorId']^1)]:u.mem_write(a,struct.pack('<h',v))
 u.mem_write(0x577fbe+t,struct.pack('<h',q['otherProfileIndex']))
 ...
 result.update(actorFlags=read(0x577f18+s)&0xffffffff,angularOffset=read(0x577ff4+s),stateCode=actor['stateCode'],profileIndex=actor['profileIndex'])
...
for i in range(2000):
 ...
 actor=dict(marker=marker,reaction=int(i%11==0),actorClass=int(i%7==0),hole=rng.randrange(1,19),shotCounter=rng.randrange(4),skillMask=rng.randrange(8),actorFlags=rng.randrange(2**32),angularOffset=rng.choice([-1966081,-1966080,-983041,-983040,0,983040,983041,1966080,1966081]),stateCode=4 if i%3 else 0,profileIndex=rng.randrange(4))
 partner=dict(reaction=int(i%13==0),actorClass=int(i%17==0))
 q=dict(actorId=actorId,previousMarker=marker if i%9 else (marker+1)%256,terrainCode=2,shotClass=rng.randrange(-2,4),sceneryTile=30 if i%8==0 else 0,curveArgument=[-1,0,1][i%3],distance=[75,76,100,101][i%4],profileHoleMark=i%2,otherProfileIndex=rng.randrange(4),profileBytes=[(i+j*67)%256 for j in range(4)],effect=['none','marker','reaction'][i%3],state=dict(actor=actor,partner=partner))
 if i%10==1:
  actor.update(reaction=0,actorClass=0,shotCounter=3,hole=2+(actorId&1));partner.update(reaction=0,actorClass=0)
  q.update(effect='none',previousMarker=marker)
 rows.append([q,run(q)])
...
for(const [q,e] of JSON.parse(readFileSync(0,'utf8'))){const a=originalAutoShotReactions(q,{
 shotClassAt:()=>q.shotClass,profileHoleMarkAt:()=>q.profileHoleMark,profileIndexFor:id=>id===q.actorId?q.state.actor.profileIndex:q.otherProfileIndex,profileByteAt:index=>q.profileBytes[index],
...
console.log('2000 original shot reaction stages with native profile classification match state, flags and ordered requests.');
""".replace('MODULE',json.dumps(module))
subprocess.run(['node','--input-type=module','-e',script],input=json.dumps(rows),text=True,check=True)
groups=[]
for index in [0,1,76,83]:
 for value in range(256):
  actor_id=value%152;u.mem_write(0x577fbe+actor_id*256,struct.pack('<h',index))
  u.mem_write(0x4d5061+index*560,bytes([value]))
  write(0x102000,0x400fff);write(0x102004,actor_id);u.reg_write(UC_X86_REG_ESP,0x102000)
  u.emu_start(0x46c140,0x400fff,count=100);assert u.reg_read(UC_X86_REG_EIP)==0x400fff
  groups.append([actor_id,index,value,u.reg_read(UC_X86_REG_EAX)])
group_module=(root/'simgolf-reborn/scene/src/simulation/original-profile-group.js').as_uri()
group_script="""import {readFileSync} from 'node:fs';const {originalProfileGroup}=await import(MODULE);
for(const [id,index,value,expected] of JSON.parse(readFileSync(0,'utf8'))){
 const actual=originalProfileGroup(id,{profileIndexFor:actor=>{if(actor!==id)throw Error('Actor mismatch');return index;},
 profileByteAt:profile=>{if(profile!==index)throw Error('Profile mismatch');return value;}});
 if(actual!==expected)throw Error(JSON.stringify({id,index,value,actual,expected}));
}console.log('1024 native profile classifications match, covering every byte at four profile indices.');
""".replace('MODULE',json.dumps(group_module))
subprocess.run(['node','--input-type=module','-e',group_script],input=json.dumps(groups),text=True,check=True)
