...
script="""import {readFileSync} from 'node:fs';import {isDeepStrictEqual} from 'node:util';const {originalGolferLoop}=await import(MODULE);
for(const row of JSON.parse(readFileSync(0,'utf8'))){const actual=originalGolferLoop(row.q,(slot,s)=>{s.seed=(s.seed+slot+1)>>>0;if(slot<151)s.actors[slot+1].holeByte=1;return s;});if(!isDeepStrictEqual(actual,row.expected))throw Error(JSON.stringify({row,actual}));}
