"""Continuous complete remark with personal phrases, native reactions, actor names and popup RNG; inert speech and controlled location/search."""
...
for a,n in [(0x4672d0,0xe90),(0x469080,0xdd),(0x4a0000,1),(0x466a20,0x19),(0x466a00,0x20),(0x406b20,0x1c9),(0x466fb0,0x320),(0x40c7f0,0xf7),(0x45ba70,0x60),(0x4b9800,8),(0x4a57a0,0x27),(0x46806a,0x102b),(0x469160,0xe9),(0x469330,0x3008),(0x46c104,58),(0x45b090,0xbd),(0x46c140,0x2c),(0x4c0000,0x30000)]:u.mem_write(a,p.get_data(a-0x400000,n))
for a in [0x4acb95,0x447a30,0x40c1f0,0x4074d0,0x4a5c60]:u.mem_write(a,b'\xc3')
...
def hook(u,a,size,data):
 global phase,enteredExplanation,explanationSeed
 if a==0x46806a:
...
for kind in range(1,66):
 for i in range(48):
  ...
  u.mem_write(0x5a4448,bytes([q['state']['interfaceFlags']]));sp=0x104000;put(sp+0x124,0);put(sp+0x128,kind);put(sp+0x12c,value);put(sp+0x18,q['selectedDelta']);put(sp+0x1c,kind-1);u.reg_write(UC_X86_REG_ESP,sp);u.reg_write(UC_X86_REG_EBP,0);events=[];phraseEvents=[];phase='entry';enteredExplanation=False;explanationSeed=None
  ...
  rows.append([q,dict(reactionWorld=world,state=s,next='continue' if enteredExplanation else 'return',popupRandomDraws=0 if not enteredExplanation or s['seed']==explanationSeed else 2)])
module=(root/'simgolf-reborn/scene/src/simulation/original-complete-remark.js').as_uri()
script="""import {readFileSync} from 'node:fs';import {isDeepStrictEqual} from 'node:util';import {originalCompleteRemark} from MODULE;import {originalActorName} from NAMES;const rows=JSON.parse(readFileSync(0,'utf8'));for(const [q,expected] of rows){for(const s of [q.state,expected.state])for(const id in s.actors)s.actors[id]=Uint8Array.from(s.actors[id]);for(const field of ['profileRecords','holeRecords','profileHistory'])for(const id in q[field])q[field][id]=Uint8Array.from(q[field][id]);const result=originalCompleteRemark(q,{resolvePhrase:(e,s)=>{if(e.address===0x466fb0){s.sourceText=originalActorName({...q.names,actorId:e.args[0],actor:s.actors[e.args[0]],sourceText:s.sourceText,appendComma:!!e.args[1]});return s;}return {...s,remarkStyle:91,sourceText:s.sourceText.split('\\0',1)[0]+'Place'+e.args[0]+','+e.args[1]};},playSpeech:(e,s)=>s,reactionContext:()=>q.reactionContext,resolveEffect:(e,s)=>({state:s,result:0}),explanationContext:r=>({state:r.state,difficulty:q.difficulty})});const got={reactionWorld:expected.reactionWorld?Object.fromEntries(Object.keys(expected.reactionWorld).map(k=>[k,result.reaction.state[k]])):null,state:result.state,next:result.next,popupRandomDraws:result.explanation?.popupRandomDraws??0};if(!isDeepStrictEqual(got,expected))throw Error(JSON.stringify({kind:q.kind,expected,got}));}console.log(`${rows.length} native complete remark cases matched`);""".replace('MODULE',json.dumps(module)).replace('NAMES',json.dumps((root/'simgolf-reborn/scene/src/simulation/original-actor-name.js').as_uri()))
subprocess.run(['node','--input-type=module','-e',script],input=json.dumps(rows),text=True,check=True)
(root/'simgolf-reborn/scene/tests/fixtures/original-complete-remark.json').write_text(json.dumps(rows,separators=(',',':'))+'\n')
