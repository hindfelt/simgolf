...
for i in range(1200):
 ...
 b=bytearray(rng.randbytes(256));b[0x29]=rng.randrange(19);b[0x20]=rng.choice([0,0x20,0xa0,0xc0,0xe0]);struct.pack_into('<h',b,0xaa,partner);struct.pack_into('<h',b,0xbe,0)
...
script="""import {readFileSync} from 'node:fs';import {isDeepStrictEqual} from 'node:util';const {originalResolvedNextHole}=await import(MODULE);let effects=0,ends=0;for(const r of JSON.parse(readFileSync(0,'utf8'))){const q=r.q,actors=[];actors[q.actorId]=Uint8Array.from(q.actor);actors[151]=Uint8Array.from(q.partner);const a=originalResolvedNextHole({...q,actors,completionRecords:[new Uint8Array(44)],holeRecords:q.holes.map(n=>{const b=new Uint8Array(520);b[0]=n;return b;})});const actual={actor:Array.from(a.state.actors[q.actorId]),calls:a.calls,next:a.next,selectionState:a.state.selectionState};if(!isDeepStrictEqual(actual,r.expected))throw Error(JSON.stringify({r,actual}));effects+=a.calls.length;ends+=a.next==='return';}console.log(`1200 continuous native resolved next-hole cases with native round exit match; ${effects} reset calls, ${ends} returns.`);""".replace('MODULE',json.dumps(module))
