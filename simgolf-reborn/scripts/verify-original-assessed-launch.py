...
def run(q):
 ...
 u.emu_start(0x423b66,0x425ab9,count=100000)
...
for i in range(1000):
 ...
 rows.append([q,run(q)])
module=(root/'simgolf-reborn/scene/src/simulation/original-assessed-launch.js').as_uri()
script="""import {readFileSync} from 'node:fs';import {isDeepStrictEqual} from 'node:util';const {originalAssessedLaunch}=await import(MODULE);
const {originalStrengthCache}=await import(CACHE);let cache=originalStrengthCache();
const rows=JSON.parse(readFileSync(0,'utf8'));for(const [q,e] of rows){const a=originalAssessedLaunch(q,cache,{terrainAt:(x,z)=>q.terrain[x*50+z],kindAt:c=>q.kinds[c],shotClassAt:lie=>q.classes[lie+1],marksAt:(x,z)=>q.marks[x*50+z],heightAt:(x,z)=>q.heights[x*50+z]});if(!isDeepStrictEqual(a,e))throw Error(JSON.stringify({q:{distance:q.distance,skillMask:q.skillMask},a,e}));cache=a.cache;}
...
if '--write-fixture' in sys.argv:
 (root/'simgolf-reborn/scene/tests/fixtures/original-assessed-launch.json').write_text(json.dumps(rows[:20],separators=(',',':'))+'\n')
