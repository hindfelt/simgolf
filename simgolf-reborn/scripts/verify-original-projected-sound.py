"""Compare positional sound dispatch through real screen projection, with controlled terrain/playback boundaries."""
...
for a,n in [(0x40c1f0,0x1e7),(0x42f270,0x485),(0x466a00,0x20),(0x45ba70,0x60),(0x4b9800,8),(0x4a57a0,0x27),(0x4c1f94,36)]:
...
def hook(u,a,size,data):
 ...
 if a==0x42eb90:
  put(get(sp+12),0);put(get(sp+16),q['terrain']['object'])
 if a==0x40bcd0:
  u.reg_write(UC_X86_REG_EAX,q['terrain']['corners'][str(get(sp+12))]&0xffffffff)
...
for i in range(2400):
 ...
 sp=0x102000
 for a,v in [(sp,0x401000),(sp+4,q['soundId']),(sp+8,q['x']),(sp+12,q['z']),(sp+16,q['duration']),(0x5a8704,q['zoom']),(0x4c183c,q['camera']['scale']),(0x820454,q['state']['seed']),(0x5a8720,q['state']['queued']),(0x5a8724,q['state']['sequenceIndex'])]:put(a,v)
 u.reg_write(UC_X86_REG_ESP,sp);events=[];draws=0;u.emu_start(0x40c1f0,0x401000,count=2500);assert u.reg_read(UC_X86_REG_EIP)==0x401000
...
script="""import {readFileSync} from 'node:fs';import {isDeepStrictEqual} from 'node:util';import {originalPositionalSoundAt} from MODULE;const rows=JSON.parse(readFileSync(0,'utf8'));for(const [q,expected] of rows){const got=originalPositionalSoundAt(q,{flagsAt:()=>q.terrain.flags,storedHeight:()=>q.terrain.stored,objectHeight:()=>q.terrain.object,cornerHeight:(c,r,d)=>q.terrain.corners[d]},(event,state)=>{if(q.mutate){state.seed=777;state.queued=99;state.sequenceIndex=123;}return state;});if(!isDeepStrictEqual(got,expected))throw Error(JSON.stringify({q,expected,got}));}console.log(`${rows.length} native projected positional-sound cases matched`);""".replace('MODULE',json.dumps(module))
subprocess.run(['node','--input-type=module','-e',script],input=json.dumps(rows),text=True,check=True)
(root/'simgolf-reborn/scene/tests/fixtures/original-projected-sound.json').write_text(json.dumps(rows[:144],separators=(',',':'))+'\n')
