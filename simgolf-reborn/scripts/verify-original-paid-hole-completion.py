"""Continuous native settlement, payment and completion, controlled external effects."""
...
counts={0x405e80:2,0x40c1f0:4,0x466fb0:2,0x40c7f0:3,0x40c580:4,0x4672d0:3}
...
for i in range(1200):
 ...
 q=dict(actor=list(b),hole=list(hole),stat=list(stat),record=list(record),periods=list(periods),notices=list(notices),globalFlags=flags,settlementMode=mode,settlementBonus=bonus,cashTotal=200,periodIndex=0,phaseCounter=100,recordHolder=-1,presentationMode=0,sourceText='Before')
 ...
 put(0x102004,0);calls=[];branch=None;u.reg_write(UC_X86_REG_ESP,0x102000);u.emu_start(0x426b00,0x400fff,count=30000)
 def h(addr,n):return hashlib.sha256(bytes(u.mem_read(addr,n))).hexdigest()
 rows.append(dict(q=q,mutate=mutate,expected=dict(actor=list(u.mem_read(0x577f00,256)),record=list(u.mem_read(0x583430,44)),holes=h(0x574500,520*20),stats=h(0x5698c0,184*32),periods=h(0x582c60,40),notices=h(0x5842b2,76*64),value=read(0x4c1848),cashTotal=read(0x570a24),periodIndex=struct.unpack('<h',u.mem_read(0x5a5784,2))[0],phaseCounter=read(0x831828),recordHolder=read(0x4c1dfc),presentationMode=read(0x5a8714),sourceText=text(),calls=calls,next=branch)))
module=(root/'simgolf-reborn/scene/src/simulation/original-paid-hole-completion.js').as_uri()
script="""import {readFileSync} from 'node:fs';import {createHash} from 'node:crypto';import {isDeepStrictEqual} from 'node:util';const {originalPaidHoleCompletion}=await import(MODULE);const hash=rows=>createHash('sha256').update(Buffer.concat(rows.map(b=>Buffer.from(b)))).digest('hex');for(const [i,r] of JSON.parse(readFileSync(0,'utf8')).entries()){const q=r.q;const a=originalPaidHoleCompletion({...q,actorId:0,actors:[Uint8Array.from(q.actor)],holeRecords:Array.from({length:20},()=>Uint8Array.from(q.hole)),statRecords:Array.from({length:32},()=>Uint8Array.from(q.stat)),completionRecords:[Uint8Array.from(q.record)],financialPeriods:Array.from({length:2},(_,i)=>Uint8Array.from(q.periods.slice(i*20,i*20+20))),completionNotices:Array.from({length:64},(_,i)=>Uint8Array.from(q.notices.slice(i*76,i*76+76)))},(e,state)=>{if(e.address===0x466fb0)state.sourceText='Gary';if(r.mutate){if(e.address===0x405e80){state.actors[0][0x21]=2;state.actors[0][0x29]=2;state.actors[0][0xc2]=1;}if(e.address===0x40c1f0)state.settlementValue=(state.settlementValue+1)|0;if(e.address===0x40c7f0){state.settlementValue=-9;state.periodIndex=1;}if(e.address===0x4672d0){state.actors[0][0x29]=3;state.actors[0][0x2a]=6;state.phaseCounter=120;}}return {state};});const s=a.state,actual={actor:Array.from(s.actors[0]),record:Array.from(s.completionRecords[0]),holes:hash(s.holeRecords),stats:hash(s.statRecords),periods:hash(s.financialPeriods),notices:hash(s.completionNotices),value:s.settlementValue,cashTotal:s.cashTotal,periodIndex:s.periodIndex,phaseCounter:s.phaseCounter,recordHolder:s.recordHolder,presentationMode:s.presentationMode,sourceText:s.sourceText,calls:a.calls,next:a.next};if(!isDeepStrictEqual(actual,r.expected))throw Error(JSON.stringify({i,r,actual}));}console.log('1200 continuous native paid-hole completion cases match.');""".replace('MODULE',json.dumps(module))
