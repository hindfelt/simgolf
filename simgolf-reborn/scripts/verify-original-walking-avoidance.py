"""Native near-walking avoidance; reaction/identity effects controlled."""
from pathlib import Path
import hashlib,json,random,struct,subprocess,pefile
from unicorn import Uc,UC_ARCH_X86,UC_MODE_32,UC_HOOK_CODE
from unicorn.x86_const import UC_X86_REG_ESP,UC_X86_REG_EBP,UC_X86_REG_EAX,UC_X86_REG_EIP,UC_X86_REG_FPCW
...
def hook(u,a,size,data):
 global end
 if a in [0x42af66,0x42ad32,0x42ad3b]:end=hex(a);u.emu_stop()
...
for i in range(700):
 ...
 u.mem_write(0x577f00,bytes(b));put(0x5a446c,q['avoidanceCursor']);put(0x568f6c,100);put(0x820454,q['seed']);put(0x102010,id);put(0x102024,q['followPartner']);u.reg_write(UC_X86_REG_ESP,0x102000);u.reg_write(UC_X86_REG_EBP,id*256);u.reg_write(UC_X86_REG_FPCW,0x37f);end=None;calls=[];u.emu_start(0x42a793,0x400fff,count=100000);assert end
...
script="""import {readFileSync} from 'node:fs';import {isDeepStrictEqual} from 'node:util';const {originalWalkingAvoidance}=await import(MODULE);const rows=JSON.parse(readFileSync(0,'utf8'));const coverage={};for(const [q,b,e] of rows){const r=originalWalkingAvoidance({...q,actors:Array.from({length:152},(_,i)=>new Uint8Array(b.slice(i*256,(i+1)*256)))},(event,state)=>{if(event.address===0x4672d0)state.actors[event.args[0]][0x78]=36;return {state,value:1};});const a={actors:r.state.actors.flatMap(b=>Array.from(b)),movementReady:r.movementReady,crowdCount:r.crowdCount,scanIndex:r.scanIndex,seed:r.state.seed,calls:r.calls,next:r.next};if(!isDeepStrictEqual(a,e))throw Error(JSON.stringify({q,fields:Object.keys(e).filter(k=>!isDeepStrictEqual(a[k],e[k]))}));coverage[r.next]=(coverage[r.next]??0)+1;}console.log(`${rows.length} native avoidance cases match. ${JSON.stringify(coverage)}`);""".replace('MODULE',json.dumps((root/'simgolf-reborn/scene/src/simulation/original-walking-avoidance.js').as_uri()))
