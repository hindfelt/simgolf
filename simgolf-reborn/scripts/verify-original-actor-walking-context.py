"""Continuous native actor decisions through partner turn selection.
...
for a in [0x40dc70,0x4672d0,0x466ea0,0x406dd0,0x465c40,0x406450,0x426b00,0x42f270,0x42f020,0x425b50]:u.mem_write(a,b'\xc3')
...
for i in range(1500):
 ...
 q=dict(globalFlags=global_flags,holeTargets=[dict(x=25,z=35)]*3,actorId=slot,actor=list(actor),partner=list(partner),visualOwners=owners,difficulty=-1,focusActor=-1,lastPairClock=0,modeByte=0,detailLevel=4,environmentByte=0,conditionRange=20,metadata=[{}, {'shape':1}],trackedX=-1,trackedZ=-1,trackedFacing=0,phaseCounter=phase,seed=seed,code=code,flags=flags,building=list(b))
 ...
 rows.append(dict(q=q,points=points,response=response,replacement=replacement,expected=dict(walkingContext=dict(entryValue=u.reg_read(UC_X86_REG_EAX),movementReady=read(0x102044),walkingOverride=read(0x102034),nextTerrain=read(0x10202c),reversalCheck=read(0x10202c),cachedFlags=read(0x102050)) if next_branch=='0x428ad1' else None,actor=list(u.mem_read(base,256)),partner=list(u.mem_read(base+256,256)),visualOwners=[struct.unpack('<h',u.mem_read(0x59e6b0+j*0x388,2))[0] for j in range(16)],focusActor=struct.unpack('<i',u.mem_read(0x4c1dfc,4))[0],trackedX=struct.unpack('<i',u.mem_read(0x5842a0,4))[0],trackedZ=struct.unpack('<i',u.mem_read(0x5842a4,4))[0],trackedFacing=u.mem_read(0x5842b6,1)[0],visualSlot=struct.unpack('<i',u.mem_read(0x102030,4))[0],seed=read(0x820454),flags=struct.unpack('<H',u.mem_read(0x53ba00,2))[0],calls=calls,randomDraws=draws,ballTile=dict(x=struct.unpack('<i',u.mem_read(0x102018,4))[0],z=struct.unpack('<i',u.mem_read(0x102020,4))[0]),ballTerrain=read(0x102014),actorIndex=read(0x10203c),actorTerrain=read(0x10202c),next=next_branch,partnerNotReady=bool(read(0x102034)) if turn_seen else None,closerToCup=bool(read(0x102074)) if turn_seen else None)))
...
script="""import {readFileSync} from 'node:fs';import {isDeepStrictEqual} from 'node:util';const {originalActorDecision}=await import(MODULE);const {originalActorWalkingContext}=await import(CONTEXT);for(const r of JSON.parse(readFileSync(0,'utf8'))){const q=r.q,actors=[];actors[q.actorId]=Uint8Array.from(q.actor);actors[1]=Uint8Array.from(q.partner);actors[2]=Uint8Array.from(q.actor);let pointIndex=0;const a=originalActorDecision({...q,actors,terrain:new Uint8Array(2500).fill(q.code),tileFlags:new Uint16Array(2500).fill(q.flags),buildings:[Uint8Array.from(q.building)]},(e,s)=>{if(e.address===0x425b50)new DataView(s.actors[0].buffer).setInt16(0xaa,2,true);if(e.address===0x466ea0){s.tileFlags.fill(r.replacement);s.actors[0][0x8c]=6;}if([0x465c40,0x406450].includes(e.address))new DataView(s.actors[0].buffer).setUint32(0x18,0x200,true);return {state:s,point:[0x42f270,0x42f020].includes(e.address)?r.points[pointIndex++]:undefined,result:e.address===0x466ea0?r.response:0};});const context=a.next==='0x428ad1'?originalActorWalkingContext(a.state,a):null;const actual={walkingContext:context?Object.fromEntries(['entryValue','movementReady','walkingOverride','nextTerrain','reversalCheck','cachedFlags'].map(k=>[k,context[k]])):null,actor:Array.from(a.state.actors[0]),partner:Array.from(a.state.actors[1]),visualOwners:a.state.visualOwners,focusActor:a.state.focusActor,trackedX:a.state.trackedX,trackedZ:a.state.trackedZ,trackedFacing:a.state.trackedFacing,visualSlot:a.visualSlot,seed:a.state.seed,flags:a.state.tileFlags[0],calls:a.calls,randomDraws:a.randomDraws,ballTile:a.ballTile,ballTerrain:a.ballTerrain,actorIndex:a.actorIndex,actorTerrain:a.actorTerrain,next:a.next,partnerNotReady:a.partnerNotReady??null,closerToCup:a.closerToCup??null};if(!isDeepStrictEqual(actual,r.expected))throw Error(JSON.stringify({r,actual}));}console.log('1500 continuous native actor decision cases match.');""".replace('MODULE',json.dumps(module)).replace('CONTEXT',json.dumps((root/'simgolf-reborn/scene/src/simulation/original-actor-walking-context.js').as_uri()))
