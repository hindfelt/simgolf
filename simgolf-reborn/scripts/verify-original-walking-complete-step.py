...
def hook(u,a,size,data):
 ...
 if a in [0x40c1f0,0x4672d0]:
  sp=u.reg_read(UC_X86_REG_ESP);args=[get(sp+4*j) for j in range(1,5 if a==0x40c1f0 else 4)];calls.append(dict(address=a,args=args))
...
for i in range(1400):
 ...
 q.update(difficulty=i%3,fastWalking=i%2,destination=dict(x=30000,z=31000),walkingOverride=i%2);b[0x29]=1;put(0x102034,q['walkingOverride']);u.reg_write(UC_X86_REG_EBX,30000);u.reg_write(UC_X86_REG_ESI,31000);put(0x820344,q['difficulty']);put(0x599a9c,q['fastWalking']);u.mem_write(0x570d38,bytes([10]*2500));u.mem_write(0x577f00,bytes(b));u.mem_write(0x53ba00,struct.pack('<2500H',*marks))
...
script="""import {readFileSync} from 'node:fs';import {isDeepStrictEqual} from 'node:util';const {originalWalkingCompleteStep}=await import(MODULE);const rows=JSON.parse(readFileSync(0,'utf8'));let sounds=0;for(const [q,b,t,metadata,e] of rows){const r=originalWalkingCompleteStep({...q,actors:[new Uint8Array(b.slice(0,256)),new Uint8Array(b.slice(256))],tileFlags:new Uint16Array(t),terrain:new Uint8Array(2500).fill(10),metadata},(_,state)=>({state}));const a={actors:r.state.actors.flatMap(b=>Array.from(b)),calls:r.calls,next:r.next};if(r.walkingRate!==undefined)a.walkingRate=r.walkingRate;if(!isDeepStrictEqual(a,e))throw Error(JSON.stringify({q,fields:Object.keys(e).filter(k=>!isDeepStrictEqual(a[k],e[k]))}));sounds+=r.calls.length;}console.log(`${rows.length} continuous native complete walking steps match; ${sounds} sound/reaction effects.`);""".replace('MODULE',json.dumps((root/'simgolf-reborn/scene/src/simulation/original-walking-complete-step.js').as_uri()))
