"""Verify chained original exact planning and flight on the same flat terrain map.
...
const {originalShotMap}=await import(MAP);const {rows}=JSON.parse(readFileSync(0,'utf8'));let cache=originalStrengthCache();
...
console.log(`${rows.length} shared-flat-map original planners and candidate trajectories match.`);
""".replace('MAP',json.dumps((root/'simgolf-reborn/scene/src/simulation/original-shot-map.js').as_uri())).replace('MODULE',json.dumps(module)).replace('STEP',json.dumps(step)).replace('CACHE',json.dumps(cache_module))
subprocess.run(['node','--input-type=module','-e',script],input=json.dumps(dict(rows=rows)),text=True,check=True)
if '--write-fixture' in sys.argv:
 (root/'simgolf-reborn/scene/tests/fixtures/original-shared-map-candidate.json').write_text(json.dumps(dict(rows=rows),separators=(',',':'))+'\n')
