...
for a in [0x40dc70,0x4672d0,0x466ea0,0x406dd0,0x465c40,0x406450,0x426b00,0x40c1f0,0x409820,0x409780,0x42f270,0x42f020,0x425b50,0x4235c0]:u.mem_write(a,b'\xc3')
...
def hook(u,a,size,data):
 ...
 if a in [0x428ad1,0x42b647,0x42d23c,0x42bdb5,0x4295ef]:
  next_branch={0x428ad1:'0x428ad1',0x42b647:'0x42b647',0x42d23c:'0x42d23c',0x42bdb5:'motion',0x4295ef:'skip'}[a];u.emu_stop()
 if a==0x4235c0:
...
for i in range(1500):
 ...
 q=dict(swingOverride=0,roundClock=0,holeTees=[dict(x=2,z=2)]*3,updateScratch=0,globalFlags=global_flags,holeTargets=[dict(x=25,z=35)]*3,actorId=slot,actor=list(actor),partner=list(partner),visualOwners=owners,difficulty=-1,focusActor=-1,lastPairClock=0,modeByte=0,detailLevel=4,environmentByte=0,conditionRange=20,metadata=[{}, {'shape':1}],trackedX=-1,trackedZ=-1,trackedFacing=0,phaseCounter=phase,seed=seed,code=code,flags=flags,building=list(b))
