"""Verify combined original club selection and resolved-target launch path through final state."""
...
for a,n in [(0x423f48,0x1bcb),(0x466a00,0x50),(0x421870,0x167),(0x405710,0x4e),(0x45ba70,0x60),(0x4a57a0,0x27),(0x4b9800,8)]:
...
def run(q):
 ...
 u.emu_start(0x423f48,0x425ab9,count=100000)
 ...
 return dict(speed=read(0x577fec),verticalSpeed=read(0x577ff0),heading=read(0x577fe8)&0xffffffff,angularOffset=read(0x577ff4),actorFlags=read(0x577f18)&0xffffffff,seed=read(0x820454)&0xffffffff,lie=read(sp+0x14),shotType=struct.unpack('<h',u.mem_read(0x577fb4,2))[0],club=u.mem_read(0x577f24,1)[0],cache=dict(next=read(0x5a8728),entries=entries))
...
for i in range(1000):
 ...
 rows.append([q,run(q)])
module=(root/'simgolf-reborn/scene/src/simulation/original-selected-launch.js').as_uri()
script="""import {readFileSync} from 'node:fs';import {isDeepStrictEqual} from 'node:util';const {originalSelectedLaunch}=await import(MODULE);
const {originalStrengthCache}=await import(CACHE);let cache=originalStrengthCache();
const rows=JSON.parse(readFileSync(0,'utf8'));for(const [q,e] of rows){const a=originalSelectedLaunch(q,cache,{kindAt:p=>(p.x+p.z)%2?0:13,shotClassAt:lie=>q.classes[lie+1]});if(!isDeepStrictEqual(a,e))throw Error(JSON.stringify({q,a,e}));cache=a.cache;}
...
if '--write-fixture' in sys.argv:
 (root/'simgolf-reborn/scene/tests/fixtures/original-selected-launch.json').write_text(json.dumps(rows[:60],separators=(',',':'))+'\n')
