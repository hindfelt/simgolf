"""Native cup scoring through pre-snap motion-to-actor handoff."""
...
counts={0x405e80:2,0x40c1f0:4,0x466fb0:2,0x40c7f0:3,0x40c580:4,0x4672d0:3,0x45b2c0:1,0x45b180:1,0x4093b0:1}
...
u.hook_add(UC_HOOK_CODE,hook);rng=random.Random(9912);rows=[]
for i in range(1200):
 ...
 put(0x4c1848,0);put(0x102010,0);put(0x102018,q['ballTile']['x']);put(0x102030,q['visualSlot']);calls=[];branch=None;u.reg_write(UC_X86_REG_ESP,0x102000);u.reg_write(UC_X86_REG_EBP,0);u.reg_write(UC_X86_REG_EDI,q['ballTile']['z']);u.emu_start(0x42c3f4,0x400fff,count=50000)
...
module=(root/'simgolf-reborn/scene/src/simulation/original-motion-cup-handoff.js').as_uri()
script="""import {readFileSync} from 'node:fs';import {createHash} from 'node:crypto';import {isDeepStrictEqual} from 'node:util';const {originalMotionCupHandoff}=await import(MODULE);const hash=rows=>createHash('sha256').update(Buffer.concat(rows.map(b=>Buffer.from(b)))).digest('hex');for(const [i,r] of JSON.parse(readFileSync(0,'utf8')).entries()){const q=r.q;const raw=Uint8Array.from(q.actor),v=new DataView(raw.buffer),ball={};for(const [name,offset] of [['x',0xdc],['z',0xe0],['height',0xe4],['speed',0xec],['verticalSpeed',0xf0],['angularOffset',0xf4]])ball[name]=v.getInt32(offset,true);ball.heading=v.getUint32(0xe8,true);const motion={captured:true,cupEntry:{ball,ballTile:q.ballTile},rngState:17};for(const offset of [0xdc,0xe0,0xe4,0xe8,0xec,0xf0,0xf4])v.setInt32(offset,999,true);const a=originalMotionCupHandoff({...q,actorId:0,actors:[raw],scoreList:Int32Array.from(q.scoreList),holeRecords:Array.from({length:21},(_,i)=>{const b=i===20?new Uint8Array(520):Uint8Array.from(q.hole);if(i===19)b[0]=0;return b;}),statRecords:Array.from({length:32},()=>Uint8Array.from(q.stat)),completionRecords:[Uint8Array.from(q.record)],financialPeriods:Array.from({length:2},(_,i)=>Uint8Array.from(q.periods.slice(i*20,i*20+20))),completionNotices:Array.from({length:64},(_,i)=>Uint8Array.from(q.notices.slice(i*76,i*76+76)))},motion,(e,state)=>{if(e.address===0x466fb0)state.sourceText='Gary';if(r.mutate){if(e.address===0x405e80){state.actors[0][0x21]=2;state.actors[0][0x29]=2;state.actors[0][0xc2]=1;}if(e.address===0x40c1f0)state.settlementValue=(state.settlementValue+1)|0;if(e.address===0x40c7f0){state.settlementValue=-9;state.periodIndex=1;}if(e.address===0x4672d0){state.actors[0][0x29]=3;state.actors[0][0x2a]=6;state.phaseCounter=120;}}return {state};});const s=a.state,actual={actor:Array.from(s.actors[0]),record:Array.from(s.completionRecords[0]),holes:hash(s.holeRecords),stats:hash(s.statRecords),periods:hash(s.financialPeriods),notices:hash(s.completionNotices),value:s.settlementValue,cashTotal:s.cashTotal,periodIndex:s.periodIndex,phaseCounter:s.phaseCounter,recordHolder:s.recordHolder,presentationMode:s.presentationMode,sourceText:s.sourceText,calls:a.calls,next:a.next,selectionState:s.selectionState,scoreList:Array.from(s.scoreList)};if(!isDeepStrictEqual(actual,r.expected))throw Error(JSON.stringify({i,r,actual}));}console.log('1200 native cup motion handoffs match after replacing stale actor ball fields.');""".replace('MODULE',json.dumps(module))
