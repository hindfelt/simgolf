"""Verify repeated original route passes, pruning, diagnostics, and ordered callbacks."""
...
for a,n in [(0x422799,0xd52),(0x466ba0,0x10e),(0x40bc50,0x33),(0x40a9f0,0x81),(0x40c1a0,0x42),(0x4a57a0,0x27),(0x4c1870,64)]:
 o=p.get_offset_from_rva(a-0x400000);u.mem_write(a,p.__data__[o:o+n])
u.mem_write(0x483330,b'\xc3')
def write(a,v,size=4):u.mem_write(a,(v&((1<<(size*8))-1)).to_bytes(size,'little'))
def read(a):return struct.unpack('<i',u.mem_read(a,4))[0]
remaining=0;lie=0;calls=[];current={};sampleIndex=0;passes=[]
def hook(u,a,s,d):
 global remaining,lie,calls,sampleIndex
...
def run(q):
 ...
 write(0x5a5b88,q['diagnostics'])
 u.reg_write(UC_X86_REG_ESP,0x102000);u.reg_write(UC_X86_REG_FPCW,0x37f)
 u.emu_start(0x422799,0x4234eb,count=5000000)
 assert u.reg_read(UC_X86_REG_EIP)==0x4234eb
 return dict(scores=[list(struct.unpack('<6i',u.mem_read(0x1020bc+i*24,24))) for i in range(441)],distances=[list(struct.unpack('<6i',u.mem_read(0x104a14+i*24,24))) for i in range(441)],flags=[list(struct.unpack('<6I',u.mem_read(0x10736c+i*24,24))) for i in range(441)],work=read(0x102044),winner=dict(score=read(0x102060),target=dict(x=read(0x102078),z=read(0x1020a4)),curve=read(0x10206c),cornerTarget=read(0x5a8730),landing=dict(x=read(0x5a7270),z=read(0x5a7278)),landingFlag=read(0x1020ac)),searchFlag=read(0x5a872c),followupFlag=read(0x1020b4),samples=read(0x102038),diagnostics=read(0x5a5b88),passes=passes)
...
for _ in range(30):
 ...
 q['diagnostics']=rng.randrange(8)
 rows.append([q,run(q),calls])
module=(root/'simgolf-reborn/scene/src/simulation/original-route-search-passes.js').as_uri()
script='''import {readFileSync} from 'node:fs';const {originalRouteSearchPasses}=await import(MODULE);
...
for(const [q,e,expectedCalls] of rows){const cells=new Map(q.cells.map(([x,z,code,shotClass,flags])=>[`${x},${z}`,{code,shotClass,flags}]));
 let index=0;const calls=[];const a=originalRouteSearchPasses({...q,assessShot:c=>{calls.push({type:"assess",...c});return q.costs[c.shape+1]+c.flag*3;},terrainAt:p=>cells.get(`${p.x},${p.z}`)},c=>{calls.push({type:"simulate",...c});return {landing:q.landings[(index++)%q.landings.length]};});
 if(JSON.stringify(a)!==JSON.stringify(e)||JSON.stringify(calls)!==JSON.stringify(expectedCalls))throw Error(JSON.stringify({a,e}));
}console.log(`${rows.length} multi-pass route searches and ordered calls match original x86.`);
...
if '--write-fixture' in __import__('sys').argv:
 fixtures=[];seen=set()
 for row in rows:
  key=(tuple(row[1]['passes']),row[0]['mode']==2)
  if key not in seen:fixtures.append(row);seen.add(key)
 (root/'simgolf-reborn/scene/tests/fixtures/original-route-search-passes.json').write_text(json.dumps(fixtures,separators=(',',':'))+'\n')
