...
const {originalAutoShotReactions}=await import(MODULE);const {originalPlannerWorldRecords}=await import(READER);
for(const [q,e] of JSON.parse(readFileSync(0,'utf8'))){const world={actors:Array.from({length:152},()=>new Uint8Array(256)),completionRecords:Array.from({length:4},()=>new Uint8Array(44)),profileRecords:q.profileBytes.map(value=>{const b=new Uint8Array(560);b[33]=value;return b;})};new DataView(world.actors[q.actorId].buffer).setInt16(0xbe,q.state.actor.profileIndex,true);new DataView(world.actors[q.actorId^1].buffer).setInt16(0xbe,q.otherProfileIndex,true);world.completionRecords[q.state.actor.profileIndex][22+q.state.actor.hole]=q.profileHoleMark;const records=originalPlannerWorldRecords(world);const a=originalAutoShotReactions(q,{
 shotClassAt:()=>q.shotClass,profileHoleMarkAt:records.profileHoleMarkAt,profileIndexFor:records.profileIndexFor,profileByteAt:records.profileByteAt,
