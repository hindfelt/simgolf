"""Continuous native golfer scheduler through all 152 actor slots.
...
for a in [0x40dc70,0x4672d0,0x466ea0,0x406dd0,0x465c40,0x406450,0x426b00,0x42f270,0x42f020,0x425b50,0x4235c0,0x4096e0,0x40c1f0,0x409820,0x409780]:u.mem_write(a,b'\xc3')
...
def hook(u,a,size,data):
 ...
 if a in [0x4235c0,0x4096e0,0x40c1f0,0x409820,0x409780]:
 ...
 if a in [0x42b647,0x42960a]:
  next_branch={0x428ad1:'0x428ad1',0x42b55c:'0x42b55c',0x42b647:'0x42b647',0x42d23c:'0x42d23c',0x42bdb5:'motion',0x42960a:'complete'}[a];u.emu_stop()
...
for i in range(300):
 ...
 phase=rng.choice([1,193,2,257]);seed=rng.randrange(2**32)
 ...
 global_flags=rng.choice([0,32]);points=[dict(x=100+j*20,y=150+j*30,visible=rng.choice([True,True,False])) for j in range(100)]
 ...
 extras=[]
 for j in range(3,152):
  extra=bytearray(256);extra[0x29]=rng.choice([0,0,255]);extra[0x8c]=rng.choice([0,0,1,2,255]);struct.pack_into('<ii',extra,8,j*256,j*512);extras.append(list(extra))
 q['extras']=extras
 q.update(effectFlagMutation=0,queueClock=100,selectionState=-1,clubhouseTile=dict(x=20,z=25),avoidanceCursor=0,fastWalking=0,cartUpgrade=0,type6Available=0,type8Available=0,type10Available=0,serviceTileX=-1,serviceTileZ=0,nearestFacilityDistance=65535);q['metadata'] += [dict(shotClass=struct.unpack('<b',u.mem_read(0x576dc2+j*48,1))[0],walkingCost=struct.unpack('<b',u.mem_read(0x576dc5+j*48,1))[0]) for j in range(2,23)];q['metadata'][1].update(flags=0,kind=0,walkingCost=1,shotClass=0,bounceCoefficient=4,rollCoefficient=3,scatterCoefficient=0)
 ...
 u.mem_write(base,bytes(actor));u.mem_write(base+256,bytes(partner));u.mem_write(base+512,bytes(actor));u.mem_write(base+768,b''.join(bytes(b) for b in extras));put(0x59d208,global_flags)
 ...
 u.mem_write(0x51a680,bytes([3,4,3,8,3,6,3,2])*2500);put(0x576dcc+48,0);u.mem_write(0x576dc0+48,bytes([4,3,0]));put(0x59a188,0)
 for j in range(32):put(0x5698cc+j*184,0)
 trace=[];moving=False;calls=[];draws=0;sample=None;next_branch=None;point_index=0;turn_seen=False;put(0x102050,0);u.reg_write(UC_X86_REG_FPCW,0x37f);u.reg_write(UC_X86_REG_ESP,0x102080);u.reg_write(UC_X86_REG_EBP,slot*256);u.reg_write(UC_X86_REG_EBX,slot);
 for tick in range(3):
  put(0x831828,(phase+tick)&0xffffffff);u.reg_write(UC_X86_REG_ESP,0x102080);u.emu_start(0x428100,0x400fff,count=10000000)
...
module=(root/'simgolf-reborn/scene/src/simulation/original-golfer-terrain-loop.js').as_uri()
script="""import {readFileSync} from 'node:fs';import {isDeepStrictEqual} from 'node:util';const {originalGolferTerrainLoop}=await import(MODULE);for(const r of JSON.parse(readFileSync(0,'utf8'))){const q=r.q,actors=Array.from({length:152},()=>new Uint8Array(256));actors[q.actorId]=Uint8Array.from(q.actor);actors[1]=Uint8Array.from(q.partner);actors[2]=Uint8Array.from(q.actor);q.extras.forEach((b,i)=>actors[i+3]=Uint8Array.from(b));const holes=Array.from({length:20},()=>{const b=new Uint8Array(520),v=new DataView(b.buffer);b[0]=4;for(const [o,n] of [[8,20],[12,25],[16,20],[20,25]])v.setInt32(o,n,true);return b;});for(let i=0;i<3;i++){const v=new DataView(holes[i].buffer);v.setInt32(24,25,true);v.setInt32(28,35,true);}let pointIndex=0;const resolve=(e,s)=>{if(e.address===0x4235c0)new DataView(s.actors[0].buffer).setUint32(0xe8,0x40000000,true);if(e.address===0x40c140&&q.effectFlagMutation){const key=q.effectFlagMutation===1?'globalFlags':'worldFlags';s[key]^=32;}if(e.address===0x425b50)new DataView(s.actors[0].buffer).setInt16(0xaa,2,true);if(e.address===0x466ea0){s.tileFlags.fill(r.replacement);s.actors[0][0x8c]=6;}if([0x465c40,0x406450].includes(e.address))new DataView(s.actors[0].buffer).setUint32(0x18,0x200,true);return {state:s,value:0,point:[0x42f270,0x42f020].includes(e.address)?r.points[pointIndex++]:undefined,result:e.address===0x466ea0?r.response:0};};let a={state:{...q,actors,holes,heights:new Uint8Array(2601).fill(3),derived:{edgeMasks:new Uint8Array(2500),surfaceHeights:new Int8Array(2500).fill(3),directionHeights:Int8Array.from({length:20000},(_,i)=>[3,4,3,8,3,6,3,2][i%8])},edgeMasks:new Uint8Array(2500),shotStatCounts:new Uint32Array(32),holeStrokeTotals:new Uint16Array(20),visitorAssignments:new Uint8Array(800),facilityRecords:new Uint8Array(4096),facilityWidths:Array(16).fill(0),traversalCosts:new Int8Array(2500).fill(2),metadataClass:new Uint8Array(128),terrain:new Uint8Array(2500).fill(q.code),tileFlags:new Uint16Array(2500).fill(q.flags),buildings:[Uint8Array.from(q.building)]},calls:[],randomDraws:0};for(let tick=0;tick<3;tick++){a.state.phaseCounter=(q.phaseCounter+tick)>>>0;const step=originalGolferTerrainLoop(a.state,resolve);a={...step,calls:[...a.calls,...step.calls],randomDraws:a.randomDraws+step.randomDraws};};const actual={counts:Array.from(a.state.shotStatCounts),totals:Array.from(a.state.holeStrokeTotals),globalFlags:a.state.globalFlags,actors:a.state.actors.flatMap(b=>Array.from(b)),holes:a.state.holes.flatMap(b=>Array.from(b)),visitorAssignments:Array.from(a.state.visitorAssignments),avoidanceCursor:a.state.avoidanceCursor,selectionState:a.state.selectionState,actor:Array.from(a.state.actors[0]),partner:Array.from(a.state.actors[1]),visualOwners:a.state.visualOwners,focusActor:a.state.focusActor,trackedX:a.state.trackedX,trackedZ:a.state.trackedZ,trackedFacing:a.state.trackedFacing,visualSlot:a.visualSlot,seed:a.state.seed,flags:a.state.tileFlags[0],calls:a.calls,randomDraws:a.randomDraws,ballTile:a.ballTile,ballTerrain:a.ballTerrain,actorIndex:a.actorIndex,actorTerrain:a.actorTerrain,next:a.next,partnerNotReady:a.partnerNotReady??null,closerToCup:a.closerToCup??null};for(const key of ['ballTile','ballTerrain','actorIndex','actorTerrain','partnerNotReady','closerToCup','visualSlot'])delete actual[key];if(!isDeepStrictEqual(actual,r.expected)){console.error(JSON.stringify({i:q.phaseCounter,next:actual.next,fields:Object.keys(r.expected).filter(k=>!isDeepStrictEqual(actual[k],r.expected[k])),calls:actual.calls,expectedCalls:r.expected.calls,expectedNext:r.expected.next,actorDiff:actual.actor.map((v,i)=>v===r.expected.actor[i]?null:[i,v,r.expected.actor[i]]).filter(Boolean),trace:r.trace}));process.exit(1)};}console.log('300 three-tick native golfer sequences match shared state, actors, holes, RNG and calls.');""".replace('MODULE',json.dumps(module))
