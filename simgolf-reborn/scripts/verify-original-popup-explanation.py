"""Continuous original explanation flow with personal phrases and actual actor naming and popup/RNG; controlled location/search boundaries."""
...
for a,n in [(0x466fb0,0x320),(0x40c7f0,0xf7),(0x45ba70,0x60),(0x4b9800,8),(0x4a57a0,0x27),(0x46806a,0x102b),(0x469160,0xe9),(0x469330,0x3008),(0x46c104,58),(0x45b090,0xbd),(0x46c140,0x2c),(0x4c0000,0x30000)]:u.mem_write(a,p.get_data(a-0x400000,n))
for a in [0x4074d0,0x4a5c60]:u.mem_write(a,b'\xc3')
...
for kind in range(1,66):
 for i in range(48):
  ...
  rows.append([q,dict(state=s,events=events,phraseEvents=phraseEvents,pronoun=None if kind==50 else read(sp+0x10),popupRandomDraws=0 if s['seed']==q['state']['seed'] else 2)])
module=(root/'simgolf-reborn/scene/src/simulation/original-complete-explanation.js').as_uri()
script="""import {readFileSync} from 'node:fs';import {isDeepStrictEqual} from 'node:util';import {originalPopupCompleteExplanation} from MODULE;import {originalActorName} from NAMES;const rows=JSON.parse(readFileSync(0,'utf8'));for(const [q,expected] of rows){for(const s of [q.state,expected.state])for(const id in s.actors)s.actors[id]=Uint8Array.from(s.actors[id]);for(const id in q.profileRecords)q.profileRecords[id]=Uint8Array.from(q.profileRecords[id]);const got=originalPopupCompleteExplanation(q,(e,s)=>{if(e.address===0x466fb0){s.sourceText=originalActorName({...q.names,actorId:e.args[0],actor:s.actors[e.args[0]],sourceText:s.sourceText,appendComma:!!e.args[1]});return s;}return {...s,remarkStyle:91,sourceText:s.sourceText.split('\\0',1)[0]+'Place'+e.args[0]+','+e.args[1]};});if(!isDeepStrictEqual(got,expected))throw Error(JSON.stringify({q,expected,got}));}console.log(`${rows.length} native complete popup-explanation cases matched`);""".replace('MODULE',json.dumps(module)).replace('NAMES',json.dumps((root/'simgolf-reborn/scene/src/simulation/original-actor-name.js').as_uri()))
subprocess.run(['node','--input-type=module','-e',script],input=json.dumps(rows),text=True,check=True)
(root/'simgolf-reborn/scene/tests/fixtures/original-popup-explanation.json').write_text(json.dumps(rows,separators=(',',':'))+'\n')
