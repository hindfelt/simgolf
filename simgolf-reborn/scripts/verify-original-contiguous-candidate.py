...
for a,n in [(0x4235c0,0x2572),(0x421b50,0x8f8),(0x406e80,0x1d0),(0x40a9f0,0x81),(0x4219e0,0x168),(0x466ba0,0x10e),(0x40bc50,0x33),(0x4c1870,64),(0x466a00,0x50),(0x421870,0x167),(0x405710,0x4e),(0x45ba70,0x60),(0x4a57a0,0x27),(0x4b9800,8)]:
...
def run(q):
 global current,ray_draws,steps,published
 current=q.copy();ray_draws=0;steps=0;published=False
 sp=0x102000
 ...
 write(0x4c1e0c,q['skillMask']);write(0x578001,5,1)
 u.mem_write(0x5608b0,bytes(2500))
 for code in set(q['terrain']):write(0x576dc0+code*48,3,1);write(0x576dc1+code*48,0,1)
 for address in range(0x59e6b0,0x5a1f30,0x388):write(address,65535,2)
 original=bytes(u.mem_read(0x577f08,256))
 u.mem_write(sp,struct.pack('<5I',0x400fff,0,q['plannerArgument'],q['targetZ'],q['mode']&0xffffffff))
 u.reg_write(UC_X86_REG_ESP,sp)
 try:u.emu_start(0x421b50,0x400fff,count=2000000)
 except Exception:
  print('failure',hex(u.reg_read(UC_X86_REG_EIP)),q['mode']);raise
 assert u.reg_read(UC_X86_REG_EIP)==0x400fff
