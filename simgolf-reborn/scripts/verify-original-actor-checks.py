"""Continuous native actor prelude, paired checks, scenery and tracking.
...
for a in [0x40dc70,0x4672d0,0x466ea0,0x406dd0,0x465c40]:u.mem_write(a,b'\xc3')
...
def hook(u,a,size,data):
 ...
 if a in [0x40dc70,0x4672d0,0x466ea0,0x406dd0,0x465c40]:
  sp=u.reg_read(UC_X86_REG_ESP);n={0x40dc70:2,0x4672d0:3,0x466ea0:1,0x406dd0:3,0x465c40:2}[a];calls.append(dict(address=a,args=[read(sp+4+j*4) for j in range(n)]))
  if a in [0x40dc70,0x406dd0]:u.reg_write(UC_X86_REG_EAX,0)
  if a==0x465c40:put(base+0x18,0x200)
  if a==0x466ea0:
...
for i in range(1000):
 ...
 struct.pack_into('<iiiiI',actor,8,20480,25600,200,200,rng.choice([0,0x200,0x100000,0x40000]));struct.pack_into('<h',actor,0xaa,1);struct.pack_into('<h',actor,0xba,4);actor[0x25]=7;actor[0x29]=1;actor[0x8c]=rng.choice([0,2,7]);actor[0x8d]=50
 ...
 q=dict(actorId=slot,actor=list(actor),partner=list(partner),visualOwners=owners,difficulty=-1,focusActor=-1,lastPairClock=0,modeByte=0,detailLevel=4,trackedX=-1,trackedZ=-1,trackedFacing=0,phaseCounter=phase,seed=seed,code=code,flags=flags,building=list(b))
 ...
 u.mem_write(0x568148,b'\x00');u.mem_write(0x5842b6,b'\x00');put(0x820454,seed);put(0x831828,phase);put(0x102010,slot)
 u.mem_write(0x570d38,bytes([code])*2500);u.mem_write(0x53ba00,struct.pack('<H',flags)*2500);u.mem_write(0x58a708,bytes(b))
 calls=[];draws=0;sample=None;u.reg_write(UC_X86_REG_ESP,0x102000);u.reg_write(UC_X86_REG_EBP,slot*256);u.reg_write(UC_X86_REG_EBX,slot);u.emu_start(0x42819c,0x4285bb,count=20000)
...
module=(root/'simgolf-reborn/scene/src/simulation/original-actor-checks.js').as_uri()
script="""import {readFileSync} from 'node:fs';import {isDeepStrictEqual} from 'node:util';const {originalActorChecks}=await import(MODULE);for(const r of JSON.parse(readFileSync(0,'utf8'))){const q=r.q,actors=[];actors[q.actorId]=Uint8Array.from(q.actor);actors[1]=Uint8Array.from(q.partner);const a=originalActorChecks({...q,actors,terrain:new Uint8Array(2500).fill(q.code),tileFlags:new Uint16Array(2500).fill(q.flags),buildings:[Uint8Array.from(q.building)]},(e,s)=>{if(e.address===0x466ea0){s.tileFlags.fill(r.replacement);s.actors[0][0x8c]=6;}if(e.address===0x465c40)new DataView(s.actors[0].buffer).setUint32(0x18,0x200,true);return {state:s,result:e.address===0x466ea0?r.response:0};});const actual={actor:Array.from(a.state.actors[0]),partner:Array.from(a.state.actors[1]),visualOwners:a.state.visualOwners,focusActor:a.state.focusActor,trackedX:a.state.trackedX,trackedZ:a.state.trackedZ,trackedFacing:a.state.trackedFacing,visualSlot:a.visualSlot,seed:a.state.seed,flags:a.state.tileFlags[0],calls:a.calls,randomDraws:a.randomDraws,sample:a.sample};if(!isDeepStrictEqual(actual,r.expected))throw Error(JSON.stringify({r,actual}));}console.log('1000 continuous native actor check cases match.');""".replace('MODULE',json.dumps(module))
