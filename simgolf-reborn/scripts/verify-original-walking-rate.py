...
def hook(u,a,size,data):
 global end
 if a in [0x4295e6,0x42b17c]:end=hex(a);u.emu_stop()
 if a==0x40c1f0:
...
for i in range(1400):
 ...
 u.mem_write(0x577f00,bytes(b));u.mem_write(0x53ba00,struct.pack('<2500H',*marks))
 ...
 u.reg_write(UC_X86_REG_ESP,0x102000);u.reg_write(UC_X86_REG_EBP,0);end=None;calls=[];u.emu_start(0x42af66,0x400fff,count=10000);assert end
 e=dict(actors=list(u.mem_read(0x577f00,512)),calls=calls,next=end)
 if end=='0x42b17c':e['walkingRate']=u.reg_read(UC_X86_REG_EDI)
 rows.append([q,list(b),marks,metadata,e])
script="""import {readFileSync} from 'node:fs';import {isDeepStrictEqual} from 'node:util';const {originalWalkingRate}=await import(MODULE);const rows=JSON.parse(readFileSync(0,'utf8'));let sounds=0;for(const [q,b,t,metadata,e] of rows){const r=originalWalkingRate({...q,actors:[new Uint8Array(b.slice(0,256)),new Uint8Array(b.slice(256))],tileFlags:new Uint16Array(t),metadata},(_,state)=>({state}));const a={actors:r.state.actors.flatMap(b=>Array.from(b)),calls:r.calls,next:r.next};if(r.walkingRate!==undefined)a.walkingRate=r.walkingRate;if(!isDeepStrictEqual(a,e))throw Error(JSON.stringify({q,fields:Object.keys(e).filter(k=>!isDeepStrictEqual(a[k],e[k]))}));sounds+=r.calls.length;}console.log(`${rows.length} native walking rates match; ${sounds} activation sounds.`);""".replace('MODULE',json.dumps((root/'simgolf-reborn/scene/src/simulation/original-walking-rate.js').as_uri()))
