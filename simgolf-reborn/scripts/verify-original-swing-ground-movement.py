...
effects={0x40c140:3,0x42f110:2,0x4096e0:1,0x4672d0:3,0x40c1f0:4,0x409820:1,0x409780:1}
...
for i in range(500):
 ...
 for extra in range(5):
  ...
  assert moving and exit_address==0x4295ef
 rows.append(dict(q=q,expected=dict(seed=read(0x820454)&0xffffffff,actor=list(u.mem_read(0x577f00,256)),calls=calls,animationTicks=animation_ticks,swingTicks=ticks)))
script="""import {readFileSync} from 'node:fs';import {isDeepStrictEqual} from 'node:util';const {originalActorSwingAnimation}=await import(ANIMATION),{originalSwingProgress}=await import(SWING),{originalActorBallMotion}=await import(POSITION),{originalActorMotionContext}=await import(CONTEXT);for(const {q,expected} of JSON.parse(readFileSync(0,'utf8'))){let state={...q,actors:Array.from({length:152},(_,i)=>i===0?Uint8Array.from(q.actor):new Uint8Array(256)),seed:17,worldFlags:0,luck:0,variant:0,terrain:new Uint8Array(2500).fill(q.ballTerrain),tileFlags:new Uint16Array(2500),edgeMasks:new Uint8Array(2500),metadata:Array.from({length:23},()=>({bounceCoefficient:4,rollCoefficient:3,scatterCoefficient:0}))},frameIndex=0,animationTicks=0,swingTicks=0;const calls=[],resolve=(e,state)=>{calls.push(e);return {state,value:e.address===0x40bc90?q.ballTerrain:0};};while(state.actors[0][0x28]!==2){if(++animationTicks>10)throw Error('Animation did not complete');const r=originalActorSwingAnimation(state,{animationDirection:q.animationDirection,frameCounts:q.frameCounts,previousFrame:state.actors[0][0x26],frameIndex});state=r.state;frameIndex=r.frameIndex;}for(swingTicks=1;swingTicks<10;swingTicks++){const r=originalSwingProgress(state,resolve);state=r.state;if(r.next==='motion'){state=originalActorBallMotion(originalActorMotionContext(state),resolve).state;break;}}for(let extra=0;extra<5;extra++){state.phaseCounter=q.phaseCounter+extra+1;state=originalSwingProgress(state,resolve).state;state=originalActorBallMotion(originalActorMotionContext(state),resolve).state;}const actual={seed:state.seed,actor:Array.from(state.actors[0]),calls,animationTicks,swingTicks};if(!isDeepStrictEqual(actual,expected)){console.error(JSON.stringify({diff:actual.actor.flatMap((v,i)=>v===expected.actor[i]?[]:[[i,v,expected.actor[i]]]),calls,expectedCalls:expected.calls,animationTicks,expectedAnimation:expected.animationTicks,swingTicks,expectedSwing:expected.swingTicks}));process.exit(1);}}console.log('500 native swing sequences with six rolling updates match actor bytes, RNG and effects.');"""
