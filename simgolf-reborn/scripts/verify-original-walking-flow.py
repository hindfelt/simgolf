...
end=None;calls=[];arrived=None;arrivalBranch=False
def hook(u,a,size,data):
 global end,arrived,arrivalBranch
 if a==0x425b10:calls.append(dict(address=a,args=[id]));return
 if a in [0x466fb0,0x40c7f0,0x447a30,0x4acb95]:
  sp=u.reg_read(UC_X86_REG_ESP);value=0
  if a==0x4acb95:value=get(sp+8);u.mem_write(value,str(get(sp+4)).encode()+b'\0')
  else:
   count={0x466fb0:2,0x40c7f0:3,0x447a30:5}[a];args=[get(sp+4*j) for j in range(1,count+1)];calls.append(dict(address=a,args=args))
   if a==0x466fb0:u.mem_write(0x518f78,bytes(u.mem_read(0x518f78,512)).split(b'\0')[0]+('Golfer '+str(args[0])).encode()+b'\0')
  u.reg_write(UC_X86_REG_EAX,value);u.reg_write(UC_X86_REG_EIP,get(sp));u.reg_write(UC_X86_REG_ESP,sp+4);return
 ...
 if a in [0x4295ef,0x42bdb5,0x42d23c,0x42b825]:end='skip' if a==0x4295ef else hex(a);u.emu_stop()
...
for address in [0x425b10,0x466fb0,0x40c7f0,0x447a30,0x4acb95,0x42a019,0x429f4c,0x4295ef,0x42bdb5,0x42d23c,0x42b825,0x425b50,0x40daa0,0x40db60,0x46c140,0x4672d0,0x40c1f0,0x40c140,0x42def0,0x40c580,0x4071d0]:u.hook_add(UC_HOOK_CODE,hook,begin=address,end=address)
...
for i in range(500):
 ...
 if i%7==0:
  struct.pack_into('<ii',b,id*256+8,23*1024+512,23*1024+512);struct.pack_into('<i',b,id*256+0xdc,0);b[id*256+0x29]=1;b[id*256+0x2a]=0;struct.pack_into('<hh',b,id*256+0xae,0,0);struct.pack_into('<h',b,id*256+0xb2,200);h[520]=4;q['walkingOverride']=0;b[(id^1)*256+0x29]=0;marks[23*50+23]=512
  for j in range(256):struct.pack_into('<h',f,j*16,-1)
 if i%3==0:
  for j in range(12):
   o=j*256;b[o+0x29]=1;b[o+0x2a]=0;b[o+0x22]=2;b[o+0x25]=7;struct.pack_into('<h',b,o+0xaa,j^1);struct.pack_into('<h',b,o+0xc6,0);struct.pack_into('<ii',b,o+8,24200,24000);struct.pack_into('<ii',b,o+0xdc,24500,24000)
  struct.pack_into('<ii',b,id*256+8,24000,24000);struct.pack_into('<h',b,id*256+0xac,-1);struct.pack_into('<I',b,id*256+0x18,0);b[id*256+0x20]=0;q['walkingOverride']=0;h[520]=4;q['selectionState']=-1
 ax,az=struct.unpack_from('<ii',b,id*256+8);q.update(actorIndex=(ax>>10)*50+(az>>10),actorTile=dict(x=ax>>10,z=az>>10),nextTerrain=1,reversalCheck=1,avoidanceCursor=0,difficulty=1,fastWalking=0,cartUpgrade=0,cachedFlags=struct.unpack_from('<I',b,id*256+0x18)[0],phaseCounter=i);put(0x5a446c,0);put(0x820344,1);put(0x599a9c,0);put(0x831828,i);put(0x10202c,1);put(0x10203c,q['actorIndex']);put(0x102040,ax>>10);put(0x10204c,az>>10);put(0x102050,q['cachedFlags']);u.mem_write(0x576dc5+48,bytes([1]));u.mem_write(0x58a6e8,bytes(32));u.mem_write(0x577f00,bytes(b));struct.pack_into('<ii',h,0x576ba0-0x574500,10,10);u.mem_write(0x574500,bytes(h));u.mem_write(0x58a708,bytes(f));u.mem_write(0x570d38,t);u.mem_write(0x53ba00,struct.pack('<2500H',*marks))
 q.update(entryValue=0,clubhouseTile=dict(x=10,z=10),sourceText="",messageFlag=0,cashUnits=1000,ledgerPeriod=0,type6Level=1,type8Level=1,type10Level=1);put(0x570a24,1000);put(0x5a5784,0);put(0x582c64,0);put(0x5a7698,1);put(0x5a76a0,1);put(0x5a76a8,1);u.mem_write(0x577254,bytes(2500));u.mem_write(0x5682dc,bytes(2500));u.mem_write(0x583430,bytes(44));u.mem_write(0x567698,bytes(800));u.mem_write(0x518f78,b"\0");u.mem_write(0x568148,b"\0")
 ...
 u.reg_write(UC_X86_REG_ESP,0x102000);u.reg_write(UC_X86_REG_EBP,id*256);u.reg_write(UC_X86_REG_EAX,0);u.reg_write(UC_X86_REG_EBX,q['actorTile']['x']);u.reg_write(UC_X86_REG_FPCW,0x37f);end=None;calls=[];arrived=None;arrivalBranch=False;u.emu_start(0x428ad1,0x400fff,count=10000000);assert end
 e=dict(selectionState=get(0x5a4440),assignments=list(u.mem_read(0x567698,800)),holes=list(u.mem_read(0x574500,len(h))),records=list(u.mem_read(0x583430,44)),sourceText=bytes(u.mem_read(0x518f78,512)).split(b"\0")[0].decode(),messageFlag=u.mem_read(0x568148,1)[0],actors=list(u.mem_read(0x577f00,len(b))),seed=get(0x820454)&0xffffffff,calls=calls,next=end,cashUnits=get(0x570a24),serviceIncome=struct.unpack('<h',u.mem_read(0x582c64,2))[0])
 rows.append([q,list(b),list(h),list(f),list(t),marks,widths,metadata,e])
script="""import {readFileSync} from 'node:fs';import {isDeepStrictEqual} from 'node:util';const {originalWalkingFlow}=await import(MODULE);const rows=JSON.parse(readFileSync(0,'utf8'));const coverage={};for(const [q,b,h,f,t,m,w,metadata,e] of rows){const r=originalWalkingFlow({...q,actors:Array.from({length:152},(_,i)=>new Uint8Array(b.slice(i*256,(i+1)*256))),holes:Array.from({length:20},(_,i)=>new Uint8Array(h.slice(i*520,(i+1)*520))),facilityRecords:new Uint8Array(f),facilityPrefix:new Uint8Array(32),facilityWidths:w,terrain:new Uint8Array(t),tileFlags:new Uint16Array(m),metadata,traversalCosts:new Int8Array(2500).fill(2),metadataClass:new Uint8Array(128),serviceIncome:new Int16Array(1),tileServiceState:new Uint8Array(2500),restDecorations:new Uint8Array(2500),completionRecords:[new Uint8Array(44)],visitorAssignments:new Uint8Array(800)},(event,state)=>{if(event.address===0x466fb0)state.sourceText+='Golfer '+event.args[0];if(event.address===0x425b50){state.actors[event.args[0]][0x29]=0;state.queueClock=(state.queueClock+17)|0;}if(event.address===0x4672d0)state.actors[event.args[0]][0x78]=36;return {state,value:1};});const a={selectionState:r.state.selectionState,assignments:Array.from(r.state.visitorAssignments),holes:r.state.holes.flatMap(b=>Array.from(b)),records:Array.from(r.state.completionRecords[0]),sourceText:r.state.sourceText,messageFlag:r.state.messageFlag,actors:r.state.actors.flatMap(b=>Array.from(b)),seed:r.state.seed,calls:r.calls,next:r.next,cashUnits:r.state.cashUnits,serviceIncome:r.state.serviceIncome[0]};coverage[r.next]=(coverage[r.next]??0)+1;if(r.calls.some(c=>c.address===0x40c7f0))coverage.departures=(coverage.departures??0)+1;if(r.calls.some(c=>c.address===0x4071d0))coverage.restVisits=(coverage.restVisits??0)+1;if(r.calls.some(c=>c.address===0x40c580))coverage.paidServiceVisits=(coverage.paidServiceVisits??0)+1;if(!isDeepStrictEqual(b.slice(q.actorId*256+8,q.actorId*256+16),Array.from(r.state.actors[q.actorId].slice(8,16))))coverage.positionChanges=(coverage.positionChanges??0)+1;if(!isDeepStrictEqual(a,e))throw Error(JSON.stringify({q,fields:Object.keys(e).filter(k=>!isDeepStrictEqual(a[k],e[k])),actual:{...a,actors:undefined,calls:undefined},expected:{...e,actors:undefined,calls:undefined}}));}console.log(`${rows.length} continuous native walking dispatches match. ${JSON.stringify(coverage)}`);""".replace('MODULE',json.dumps((root/'simgolf-reborn/scene/src/simulation/original-walking-flow.js').as_uri()))
