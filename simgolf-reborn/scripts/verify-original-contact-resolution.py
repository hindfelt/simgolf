...
from unicorn.x86_const import UC_X86_REG_ESP,UC_X86_REG_EBP,UC_X86_REG_EBX,UC_X86_REG_EDI,UC_X86_REG_FPCW,UC_X86_REG_EAX
...
for a in [0x40c1f0,0x4672d0,0x4093b0,0x4219e0,0x40c140]:u.mem_write(a,b'\xc3')
...
def hook(u,a,size,data):
 ...
 if a in [0x40c1f0,0x4672d0,0x4093b0,0x4219e0,0x40c140]:
  sp=u.reg_read(UC_X86_REG_ESP);calls.append(dict(address=a,args=[struct.unpack('<i',u.mem_read(sp+4+j*4,4))[0] for j in range(4 if a==0x40c1f0 else 3 if a in [0x4672d0,0x40c140] else 1)]))
...
for i in range(800):
 ...
 for c,v in scatter.items():u.mem_write(0x576dc0+c*48,b'\x00');metadata[c]=dict(scatterCoefficient=v);u.mem_write(0x576dc2+c*48,bytes([v&255]))
 q=dict(bounceCoefficient=0,boundaryFlags=0,ballTerrain=code,scatterCoefficient=scatter[code],direction=0,luck=0,actorId=0,actors=[list(b)],landingTerrain=code,landingTile=dict(x=24,z=24),centreFlag=centre,holeTargets=[None,dict(x=45,z=45)],terrain=terrain,metadata=metadata,seed=17)
 u.mem_write(0x574500+520,bytes(520));u.mem_write(0x574500+520,b'\x04');u.mem_write(0x5698c0,bytes(184));u.mem_write(0x53d934,bytes(2500));u.mem_write(0x53ba00,bytes(5000));put(0x820344,0);put(0x102030,-1);put(0x102050,0);put(0x10201c,0)
 ...
 calls=[];samples=0;mutate=i%2==0;u.reg_write(UC_X86_REG_ESP,0x102000);u.reg_write(UC_X86_REG_EBP,0);u.reg_write(UC_X86_REG_EBX,24);u.reg_write(UC_X86_REG_EDI,code);u.reg_write(UC_X86_REG_EAX,0);u.emu_start(0x42c527,0x400fff,count=100000)
...
module=(root/'simgolf-reborn/scene/src/simulation/original-contact-resolution.js').as_uri()
script="""import {readFileSync} from 'node:fs';import {isDeepStrictEqual} from 'node:util';const {originalContactResolution}=await import(MODULE);let samples=0,penalties=0;for(const r of JSON.parse(readFileSync(0,'utf8'))){r.q.actors=r.q.actors.map(b=>Uint8Array.from(b));r.q.terrain=Uint8Array.from(r.q.terrain);const h=new Uint8Array(520),v=new DataView(h.buffer);h[0]=4;v.setInt32(0x18,45,true);v.setInt32(0x1c,45,true);Object.assign(r.q,{holeRecords:[null,h],statRecords:[new Uint8Array(184)],tileWear:new Uint8Array(2500),tileFlags:new Uint16Array(2500),difficulty:0,visualSlot:-1});const a=originalContactResolution(r.q,(e,state)=>{if(r.mutate&&e.address===0x4672d0){const v=new DataView(state.actors[0].buffer);v.setInt32(0xcc,v.getInt32(0xcc,true)+128,true);v.setInt32(0xe0,v.getInt32(0xe0,true)+64,true);}return {state,result:100,value:e.address===0x40bc90?state.terrain[(e.args[0]>>10)*50+(e.args[1]>>10)]:0};});const actual={hole:Array.from(a.state.holeRecords[1]),stat:Array.from(a.state.statRecords[0]),wear:a.state.tileWear[1224],actor:Array.from(a.state.actors[0]),calls:a.calls,candidates:a.candidates,seed:a.state.seed};if(!isDeepStrictEqual(actual,r.expected))throw Error(JSON.stringify({r,actual}));samples+=a.candidates;penalties+=a.penalty?1:0;}console.log(`800 continuous native contact-to-accounting paths match; ${samples} drop candidates and ${penalties} penalties.`);""".replace('MODULE',json.dumps(module))
