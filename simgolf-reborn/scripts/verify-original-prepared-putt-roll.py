...
motion_effects={0x42f110:2,0x4096e0:1,0x4672d0:3,0x40c1f0:4,0x409820:1,0x409780:1,0x40c140:3}
...
motion_calls=[];motion_done=False

def motion_hook(v,a,n,d):
 global motion_done
 if a in [0x42ca9d,0x42c3f4]:motion_done=True;v.emu_stop()
...
for q,e in rows:
 ...
 for c in q['cells']:v.mem_write(0x576dc0+c[2]*48,bytes([3,0,c[3]&255]))
 vp(0x102010,0);vp(0x102014,q['startTerrain']);vp(0x102018,vr(0x577fdc)>>10);vp(0x102020,vr(0x577fe0)>>10);vp(0x102030,-1)
 animation_ticks=0;motion_calls=[];motion_done=False
 ...
 for swing_ticks in range(1,100):
  vp(0x102018,vr(0x577fdc)>>10);vp(0x102020,vr(0x577fe0)>>10);vp(0x102028,0);vp(0x831828,swing_ticks)
 ...
 assert motion_done
 q['motionExpected']=dict(terminal=hex(v.reg_read(UC_X86_REG_EIP)),actors=list(v.mem_read(0x577f00,512)),seed=vr(0x820454)&0xffffffff,calls=motion_calls,animationTicks=animation_ticks,swingTicks=swing_ticks)
...
const {originalShotPreparation}=await import(PREPARATION);const {originalPlannedShotPreparation}=await import(EFFECT);const {originalPlannerInput}=await import(INPUT);const {originalPlannerActor}=await import(ACTOR);const {applyOriginalPlannerResult}=await import(RESULT);

const {originalActorMotionContext}=await import(CONTEXT),{originalMotionSample}=await import(SAMPLE),{originalActorGroundDecision}=await import(DECISION),{originalActorGroundReflection}=await import(REFLECTION),{originalPostContactMotion}=await import(CONTACT);
function advance(snapshot,resolve){const context=originalActorMotionContext(snapshot),p=originalActorPositionStep(context,resolve),state=p.state,a=new DataView(state.actors[0].buffer),tile=context.ballTile;const sample=originalMotionSample({x:a.getInt32(0xdc,true),z:a.getInt32(0xe0,true),heading:a.getUint32(0xe8,true),cellX:tile.x,cellZ:tile.z,terrainCode:context.ballTerrain},(x,z)=>state.terrain[x*50+z]);Object.assign(state,sample,{previousTerrainHeight:p.previousTerrainHeight,stepX:p.stepX,stepCosine:p.stepCosine,ballTile:tile,ballTerrain:context.ballTerrain});if(a.getInt32(0xe4,true)>1)throw Error('Unexpected airborne putt');const d=originalActorGroundDecision(state,resolve);if(d.captured)return {state:d.state,terminal:'0x42c3f4'};const r=originalActorGroundReflection(d.state,resolve),m=originalPostContactMotion(r.state,resolve);return {state:m.state,terminal:m.stopped?'0x42ca9d':null};}
...
function verifyMovement(snapshot,q){let state={...structuredClone(snapshot),ballTerrain:q.startTerrain,visualSlot:-1,worldFlags:snapshot.globalFlags,terrain:Uint8Array.from(q.cells.map(c=>c[2])),edgeMasks:new Uint8Array(2500),metadata:Array.from({length:23},(_,i)=>({bounceCoefficient:3,rollCoefficient:0,scatterCoefficient:q.cells.find(c=>c[2]===i)?.[3]||0})),luck:5,variant:0},frameIndex=0,animationTicks=0,swingTicks=0;state.actors=Array.from({length:152},(_,i)=>state.actors[i]||new Uint8Array(256));state=originalSwingClearance({...state,swingOverride:0,roundClock:0}).state;let terminal=null;const calls=[],resolve=(e,state)=>{calls.push(e);return {state,value:e.address===0x40bc90?state.terrain[(e.args[0]>>10)*50+(e.args[1]>>10)]:0};};while(state.actors[0][0x28]!==2){if(++animationTicks>10)throw Error('Animation timeout');const a=originalActorSwingAnimation(state,{animationDirection:0,frameCounts:Array(160).fill(8),previousFrame:state.actors[0][0x26],frameIndex});state=a.state;frameIndex=a.frameIndex;}for(swingTicks=1;swingTicks<100;swingTicks++){state.phaseCounter=swingTicks;const a=originalSwingProgress(state,resolve);state=a.state;if(a.next==='motion'){const m=advance(state,resolve);state=m.state;terminal=m.terminal;if(terminal)break;}}const actual={terminal,actors:state.actors.slice(0,2).flatMap(b=>Array.from(b)),seed:state.seed,calls,animationTicks,swingTicks};if(!isDeepStrictEqual(actual,q.motionExpected))throw Error(JSON.stringify({scenario:q.scenario,actual,expected:q.motionExpected}));}

for(const [q,e] of JSON.parse(readFileSync(0,'utf8'))){const snapshot={...q.initialSearchGlobals,shotStatCounts:new Uint32Array(32),holeStrokeTotals:Uint16Array.from({length:20},(_,i)=>q.initialHoles[i*520+0x162]|q.initialHoles[i*520+0x163]<<8),tileFlags:Uint16Array.from(q.cells.map(c=>c[4])),metadata:Array.from({length:23},()=>({shotClass:99,scatterCoefficient:99})),actorId:0,actors:[Uint8Array.from(q.initialRecords.slice(0,256)),Uint8Array.from(q.initialRecords.slice(256))],holes:Array.from({length:20},(_,i)=>Uint8Array.from(q.initialHoles.slice(i*520,(i+1)*520))),seed:q.automatic.state.seed,strengthCache:q.automatic.state.cache,diagnostics:q.automatic.state.diagnostics,landing:q.automatic.state.landing};const preparation=originalPlannedShotPreparation({...snapshot,updateScratch:0,ballTerrain:q.startTerrain,ballTile:{x:q.origin.x>>10,z:q.origin.z>>10}},q.automatic,{map:searchLaunchMap(q),physical:{luck:5},searchState:q.outerSearchState,score:0,scoreAt:(x,z)=>q.aimScores[x*50+z]},{emit:(_event,state)=>state});const {searched,...a}=preparation.planner;const actors=[Uint8Array.from(q.initialRecords.slice(0,256)),Uint8Array.from(q.initialRecords.slice(256))],input={actors,actorId:0,holes:Array.from({length:20},(_,i)=>Uint8Array.from(q.initialHoles.slice(i*520,(i+1)*520))),metadata:Array.from({length:23},()=>({shotClass:99,scatterCoefficient:99}))};if(!isDeepStrictEqual(originalPlannerActor(input),q.automatic.state.actor))throw Error('Planner actor input differs from native');const applied=preparation.state;verifyMovement(applied,q);if(!isDeepStrictEqual(Array.from(applied.shotStatCounts),q.puttCounts))throw Error('Putt counts differ');const searchGlobals=Object.fromEntries(Object.keys(q.searchGlobals).map(k=>[k,applied[k]]));if(!isDeepStrictEqual(searchGlobals,q.searchGlobals))throw Error(JSON.stringify({actual:searchGlobals,expected:q.searchGlobals}));const shared={seed:applied.seed,cache:applied.strengthCache,diagnostics:applied.diagnostics,landing:applied.landing,holes:applied.holes.flatMap(b=>Array.from(b)),overrides:[17,20].map(code=>({code,shotClass:applied.metadata[code].shotClass}))};const expectedShared={seed:e.state.seed,cache:e.state.cache,diagnostics:e.state.diagnostics,landing:e.state.landing,holes:q.finalHoles,overrides:e.shotClassOverrides};if(!isDeepStrictEqual(shared,expectedShared))throw Error('Planner shared state differs from native');const expected=Uint8Array.from(q.finalRecords);const diff=applied.actors.flatMap(b=>Array.from(b)).flatMap((v,i)=>v===expected[i]?[]:[[i,v,expected[i]]]);if(diff.length)throw Error(JSON.stringify({scenario:q.scenario,recordDiff:diff}));if(!isDeepStrictEqual(a,e)){
...
console.log('36 native planned putts match preparation, animation, impact and rolling termination.');
