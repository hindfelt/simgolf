"""Full ordinary native hole completion through round return."""
...
counts={0x405e80:2,0x40c1f0:4,0x466fb0:2,0x40c7f0:3,0x40c580:4,0x4672d0:3,0x45b2c0:1,0x45b180:1}
...
for i in range(1200):
 ...
 q=dict(actor=list(b),hole=list(hole),stat=list(stat),record=list(record),periods=list(periods),notices=list(notices),globalFlags=flags,settlementMode=mode,settlementBonus=bonus,cashTotal=200,periodIndex=0,phaseCounter=100,recordHolder=-1,presentationMode=0,sourceText='Before',courseHoleCount=19,selectionState=0,difficulty=1,performanceBonus=0,secondaryBalance=0,adjustmentSetting=0,scoreList=[0]*10)
