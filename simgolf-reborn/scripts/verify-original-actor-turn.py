...
for a in [0x40dc70,0x4672d0,0x466ea0,0x406dd0,0x465c40,0x406450,0x426b00,0x40c1f0,0x409820,0x409780,0x42f270,0x42f020,0x425b50,0x4235c0,0x42f110,0x4096e0]:u.mem_write(a,b'\xc3')
...
next_branch=None;point_index=0;points=[];turn_seen=False;moving=False;closer=None
def hook(u,a,size,data):
 global draws,sample,next_branch,point_index,turn_seen,moving,closer
 if a==0x42bdb5:closer=bool(read(0x102074))
 if a==0x42bdc3:moving=True
 if a in [0x42f110,0x4096e0]:
  sp=u.reg_read(UC_X86_REG_ESP);calls.append(dict(address=a,args=[read(sp+4+j*4) for j in range(2 if a==0x42f110 else 1)]));u.reg_write(UC_X86_REG_EAX,0)
 if a==0x428992:turn_seen=True
 if a in [0x428ad1,0x42b647,0x42d23c,0x4295ef]:
  next_branch={0x428ad1:'0x428ad1',0x42b647:'0x42b647',0x42d23c:'0x42d23c',0x42bdb5:'motion',0x4295ef:'0x4295ef' if moving else 'skip'}[a];u.emu_stop()
 if a==0x4235c0:
 ...
 if a in [0x40dc70,0x4672d0,0x466ea0,0x406dd0,0x465c40,0x406450,0x426b00,0x40c1f0,0x409820,0x409780]:
  sp=u.reg_read(UC_X86_REG_ESP);n={0x40dc70:2,0x4672d0:3,0x466ea0:1,0x406dd0:3,0x465c40:2,0x406450:1,0x426b00:1,0x40c1f0:4,0x409820:1,0x409780:1}[a];calls.append(dict(address=a,args=[read(sp+4+j*4) for j in range(n)]))
...
u.reg_write(UC_X86_REG_FPCW,0x37f);u.hook_add(UC_HOOK_CODE,hook);u.reg_write(UC_X86_REG_ESP,0x102000);u.emu_start(0x491380,0x4913bc,count=20000)
...
for i in range(1500):
 ...
 q=dict(swingOverride=0,roundClock=0,holeTees=[dict(x=2,z=2)]*3,updateScratch=0,globalFlags=global_flags,holeTargets=[dict(x=25,z=35)]*3,actorId=slot,actor=list(actor),partner=list(partner),visualOwners=owners,difficulty=-1,focusActor=-1,lastPairClock=0,modeByte=0,detailLevel=4,environmentByte=0,conditionRange=20,variant=0,luck=0,worldFlags=global_flags,metadata=[{}, {'shape':1,'bounceCoefficient':4,'rollCoefficient':3,'scatterCoefficient':0}],trackedX=-1,trackedZ=-1,trackedFacing=0,phaseCounter=phase,seed=seed,code=code,flags=flags,building=list(b))
 ...
 for j in range(3):put(0x574508+j*520,2);put(0x57450c+j*520,2);put(0x574518+j*520,25);put(0x57451c+j*520,35)
 for j,v in enumerate(owners):u.mem_write(0x59e6b0+j*0x388,struct.pack('<h',v))
 for a,v in [(0x599a9c,0),(0x568f6c,0),(0x59a188,0),(0x820344,-1),(0x4c1dfc,-1),(0x599a98,0),(0x4c183c,4),(0x5842a0,-1),(0x5842a4,-1)]:put(a,v)
 u.mem_write(0x576dc0+48,bytes([4,3,0]));u.mem_write(0x576dc7+48,b'\x01');put(0x5672a0,20);u.mem_write(0x5a1f30,b'\x00');u.mem_write(0x568148,b'\x00');u.mem_write(0x5842b6,b'\x00');put(0x820454,seed);put(0x831828,phase);put(0x102010,slot)
