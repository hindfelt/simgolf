"""Verify exact target setup through assessment and final launch with original instructions."""
...
for a,n in [(0x42365d,0x24b6),(0x466ba0,0x10e),(0x40bc50,0x33),(0x4c1870,64),(0x466a00,0x50),(0x421870,0x167),(0x405710,0x4e),(0x45ba70,0x60),(0x4a57a0,0x27),(0x4b9800,8)]:
...
def run(q):
 ...
 u.emu_start(0x42365d,0x425ab9,count=100000)
...
for i in range(1000):
 ...
 rows.append([q,run(q)])
module=(root/'simgolf-reborn/scene/src/simulation/original-resolved-shot.js').as_uri()
script="""import {readFileSync} from 'node:fs';import {isDeepStrictEqual} from 'node:util';const {originalResolvedShot}=await import(MODULE);
const {originalStrengthCache}=await import(CACHE);let cache=originalStrengthCache();
const rows=JSON.parse(readFileSync(0,'utf8'));for(const [q,e] of rows){const a=originalResolvedShot(q,cache,{terrainAt:(x,z)=>q.terrain[x*50+z],kindAt:c=>q.kinds[c],shotClassAt:lie=>q.classes[lie+1],marksAt:(x,z)=>q.marks[x*50+z],heightAt:(x,z)=>q.heights[x*50+z]});if(!isDeepStrictEqual(a,e))throw Error(JSON.stringify({q:{distance:q.distance,skillMask:q.skillMask},a,e}));cache=a.cache;}
...
if '--write-fixture' in sys.argv:
 (root/'simgolf-reborn/scene/tests/fixtures/original-resolved-shot.json').write_text(json.dumps(rows[:20],separators=(',',':'))+'\n')
