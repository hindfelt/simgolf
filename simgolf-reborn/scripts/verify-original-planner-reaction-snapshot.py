...
def remarks(u,a,size,data):
 ...
 if a==0x4672d0:
  ...
  current.setdefault('reactionSnapshots',[]).append(dict(actor={**{name:u.mem_read(addr,1)[0] for name,addr in byte_fields.items()},**{name:struct.unpack('<H',u.mem_read(addr,2))[0] for name,addr in word_fields.items()},'actorFlags':read(0x577f18)&0xffffffff,'angularOffset':read(0x577ff4),'target':dict(x=read(0x577fd4),z=read(0x577fd8))},seed=read(0x820454)&0xffffffff,speed=read(0x577fec),heading=read(0x577fe8)&0xffffffff,verticalSpeed=read(0x577ff0),holeCounter=read(0x574524+current['state']['actor']['hole']*520),partner=dict(actorClass=u.mem_read(0x578020,1)[0],reaction=u.mem_read(0x57808c,1)[0])))
...
for(const [q,e] of JSON.parse(readFileSync(0,'utf8'))){const snapshots=[],effects=middleEffects(q);const a=run(q,middleMap(q),{emit:(event,state)=>{const expected=q.reactionSnapshots[snapshots.length];snapshots.push({actor:Object.fromEntries(Object.keys(expected.actor).map(k=>[k,state.actor[k]])),seed:state.seed,speed:state.speed,heading:state.heading,verticalSpeed:state.verticalSpeed,holeCounter:state.holeCounter,partner:structuredClone(state.partner)});return effects.emit(event,state);}});if(!isDeepStrictEqual(snapshots,q.reactionSnapshots||[])){console.error(JSON.stringify({diff:snapshots.map((a,i)=>({actual:a,expected:q.reactionSnapshots[i]})).filter(r=>!isDeepStrictEqual(r.actual,r.expected)).slice(0,1)}));process.exit(1);}if(!isDeepStrictEqual(a,e))throw Error(JSON.stringify({q,a,e}));}
