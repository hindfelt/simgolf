...
def hook(u,a,size,data):
 ...
 if a in [0x46c140,0x4672d0,0x40c1f0]:
  sp=u.reg_read(UC_X86_REG_ESP);args=[get(sp+4*j) for j in range(1,2 if a==0x46c140 else 5 if a==0x40c1f0 else 4)];calls.append(dict(address=a,args=args))
...
for i in range(700):
 ...
 q.update(actorIndex=23*50+23,actorTile=dict(x=23,z=23),nextTerrain=1,phaseCounter=i,worldFlags=0,selectionState=-1,cachedFlags=0,cartUpgrade=0,fastWalking=0,destination=dict(x=24250,z=23800),walkingOverride=0);put(0x59d208,0);put(0x5a4440,-1);put(0x599a9c,0);put(0x831828,i);put(0x102070,500);put(0x10203c,q['actorIndex']);put(0x102040,23);put(0x10204c,23);put(0x102050,0);put(0x102034,0);u.mem_write(0x570d38,bytes([10]*2500));u.mem_write(0x53ba00,bytes(5000));u.mem_write(0x576dc2+48,bytes([0]));u.mem_write(0x576dc5+48,bytes([1]));u.mem_write(0x577f00,bytes(b));put(0x5a446c,q['avoidanceCursor']);put(0x568f6c,100);put(0x820454,q['seed']);put(0x102010,id);put(0x102024,q['followPartner']);u.reg_write(UC_X86_REG_ESP,0x102000);u.reg_write(UC_X86_REG_EBP,id*256);u.reg_write(UC_X86_REG_FPCW,0x37f);end=None;calls=[];u.emu_start(0x42a71c,0x400fff,count=100000);assert end
 rows.append([q,list(b),dict(actors=list(u.mem_read(0x577f00,len(b))),seed=get(0x820454)&0xffffffff,calls=calls,next=end)])
script="""import {readFileSync} from 'node:fs';import {isDeepStrictEqual} from 'node:util';const {originalWalkingNearTick}=await import(MODULE);const rows=JSON.parse(readFileSync(0,'utf8'));const coverage={};for(const [q,b,e] of rows){const r=originalWalkingNearTick({...q,actors:Array.from({length:152},(_,i)=>new Uint8Array(b.slice(i*256,(i+1)*256))),terrain:new Uint8Array(2500).fill(10),tileFlags:new Uint16Array(2500),metadata:[{}, {walkingCost:1,shotClass:0}]},(event,state)=>{if(event.address===0x4672d0)state.actors[event.args[0]][0x78]=36;return {state,value:1};});const a={actors:r.state.actors.flatMap(b=>Array.from(b)),seed:r.state.seed,calls:r.calls,next:r.next};if(!isDeepStrictEqual(a,e))throw Error(JSON.stringify({q,fields:Object.keys(e).filter(k=>!isDeepStrictEqual(a[k],e[k]))}));coverage[r.next]=(coverage[r.next]??0)+1;const before=b.slice(q.actorId*256+8,q.actorId*256+16),after=Array.from(r.state.actors[q.actorId].slice(8,16));if(!isDeepStrictEqual(before,after))coverage.positionChanges=(coverage.positionChanges??0)+1;}console.log(`${rows.length} continuous native near-walking ticks match. ${JSON.stringify(coverage)}`);""".replace('MODULE',json.dumps((root/'simgolf-reborn/scene/src/simulation/original-walking-near-tick.js').as_uri()))
