"""Original settlement gates, first-fee message, popup RNG and posting; controlled name and integer formatting."""
...
for a,n in [(0x426c92,0x1d9),(0x40c580,0x76),(0x40c7f0,0xf7),(0x45ba70,0x60),(0x4b9800,8),(0x4a57a0,0x27),(0x4c0000,0x30000)]:u.mem_write(a,p.get_data(a-0x400000,n))
...
for units in [-2147483648,-32769,-10,-1,0,1,10,32767,2147483647]:
 for i in range(128):
  ...
  u.reg_write(UC_X86_REG_ESP,0x102000);u.reg_write(UC_X86_REG_EBP,0);u.reg_write(UC_X86_REG_ECX,units&0xffffffff);u.reg_write(UC_X86_REG_EBX,0);events=[];u.emu_start(0x426c92,0x426e6b,count=4000)
  ...
  rows.append([q,dict(state=out,events=events,popupRandomDraws=0 if out['seed']==123 else 2,posted=not bool(q['globalFlags']&0x200000))])
script="""import {readFileSync} from 'node:fs';import {isDeepStrictEqual} from 'node:util';import {originalFeeSettlement} from MODULE;for(const [q,out] of JSON.parse(readFileSync(0,'utf8'))){for(const s of [q.state,out.state]){s.actors[0]=Uint8Array.from(s.actors[0]);for(const id in s.holeRecords)s.holeRecords[id]=Uint8Array.from(s.holeRecords[id]);}const r=originalFeeSettlement(q);if(!isDeepStrictEqual(r,out))throw Error(JSON.stringify({q,out,r}));}""".replace('MODULE',json.dumps((root/'simgolf-reborn/scene/src/simulation/original-fee-settlement.js').as_uri()))
subprocess.run(['node','--input-type=module','-e',script],input=json.dumps(rows),text=True,check=True);print(len(rows),'native fee settlements matched')
(root/'simgolf-reborn/scene/tests/fixtures/original-fee-settlement.json').write_text(json.dumps(rows,separators=(',',':'))+'\n')
