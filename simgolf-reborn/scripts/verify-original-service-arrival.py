...
for i in range(1200):
 ...
 b[0x29]=1;struct.pack_into('<h',b,0xaa,1);struct.pack_into('<i',b,0xdc,0);struct.pack_into('<i',b,0xec,0);partner=bytearray(256);partner[0x29]=rng.choice([0,1,2]);partner[0x2a]=rng.randrange(2);struct.pack_into('<ii',partner,8,23000,24000);h=bytearray(20*520);struct.pack_into('<ii',h,520+0x18,25,25);decorations=bytearray(2500);decorations[23*50+24]=q['restDecoration'];q.update(serviceIndex=-2,walkingOverride=0,movementReady=1,waitingGroups=0,startBall=dict(x=24000,z=23000));u.mem_write(0x574500,bytes(h));u.mem_write(0x577f00,bytes(b)+bytes(partner));u.mem_write(0x5682dc+23*50+24,bytes([q['restDecoration']]));put(0x820454,q['seed'])
 for off,v in [(0x1c,-2),(0x34,0),(0x44,1),(0x38,0),(0x64,24000),(0x68,23000),(0x10,0),(0x40,23),(0x4c,24),(0x6c,23*50)]:put(0x102000+off,v)
 u.reg_write(UC_X86_REG_ESP,0x102000);u.reg_write(UC_X86_REG_EBP,0);u.reg_write(UC_X86_REG_FPCW,0x37f);calls=[];end=False;attempts=0;u.emu_start(0x42a019,0x400fff,count=100000);assert end
 rows.append([q,list(b),list(partner),list(h),list(decorations),list(terrain),flags,metadata,dict(next=end,actor=list(u.mem_read(0x577f00,256)),seed=get(0x820454)&0xffffffff,calls=calls)])
script="""import {readFileSync} from 'node:fs';import {isDeepStrictEqual} from 'node:util';const {originalServiceArrival}=await import(MODULE);const rows=JSON.parse(readFileSync(0,'utf8'));for(const [q,b,p,h,d,t,f,m,e] of rows){let attempts=0;const r=originalServiceArrival({...q,actors:[new Uint8Array(b),new Uint8Array(p)],holes:Array.from({length:20},(_,i)=>new Uint8Array(h.slice(i*520,(i+1)*520))),restDecorations:new Uint8Array(d),terrain:new Uint8Array(t),tileFlags:new Uint16Array(f),metadata:m.map(shotClass=>({shotClass}))},(event,state)=>{if(event.address===0x4672d0)state.actors[0][0x8d]=event.args[1];return {state,value:event.address===0x4071d0?Number(++attempts>=q.acceptAfter):0};});const a={next:r.next,actor:Array.from(r.state.actors[0]),seed:r.state.seed,calls:r.calls};if(!isDeepStrictEqual(a,e))throw Error(JSON.stringify({q,fields:Object.keys(e).filter(k=>!isDeepStrictEqual(a[k],e[k]))}));}console.log(`${rows.length} native rest-through-partner arrivals match actors, RNG and ordered effects.`);""".replace('MODULE',json.dumps((root/'simgolf-reborn/scene/src/simulation/original-service-arrival.js').as_uri()))
