...
for q,_ in json.loads((root/'simgolf-reborn/scene/tests/fixtures/original-search-launch.json').read_text()):
 ...
 q['initialRecords']=initial_records;q['initialHoles']=initial_holes;q['finalHoles']=list(u.mem_read(0x574500,20*520));q['finalRecords']=list(u.mem_read(0x577f00,512));rows.append([q,final]);print('Native outer planner finished',q['scenario'],q['mode'],flush=True)
...
const {originalPlannerInput}=await import(INPUT);const {originalPlannerActor}=await import(ACTOR);const {applyOriginalPlannerResult}=await import(RESULT);
for(const [q,e] of JSON.parse(readFileSync(0,'utf8'))){const snapshot={actorId:0,actors:[Uint8Array.from(q.initialRecords.slice(0,256)),Uint8Array.from(q.initialRecords.slice(256))],holes:Array.from({length:20},(_,i)=>Uint8Array.from(q.initialHoles.slice(i*520,(i+1)*520))),seed:q.automatic.state.seed,strengthCache:q.automatic.state.cache,diagnostics:q.automatic.state.diagnostics,landing:q.automatic.state.landing};const prepared=originalPlannerInput(snapshot,q.automatic);const {searched,...a}=originalAutomaticPlanner(prepared,{map:searchLaunchMap(q),physical:{luck:5},searchState:q.outerSearchState,score:0,scoreAt:(x,z)=>q.aimScores[x*50+z]},{emit:(_event,state)=>state});const actors=[Uint8Array.from(q.initialRecords.slice(0,256)),Uint8Array.from(q.initialRecords.slice(256))],input={actors,actorId:0,holes:Array.from({length:20},(_,i)=>Uint8Array.from(q.initialHoles.slice(i*520,(i+1)*520))),metadata:Array.from({length:23},()=>({shotClass:99,scatterCoefficient:99}))};if(!isDeepStrictEqual(originalPlannerActor(input),q.automatic.state.actor))throw Error('Planner actor input differs from native');const applied=applyOriginalPlannerResult(input,a);const shared={seed:applied.seed,cache:applied.strengthCache,diagnostics:applied.diagnostics,landing:applied.landing,holes:applied.holes.flatMap(b=>Array.from(b)),overrides:[17,20].map(code=>({code,shotClass:applied.metadata[code].shotClass}))};const expectedShared={seed:e.state.seed,cache:e.state.cache,diagnostics:e.state.diagnostics,landing:e.state.landing,holes:q.finalHoles,overrides:e.shotClassOverrides};if(!isDeepStrictEqual(shared,expectedShared))throw Error('Planner shared state differs from native');const expected=Uint8Array.from(q.finalRecords);const diff=applied.actors.flatMap(b=>Array.from(b)).flatMap((v,i)=>v===expected[i]?[]:[[i,v,expected[i]]]);if(diff.length)throw Error(JSON.stringify({scenario:q.scenario,recordDiff:diff}));if(!isDeepStrictEqual(a,e)){
...
console.log('Six packed-input automatic planner integrations match native outputs and shared state.');
