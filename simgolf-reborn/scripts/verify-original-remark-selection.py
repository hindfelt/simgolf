...
for a,n in [(0x467502,0x870),(0x469080,0xdd)]:
...
for a in (0x46c140,0x40c1f0,0x4a0000):u.mem_write(a,b'\xc3')
q=None;events=[]
def hook(u,a,size,data):
 ...
 if q['mutate']:
  if a==0x46c140:put(0x577f08,2048)
...
script="""import {readFileSync} from 'node:fs';import {isDeepStrictEqual} from 'node:util';import {originalRemarkSelection} from MODULE;const rows=JSON.parse(readFileSync(0,'utf8'));for(const [q,expected] of rows){q.state.actor=Uint8Array.from(q.state.actor);const got=originalRemarkSelection(q,(event,state)=>{const v=new DataView(state.actor.buffer);if(q.mutate){if(event.address===0x46c140)v.setInt32(0,2048,true);if(event.address===0x40c1f0){v.setInt16(0xb6,1,true);state.actor[0x1d]=99;}if(event.address===0x4a0000)state.actor[0x1e]=88;}return {state,result:event.address===0x46c140?q.voice:0};});got.state.actor=[...got.state.actor];if(!isDeepStrictEqual(got,expected))throw Error(JSON.stringify({q,expected,got}));}console.log(`${rows.length} native per-kind selection cases matched`);""".replace('MODULE',json.dumps(module))
