"""Continuous native destination preparation; only visitor cleanup is controlled."""
...
def hook(u,a,size,data):
 global end
 if a in [0x4295ef,0x429f27]:end='skip' if a==0x4295ef else hex(a);u.emu_stop()
...
for i in range(500):
 ...
 u.mem_write(0x577f00,bytes(b));u.mem_write(0x574500,bytes(h));u.mem_write(0x58a708,bytes(f));u.mem_write(0x570d38,t);u.mem_write(0x53ba00,struct.pack('<2500H',*marks))
 ...
 u.reg_write(UC_X86_REG_ESP,0x102000);u.reg_write(UC_X86_REG_EBP,id*256);u.reg_write(UC_X86_REG_EAX,b[id*256+0x29]);u.reg_write(UC_X86_REG_FPCW,0x37f);end=None;calls=[];u.emu_start(0x4290ca,0x400fff,count=100000);assert end
 ...
 if end!='skip':e['destination']=dict(x=u.reg_read(UC_X86_REG_EBX),z=u.reg_read(UC_X86_REG_ESI))
 rows.append([q,list(b),list(h),list(f),list(t),marks,widths,metadata,e])
script="""import {readFileSync} from 'node:fs';import {isDeepStrictEqual} from 'node:util';const {originalWalkingPreparation}=await import(MODULE);const rows=JSON.parse(readFileSync(0,'utf8'));const coverage={};for(const [q,b,h,f,t,m,w,metadata,e] of rows){const r=originalWalkingPreparation({...q,actors:Array.from({length:152},(_,i)=>new Uint8Array(b.slice(i*256,(i+1)*256))),holes:Array.from({length:20},(_,i)=>new Uint8Array(h.slice(i*520,(i+1)*520))),facilityRecords:new Uint8Array(f),facilityWidths:w,terrain:new Uint8Array(t),tileFlags:new Uint16Array(m),metadata},(event,state)=>{state.actors[event.args[0]][0x29]=0;state.queueClock=(state.queueClock+17)|0;return {state};});const a={actors:r.state.actors.flatMap(b=>Array.from(b)),waitingGroups:r.waitingGroups,followPartner:r.followPartner,cupHeading:r.cupHeading,movementReady:r.movementReady??q.movementReady,serviceIndex:r.serviceIndex??q.serviceIndex,seed:r.state.seed,calls:r.calls,next:r.next};Object.assign(a,{queueClock:r.state.queueClock,nearestFacilityDistance:r.state.nearestFacilityDistance,serviceTileX:r.state.serviceTileX,serviceTileZ:r.state.serviceTileZ});if(r.destination)a.destination=r.destination;coverage[r.next]=(coverage[r.next]??0)+1;for(const call of r.calls){const key=call.address.toString(16)+(call.address===0x40daa0?':'+call.args[0]:'');coverage[key]=(coverage[key]??0)+1;}if(!isDeepStrictEqual(a,e))throw Error(JSON.stringify({q,fields:Object.keys(e).filter(k=>!isDeepStrictEqual(a[k],e[k])),actual:{...a,actors:undefined,calls:undefined},expected:{...e,actors:undefined,calls:undefined}}));}console.log(`${rows.length} continuous native walking preparations match. ${JSON.stringify(coverage)}`);""".replace('MODULE',json.dumps((root/'simgolf-reborn/scene/src/simulation/original-walking-preparation.js').as_uri()))
