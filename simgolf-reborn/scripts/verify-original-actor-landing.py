...
def hook(u,a,size,data):
 global end
 if a in [0x42c9ea,0x42ca6c]:end=hex(a);u.emu_stop();return
...
for i in range(1600):
 ...
 q.update(luck=rng.randrange(256),scatterCoefficient=rng.randrange(-2,6),seed=rng.randrange(2**32),direction=rng.randrange(8),slopes=[rng.randrange(-8,9) for _ in range(8)],currentTerrainCode=rng.choice([1,17]))
 ...
 assert end
 rows.append([q,list(b),dict(actor=list(u.mem_read(0x577f00,256)),calls=calls,next=end,landed=end=='0x42c9ea',seed=read(0x820454)&0xffffffff,terrainStopX=read(0x57724c),terrainStopZ=read(0x577250),terrainStopState=read(0x4c1e18))])
script="""import {readFileSync} from 'node:fs';import {isDeepStrictEqual} from 'node:util';
const {originalActorLanding}=await import(MODULE);const rows=JSON.parse(readFileSync(0,'utf8'));let effects=0;
for(const [q,b,e] of rows){const hole=new Uint8Array(520),h=new DataView(hole.buffer);h.setInt32(0x18,20,true);h.setInt32(0x1c,20,true);const r=originalActorLanding({...q,actors:[new Uint8Array(b)],holes:[null,hole],terrainStopX:77,terrainStopZ:77,terrainStopState:77},(event,state)=>{const a=new DataView(state.actors[0].buffer);if(event.address===0x40c1f0)a.setInt32(0xf0,q.replacementVelocity,true);if(event.address===0x4096e0)a.setUint32(0x18,0x8123,true);return {state,value:event.address===0x40c140?q.slopes[event.args[2]]:q.currentTerrainCode};});const a={actor:Array.from(r.state.actors[0]),calls:r.calls,next:r.next,landed:r.landed,seed:r.state.seed,terrainStopX:r.state.terrainStopX,terrainStopZ:r.state.terrainStopZ,terrainStopState:r.state.terrainStopState};if(!isDeepStrictEqual(a,e))throw Error(JSON.stringify({q,a,e}));effects+=r.calls.length;}console.log(`${rows.length} continuous native landing sequences match; ${effects} ordered effects.`);
""".replace('MODULE',json.dumps((root/'simgolf-reborn/scene/src/simulation/original-actor-landing.js').as_uri()))
