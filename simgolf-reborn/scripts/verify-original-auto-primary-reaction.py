...
script="""import {readFileSync} from 'node:fs';import {isDeepStrictEqual} from 'node:util';const {originalAutoPrimaryReaction}=await import(MODULE);
for(const [q,e] of JSON.parse(readFileSync(0,'utf8'))){const a=originalAutoPrimaryReaction(q,{
 terrainAt:()=>3,shotClassAt:code=>code===q.terrainCode?q.originClass:q.targetClass,holeRecordAt:()=>q.holeRecord,
...
for flags in range(65536):
 actor_id=flags%152
 u.mem_write(0x577f90+actor_id*256,struct.pack('<H',flags))
 write(0x102000,0x400fff);write(0x102004,actor_id);u.reg_write(UC_X86_REG_ESP,0x102000)
 u.emu_start(0x466ea0,0x400fff,count=100);assert u.reg_read(UC_X86_REG_EIP)==0x400fff
 classes.append(u.reg_read(UC_X86_REG_EAX))
condition_module=(root/'simgolf-reborn/scene/src/simulation/original-condition-class.js').as_uri()
condition_script="""import {readFileSync} from 'node:fs';const {originalConditionClass}=await import(MODULE);
const expected=JSON.parse(readFileSync(0,'utf8'));
for(let flags=0;flags<expected.length;flags++)if(originalConditionClass(flags)!==expected[flags])throw Error(`Condition mismatch ${flags}`);
console.log('All 65536 actor condition words match the native classifier.');
""".replace('MODULE',json.dumps(condition_module))
subprocess.run(['node','--input-type=module','-e',condition_script],input=json.dumps(classes),text=True,check=True)
if '--write-fixture' in sys.argv:
 (root/'simgolf-reborn/scene/tests/fixtures/original-auto-primary-reaction.json').write_text(json.dumps(rows[:180],separators=(',',':'))+'\n')
