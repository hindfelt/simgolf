...
for a,n in [(0x424cb6,0x34b),(0x45ba70,0x60),(0x4b9800,8),(0x4a57a0,0x27)]:
 o=p.get_offset_from_rva(a-0x400000);u.mem_write(a,p.__data__[o:o+n])
u.reg_write(UC_X86_REG_FPCW,0x37f)
u.mem_write(0x4672d0,b'\xc3');u.mem_write(0x466ea0,b'\xc3')
...
def hook(u,a,size,data):
 ...
 if a==0x466ea0:u.reg_write(UC_X86_REG_EAX,int(current['featureEligible']))
 if a==0x4672d0:
...
def run(q):
 ...
 result=dict(actorFlags=read(0x577f18+s)&0xffffffff,hole=actor['hole'],shotCounter=actor['shotCounter'],stateCode=actor['stateCode'],elevationCounter=u.mem_read(0x577f23+s,1)[0])
...
for(const [q,e] of JSON.parse(readFileSync(0,'utf8'))){const a=originalAutoPrimaryReaction(q,{
 terrainAt:()=>3,shotClassAt:code=>code===q.terrainCode?q.originClass:q.targetClass,featureEligible:()=>q.featureEligible,holeRecordAt:()=>q.holeRecord,
