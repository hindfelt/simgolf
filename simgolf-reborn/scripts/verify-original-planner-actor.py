...
for q,_ in json.loads((root/'simgolf-reborn/scene/tests/fixtures/original-search-launch.json').read_text()):
 ...
 initial_records=list(u.mem_read(0x577f00,512))
 ...
 q['initialRecords']=initial_records;q['finalRecords']=list(u.mem_read(0x577f00,512));rows.append([q,final]);print('Native outer planner finished',q['scenario'],q['mode'],flush=True)
...
const {originalPlannerActor,applyOriginalPlannerActor}=await import(ACTOR);
for(const [q,e] of JSON.parse(readFileSync(0,'utf8'))){const {searched,...a}=originalAutomaticPlanner(q.automatic,{map:searchLaunchMap(q),physical:{luck:5},searchState:q.outerSearchState,score:0,scoreAt:(x,z)=>q.aimScores[x*50+z]},{emit:(_event,state)=>state});const actors=[Uint8Array.from(q.initialRecords.slice(0,256)),Uint8Array.from(q.initialRecords.slice(256))],input={actors,actorId:0};if(!isDeepStrictEqual(originalPlannerActor(input),q.automatic.state.actor))throw Error('Planner actor input differs from native');const applied=applyOriginalPlannerActor(input,a.state);const expected=Uint8Array.from(q.finalRecords);const diff=applied.actors.flatMap(b=>Array.from(b)).flatMap((v,i)=>v===expected[i]?[]:[[i,v,expected[i]]]);if(diff.length)throw Error(JSON.stringify({scenario:q.scenario,recordDiff:diff}));if(!isDeepStrictEqual(a,e)){
...
console.log('Six automatic planner actor read/write integrations match native actor and adjacent-slot bytes.');
""".replace('ACTOR',json.dumps((root/'simgolf-reborn/scene/src/simulation/original-planner-actor.js').as_uri())).replace('PLANNER',json.dumps((root/'simgolf-reborn/scene/src/simulation/original-automatic-planner.js').as_uri())).replace('MAP',json.dumps((root/'simgolf-reborn/scene/tests/helpers/original-search-launch.js').as_uri()))
