...
Choose **Inspect / pan** to drag the view. In construction mode, right-drag pans; two fingers pan/zoom on touch. Scroll/pinch zooms. Use the ↶ / ↷ buttons or Q / E to rotate the course in 45° steps; Home (⌂) restores the original view. Hold Space and drag to pan; use the Pause button to pause. Right-click removes objects in construction mode. Construction tools paint a square grid and reject overlaps with tees, greens, buildings and protected course features. Water and player-built bridges can be edited. The old art study remains at `http://127.0.0.1:4176/?mode=art` with its own review controls.
