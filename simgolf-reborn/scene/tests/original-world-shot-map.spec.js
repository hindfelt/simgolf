...
function world(){return {phaseCounter:1,globalFlags:0,terrain:new Uint8Array(2500).fill(2),heights:new Uint8Array(2601).fill(3),tileFlags:new Uint16Array(2500),metadata:Array.from({length:23},(_,i)=>originalTerrainMetadata(i)),derived:{surfaceHeights:new Int8Array(2500),directionHeights:new Int8Array(20000).fill(6),edgeMasks:new Uint8Array(2500)}};}
test('planning and motion bind the same terrain while preserving social readers',()=>{
 const s=world();s.terrain[510]=1;s.tileFlags[510]=128;s.derived.edgeMasks[510]=64;s.heights[10*51+10]=7;
 const map=originalWorldShotMap(s,{planning:{terrainAt:()=>99,heightAt:()=>99,categoryAt:()=>16}});
 expect(map.planning.terrainAt(10,10)).toBe(1);expect(map.planning.heightAt(10,10)).toBe(7);expect(map.planning.categoryAt(1)).toBe(16);
