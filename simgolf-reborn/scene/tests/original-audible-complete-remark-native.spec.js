...
test('audible complete remarks match continuous original execution including world write-back',()=>{
 for(const [q,expected] of rows){
  const before=structuredClone(q);
  const result=originalAudibleCompleteRemark(q,{
   resolvePhrase:(e,s)=>{
  ...
  expect(written.queued).toBe(expected.state.queued);expect(written.sequenceIndex).toBe(expected.state.sequenceIndex);expect(written.seed).toBe(expected.state.seed);expect(written.actors).toEqual(expected.state.actors);
