...
test('missing ball reaches tee stance and service admission',()=>{
 ...
 const r=originalWalkingPreparation(s,()=>{throw Error('Unexpected cleanup');});
 expect(r.next).toBe('0x429e0d');expect(r.serviceIndex).toBe(0);expect(r.destination).toEqual({x:24064,z:24064});expect(r.teePosition).toEqual({x:20992,z:20992});expect(r.destination).not.toEqual(r.teePosition);
});
test('unavailable primary facility falls back to a marked service tile',()=>{
 ...
 const r=originalWalkingPreparation(s,()=>{throw Error('Unexpected cleanup');});
 expect(r.next).toBe('0x429e0d');expect(r.serviceIndex).toBe(-2);expect(r.destination).toEqual({x:24064,z:24064});expect(r.calls.map(c=>c.address)).toEqual([0x40daa0,0x40db60]);
});
test('partner progress permits a secondary facility after other service checks',()=>{
 const s=fresh(),a=new DataView(s.actors[0].buffer);a.setInt32(0xdc,0,true);s.actors[1][0x29]=0;s.actors[1][0x2a]=1;
 s.facilityRecords=new Uint8Array(4096);s.facilityWidths=Array(16).fill(2);const f=new DataView(s.facilityRecords.buffer);f.setInt16(0,3,true);f.setInt16(2,23,true);f.setInt16(4,23,true);
 const r=originalWalkingPreparation(s,()=>{throw Error('Unexpected cleanup');});
 expect(r.next).toBe('0x429e0d');expect(r.serviceIndex).toBe(0);expect(r.destination).toEqual({x:24064,z:24064});expect(r.calls.map(c=>c.args[0])).toEqual([3]);
});
