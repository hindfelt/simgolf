...
test('missing ball reaches tee stance and service admission',()=>{
 ...
 const r=originalWalkingPreparation(s,()=>{throw Error('Unexpected cleanup');});
 expect(r.next).toBe('0x429f27');expect(r.serviceIndex).toBe(0);expect(r.destination).toEqual({x:24064,z:24064});expect(r.teePosition).toEqual({x:20992,z:20992});expect(r.destination).not.toEqual(r.teePosition);
});
test('unavailable primary facility falls back to a marked service tile',()=>{
 ...
 const r=originalWalkingPreparation(s,()=>{throw Error('Unexpected cleanup');});
 expect(r.next).toBe('0x429f27');expect(r.serviceIndex).toBe(-2);expect(r.destination).toEqual({x:24064,z:24064});expect(r.calls.map(c=>c.address)).toEqual([0x40daa0,0x40db60]);
});
