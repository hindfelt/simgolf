...
const rows=JSON.parse(readFileSync(new URL('./fixtures/original-audible-after-fee-completion.json',import.meta.url)));
...
test('actual conditional completion remark through reset matches continuous native execution',()=>{
 const kinds=new Set();let skipped=0;
 for(const [q,out] of rows){const before=structuredClone(q);const r=originalAudibleAfterFeeCompletion(q,{
