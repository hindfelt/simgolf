...
test('reaction mutations select the refreshed scorecard and event continuation',()=>{
 ...
 expect(r.state.completionRecords[0][24]).toBe(6);expect(new DataView(r.state.holeRecords[2].buffer).getInt32(0x1ec,true)).toBe(50);expect(r.next).toBe('0x427e25');
});
