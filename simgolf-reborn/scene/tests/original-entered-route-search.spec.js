...
import {originalEnteredRouteSearch} from '../src/simulation/original-route-search.js';
const rows=JSON.parse(readFileSync(new URL('./fixtures/original-entered-route-search.json',import.meta.url),'utf8'));
function run(q){
 const cells=new Map(q.cells.map(([x,z,code,shotClass,flags])=>[`${x},${z}`,{code,shotClass,flags}]));
 const calls=[];let i=0;
 const result=originalEnteredRouteSearch({...q,terrainAt:p=>cells.get(`${p.x},${p.z}`),
  assessShot:c=>{calls.push({type:'assess',...c});return q.costs[c.shape+1]+c.flag*3;}},
  c=>{calls.push({type:'initialRange',...c});return q.range;},
...
test('entered search matches original tables, published result and ordered callbacks',()=>{
...
test('no candidate publishes cup fallback and clears temporary search state',()=>{
 ...
 expect(result.result.candidateSkillMask).toBe(7);expect(calls.map(c=>c.type)).toEqual(['initialRange','range']);
});
test('entered search replays from serialized state',()=>{
