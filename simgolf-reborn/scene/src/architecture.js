...
export function buildClubhouse(scene, environment = null) {
  if(environment === "tropical" || environment === "links" || environment === "desert") return buildRegionalClubhouse(scene,environment);
...
function buildRegionalClubhouse(scene,environment){
 const tropical=environment==='tropical',desert=environment==='desert',g=new THREE.Group();g.name='clubhouse';g.userData.environment=environment;
 ...
 const wall=mat(tropical?0xaa8250:desert?0xcfab7c:0xc5c3a4,tropical?'bark':'stone'),wood=mat(0x624932,'bark'),trim=mat(tropical?0xdeb77c:0xe0ddc0),roof=mat(tropical?0xc6ab71:0x778586),glass=mat(0x294e4f);
 ...
 const roofPart=(x,z,w,d)=>{
  if(desert){
   b.box(w,.35,d,wall,x,5.85,z);
   for(const sign of [-1,1]){b.box(w,.65,.28,wall,x,6.2,z+sign*(d/2-.14));b.box(.28,.65,d,wall,x+sign*(w/2-.14),6.2,z);}
   return;
  }
 ...
 if(!tropical&&!desert){for(const x of [-10,9]){b.box(1.2,3,1.3,wall,x,8,-1);b.box(1.5,.25,1.6,trim,x,9.5,-1);}}
