import {LANDSCAPES} from './simulation/generated-landscape.js';
import {ENVIRONMENTS} from './simulation/environments.js';
  ...
  try{
   ...
   for(const event of competitions){
    ...
    async function view(){try{
     ...
     const setting=document.createElement('p');setting.textContent=`${LANDSCAPES[current.landscape]||LANDSCAPES.classic} · ${ENVIRONMENTS[current.environment]?.name||'Classic mixed resort'}`;body.append(setting);
