import {tournamentCalendar} from './tournament-calendar.js';
   ...
   for(const event of tournaments){
    ...
    async function view(){
     try{
      ...
      const info=document.createElement('p');info.textContent=`${current.rounds} ${current.rounds===1?'round':'rounds'} · course version ${current.courseDigest.slice(0,8)}. ${current.status==='complete'?'This event is complete. Results are permanent.':current.status==='locked'?'Registration closed. Entrants can play or resume their rounds.':current.status==='cancelled'?'This event is cancelled.':'Registration is open.'}`;body.append(info);const deadline=document.createElement('p');deadline.textContent=current.endsAt?`Finish and save all rounds by ${new Date(current.endsAt).toLocaleString()}. Unfinished entrants receive no placing.`:`Playing window: ${current.durationHours<24?`${current.durationHours} ${current.durationHours===1?'hour':'hours'}`:`${current.durationHours/24} days`} after ${current.startsAt?'the scheduled start':'registration closes'}. Finish and save all rounds before the deadline; unfinished entrants receive no placing.`;body.append(deadline);
      if(current.startsAt){const start=document.createElement('p');start.textContent=`Scheduled start: ${new Date(current.startsAt).toLocaleString()}. Join before this time.`;body.append(start);const calendar=document.createElement('button');calendar.textContent='Add to calendar';calendar.onclick=()=>{const url=URL.createObjectURL(new Blob([tournamentCalendar(current,location.origin)],{type:'text/calendar'})),a=document.createElement('a');a.href=url;a.download='fairway-baron-tournament.ics';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);};body.append(calendar);}
      ...
      if(['locked','complete'].includes(current.status)){
       ...
       for(const score of scores.standings){const line=document.createElement('p');line.textContent=`${scores.status==='complete'&&score.rank<=3&&!score.withdrawn?['','Gold · ','Silver · ','Bronze · '][score.rank]:''}${score.rank?score.rank+'. ':''}${score.name}${score.withdrawn?(score.withdrawalReason==='deadline'?' (deadline missed)':' (withdrawn)'):''}: ${score.roundsCompleted}/${scores.rounds} rounds · ${score.roundsCompleted?score.strokes:'—'} strokes`;body.append(line);}
      ...
      const actions=current.ownerId===player.id?(['cancelled','complete'].includes(current.status)?[]:current.status==='locked'?[['cancel','Cancel tournament']]:current.startsAt?[['cancel','Cancel tournament']]:[['lock','Close registration'],['cancel','Cancel tournament']]):current.status==='registration'?[entered?['leave','Leave tournament']:['join','Join tournament']]:[];
