...
try{
 ...
 for(const provider of providers){
  ...
  const link=document.createElement('a');link.className='provider';link.href=`/api/auth/${provider.id}/start`;link.textContent=`Continue with ${provider.name}`;document.querySelector('#providers').append(link);
 }
...
emailForm.onsubmit=async e=>{e.preventDefault();const button=emailForm.querySelector('button');button.disabled=true;try{const result=await post('/api/auth/email/start',{email:document.querySelector('#email').value});codeId=result.id;codeForm.hidden=false;document.querySelector('#code').focus();message('Check your inbox. Your code expires in 10 minutes.');}catch(e){message(e.message);}finally{button.disabled=false;}};
codeForm.onsubmit=async e=>{e.preventDefault();const button=codeForm.querySelector('button');button.disabled=true;try{await post('/api/auth/email/verify',{id:codeId,code:document.querySelector('#code').value});location.assign('/');}catch(e){message(e.message);}finally{button.disabled=false;}};
