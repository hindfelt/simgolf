...
export function originalActorMotionContext(snapshot,locals={}){
 ...
 const a=new DataView(b.buffer,b.byteOffset,b.byteLength),x=locals.ballTile?.x??(a.getInt32(0xdc,true)>>10),z=locals.ballTile?.z??(a.getInt32(0xe0,true)>>10),index=x*50+z;
 if(x<0||z<0||x>=50||z>=50)throw Error('Original motion outside map requires outer actor handling.');
 const code=locals.ballTerrain??originalTerrainByte(state,x,z),m=state.metadata?.[code];
