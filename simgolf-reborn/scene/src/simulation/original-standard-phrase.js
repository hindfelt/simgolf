...
export function originalStandardPhrase(q,resolve){
 ...
 const call=(address,args)=>{const event={address,args};events.push(event);if(typeof resolve!=='function')throw Error('Original standard phrase helper requires a resolver.');const reply=resolve(structuredClone(event),structuredClone(state));if(!reply||typeof reply.then==='function')throw Error('Expected synchronous speculative phrase state.');state=structuredClone(reply);};
...
export function originalDescribedStandardPhrase(q,names,locationContext){
 return originalStandardPhrase(q,(event,state)=>{
  if(event.address===0x466fb0){state.sourceText=originalActorName({...names,actorId:event.args[0],actor:state.actors[event.args[0]],sourceText:state.sourceText,appendComma:false});return state;}
