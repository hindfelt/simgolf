...
export function originalActorGroundDecision(snapshot,resolve){
 ...
 const terrainAt=(x,z)=>{if(x<0||z<0||x>=50||z>=50)throw Error('Original ground neighbor storage required.');return (state.terrain[x*50+z]<<24)>>24;};
 ...
 const capture=originalCupCapture({x:a.getInt32(0xdc,true),z:a.getInt32(0xe0,true),cellX,cellZ,terrainCode:terrainAt(cellX,cellZ),cellFlags:state.tileFlags[cellX*50+cellZ],speed:contact.speed,club:a.getUint8(0x24),eventFlag:!!(state.worldFlags&0x200000)});
