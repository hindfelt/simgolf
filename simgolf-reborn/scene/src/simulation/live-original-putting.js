...
export function startLiveOriginalPutt(g,v,cup,{lie,height,skill,blocked=()=>false}){
 ...
 for(let t=0;t<=1;t+=.1)for(const side of [-.35,0,.35]){
  ...
  if(blocked(from,{x,z})||lie(g,{x,z})!=='green')return null;
  if(Math.abs(height(g,x,z)-level)>.001){if(g.liveFlightVersion!==2)return null;contours=true;}
 }
 ...
 return {...(contours?{contours:true}:{}),cup:{x:cup.x,z:cup.z},level,elapsed:0,state:originalFlatPuttStart({x,z,height:0,verticalSpeed:0,club:13,heading:originalHeading(centre-x,centre-z),angularOffset:aim.angularOffset,speed:launch.speed,seed:g.rng},{cupX:20,cupZ:20,eventFlag:!!v.pro})};
}
export function stepLiveOriginalPutt(g,shot,dt,{lie,height,blocked=()=>false}){
 ...
 while(p.elapsed>=.05-1e-9&&p.state.status==='rolling'){
  ...
  if(p.contours){
   // Live terrain gravity is an explicit adaptation. Recovered flat-green
   // stepping/cup capture remains intact; native origin-green motion ignores
   // directional slopes, so browser contours require their own force binding.
   const b=p.state.ball,a=b.heading/0x100000000*2*Math.PI;
   const gx=(height(g,point.x+.25,point.z)-height(g,point.x-.25,point.z))*2;
   const gz=(height(g,point.x,point.z+.25)-height(g,point.x,point.z-.25))*2;
   const vx=Math.sin(a)*b.speed-Math.max(-4,Math.min(4,gx))*80;
   const vz=-Math.cos(a)*b.speed-Math.max(-4,Math.min(4,gz))*80;
   b.speed=Math.min(100000,Math.round(Math.hypot(vx,vz)));
   b.heading=Math.round(Math.atan2(vx,-vz)*0x100000000/(2*Math.PI))>>>0;
  }
  ...
  if(blocked(point,next)||lie(g,next)!=='green'||(!p.contours&&Math.abs(height(g,next.x,next.z)-p.level)>.001)){p.state.status='stopped';p.state.ball.speed=0;break;}
...
export function validateLiveOriginalPutt(p){
 if(!p||(p.contours!==undefined&&p.contours!==true)||![p.cup?.x,p.cup?.z,p.level,p.elapsed].every(Number.isFinite)||p.elapsed<0||p.elapsed>.051||!p.state||p.state.version!==1||!['rolling','stopped','captured'].includes(p.state.status)||!Number.isInteger(p.state.steps)||p.state.steps<0||p.state.steps>10000)throw Error('Invalid live putt.');
