import {advanceTournament,runTournamentSchedule} from './tournament-schedule.js';
...
export async function createTournament(db,ownerId,{title,publicationId,rounds=1,capacity=16,durationHours=168,startsAt=null}){
 const owner=await active(db,ownerId);
 if(startsAt!==null&&(!Number.isSafeInteger(startsAt)||startsAt<Date.now()+60000||startsAt>Date.now()+90*86400000))throw fail(400,"Schedule the start between one minute and 90 days from now.");
 ...
 await db.batch([
  db.prepare("INSERT INTO tournaments(id,owner_id,title,publication_id,course_digest,course_author_id,course_author_name,course_package,rounds,capacity,duration_hours,seed,status,created_at,starts_at) SELECT ?,p.id,?,?,?,?,?,?,?,?,?,?,'registration',?,? FROM players p WHERE p.id=? AND p.disabled_at IS NULL").bind(id,title.trim(),publicationId,course.digest,course.authorId,course.authorName,JSON.stringify(course.package),rounds,capacity,durationHours,seed,now,startsAt,ownerId),
...
export async function getTournament(db,id){
 ...
 return {startsAt:row.starts_at,id:row.id,ownerId:row.owner_id,title:row.title,publicationId:row.publication_id,courseDigest:row.course_digest,courseAuthorId:row.course_author_id,courseAuthorName:row.course_author_name,course:JSON.parse(row.course_package),rounds:row.rounds,capacity:row.capacity,durationHours:row.duration_hours,endsAt:row.ends_at,status:row.sealed?'complete':row.status,createdAt:row.created_at,entrants:entries.results.map(e=>({...e,suspended:!!e.suspended,withdrawn:!!e.withdrawn}))};
}
export async function listTournaments(db){
 await runTournamentSchedule(db);
...
export async function joinTournament(db,id,playerId){
 ...
 await advanceTournament(db,id);
 const joined=await db.prepare("INSERT OR IGNORE INTO tournament_entries(tournament_id,player_id,player_name,joined_at) SELECT t.id,p.id,?,? FROM tournaments t JOIN players owner ON owner.id=t.owner_id JOIN players p ON p.id=? WHERE t.id=? AND t.status='registration' AND (t.starts_at IS NULL OR t.starts_at>?) AND p.disabled_at IS NULL AND owner.disabled_at IS NULL AND (SELECT count(*) FROM tournament_entries WHERE tournament_id=t.id)<t.capacity RETURNING player_id").bind(player.name,Date.now(),playerId,id,Date.now()).first();
...
export async function setTournamentStatus(db,id,playerId,status){
 ...
 if(status==='locked'&&event.startsAt>Date.now())throw fail(409,'This event starts automatically at its scheduled time.');
