...
Account-owned cloud saves are storage, not trusted competitive scores. Cooperative editing, earnings competitions and tournament rounds use separate server-authoritative state. These multiplayer features are locally verified but not yet deployed; hosted rollout and remaining fidelity work are tracked in [the multiplayer backlog](backlog.md).

Shared-building preview: Account → Shared courses lists separately stored cooperative resorts. Owners grant editor/spectator access to existing player IDs; the server supplies ownership, starting funds, simulation time and command validation. Shared resorts continue running when browsers close. Creating or entering one does not replace the player's local resort or ordinary cloud-save slot. Online tournament rounds and earnings standings use the separate event systems documented below.
...
Published-course support requires migration `0004_published_courses.sql` (not yet applied remotely). Owners explicitly publish from Manage access. The library is visible to registered players and contains fixed course packages plus author display names. It excludes resort financial and career state. Account deletion removes authored publications; suspension hides them. Tournament-specific retention and scoring are documented below; they are locally implemented and still await hosted rollout.

Tournament registration requires migration `0005_tournament_lobbies.sql` (local only). Registered players can create/join events; ownership and entrant identity are server-derived. Events retain fixed course copies independently of publications. Account deletion removes entries, cancels owned events and anonymizes author attribution while retaining those copies. Online round execution now uses isolated server-owned rounds; migration `0006_tournament_rounds.sql` and the `TournamentRoundHost` Durable Object binding are also required before deployment. Each entrant uses the same server-generated starting profile. Local golfer imports and submitted scores are rejected. Thinking time pauses, active shots use elapsed server time, completed rounds produce server scorecards and equal totals share rank. Deleting a non-organizer account after registration closes records a withdrawal instead of cancelling other players’ event.
...
Tournament deadlines require migration `0008_tournament_deadlines.sql` (local only). The organizer chooses a 1-, 3-, 7- or 14-day playing window in the lobby; the server accepts integer windows from 1 to 720 hours. The window starts once when registration closes. All rounds must finish and be saved on the server before the displayed deadline. After the cutoff, unfinished entries are marked “deadline missed”, keep any completed scorecards and receive no placing. This applies to disconnected entrants too; a pending shot cannot resume beyond the cutoff. Event access, listing, standings, organizer actions and account deletion settle due deadlines. No browser timer is trusted. Storage finalization happens on the next server access rather than through a background alarm; command guards enforce the cutoff regardless. Existing unfinished locked events receive seven days from migration. Already-final results are unchanged.


Invitation-aware sign-in requires migration `0009_signin_destination.sql` (local only). Signed-out shared-course, tournament-round, tournament-invitation and testing URLs retain a canonical destination through provider sign-in or email verification. The destination is stored in the browser-bound server challenge and revalidated before redirect. External URLs, action endpoints and ambiguous game modes fall back to the normal game. Provider linking and session-expiry redirects retain this context too.

Tournament panels expose a copyable invitation URL (`/?event=<id>`). Opening it shows the selected registration panel after sign-in; recipients explicitly choose Join tournament. Links do not grant course-editing access or bypass registration capacity, account restrictions or event closure. A linked event can be opened directly even when it falls outside the latest 100 listed events. Cancelled provider sign-in retains the invitation for retry when its browser state can be verified.


Earnings competitions require migration `0010_earnings_competitions.sql` (local only). Account → Earnings competitions supports registration, invitations, joining/leaving and an organizer-controlled simultaneous start. Each entrant receives an independent server-owned course copied from the same initial land/seed with $50,000. Windows are 10–120 minutes. Highest net cash change wins, counting every income and expense entry, including construction and wages. An entrant must retain an open tee/green hole and complete paid visitor play to qualify. Equal totals share a place; untouched properties cannot win.

Only the entrant may edit their competition course; they may grant spectator access. The existing Durable Object course host runs it while browsers are closed, retains catch-up time and stops at the shared cutoff. Finished courses become read-only and their host alarms stop. Results come from reconciled server ledger/cash, not submitted balances or scores. A running competition cannot be cancelled. Deleting an entrant retains any completed result under “Former player”, otherwise withdraws the entry; a running event continues when its organizer deletes their account. Suspended unfinished entrants are disqualified at the cutoff. Account deletion still removes owned course simulation state. Large-event capacity and hosted end-to-end operation remain to verify before deployment.

Finished tournament rounds remain reviewable by their active, non-withdrawn entrants after the organizer deletes or suspends their account. Review requires a sealed final result and an existing completed personal round; it cannot create rounds or advance play. Exact accepted-command retries retain their receipt, while new shots reject. Deleting the entrant still removes their private round state, and suspending the entrant still blocks access.

Earnings organizers can choose the existing classic, rolling, river or coastal landscape generator and a parklands, links, desert, tropical or classic mixed environment. The registration panel displays both choices. The server validates these names, creates one random seed and initial property, and copies that exact state to every entrant at start. Clients cannot supply a seed or imported terrain. These settings are read from the stored initial state, so this addition needs no new migration and existing events retain their original setup.
