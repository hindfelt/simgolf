...
### Shared reaction outcome stage: 0x467e4f–0x46806a (2026-09-11)

`original-remark-outcome.js` reconstructs the common outcome block after per-kind reaction selection. A mild -1 reaction for an actor with status byte0 can be suppressed unless a prior history entry matches or word+0x82 has flag0x8000. The history window depends on difficulty (indices1–2,1–4 or1–9); global mode1 suppresses this case and mode2 bypasses the repetition filter. Negative adjustments are reduced with the original signed division unless actor status mask0xe0 equals0x40.

The actor's signed word at+0xa4 is updated with16-bit wrapping before clamping to[-10,10]. The hole reaction total wraps as a signed word. One original RNG draw with bound6 controls the terrain side effect, with an actor flag override. Eligible negative outcomes set terrain flags0x4800, growth byte1 and dirty marker-1, subject to global flag0x04000000, tile flags0x0c00 and terrain code17 exclusions. Per-kind counters, last values, actor reaction flags and tile positive/negative counters are updated in native order.

A native comparison exposed an overlapping hole record: kind64's per-kind count occupies the same word as the running reaction total. The adapter requires consistent input views, propagates both writes and returns consistent signed/unsigned output views. Treating the fields as independent changes original state.

`verify-original-remark-outcome.py` executes the entire block, real clamp helper and original RNG across1600 cases. It compares the complete256-byte actor record, hole values, tile state, effective delta and RNG seed.48 native fixtures are committed; eight combined entry/history/outcome tests pass. The fixture supplies the already-selected delta, current map tile and hole records. Earlier delta selection, pre-outcome actor effects, record dispatch and later social/display effects are not stubbed as if complete—they remain outside this stage and must be composed before live integration. The accumulator's full relationship to live happiness/fees remains to be mapped.

### Reaction preamble: 0x467d72–0x467e4f (2026-09-11)

`original-remark-preamble.js` restores the complete pre-reaction actor snapshot for request40, then preserves the zero-delta early return for signed request IDs>=48. For nonzero actor status, the signed byte at actor offset0x36 clears when its sign opposes the incoming delta; otherwise it accumulates with byte wrapping. Requests2,3,12,13 reset a negative result to1 when it is lower than the negative incoming delta.
...
### Empty secondary action and positional sound dependency (2026-09-11)

The supplied executable contains a single RET byte at0x4a0000, followed by alignment NOPs. The reaction-selection stage now records eligible calls without invoking a mutable resolver. No progression/state effect is invented for this function. Native selection and contiguous adjustment verifiers load that byte from the executable and no longer mutate actor state at this address. Both1320-case comparisons match; eight focused tests pass, including an explicit no-state-change/no-resolver check.

Further inspection identifies0x40c1f0–0x40c3d6 as positional sound dispatch. It calls screen projection0x42f270, applies screen/zoom eligibility and bounds, derives sound parameters, and calls0x447a30. The normal branch consumes the shared RNG with bound600; an alternate branch uses a36-entry signed-byte sequence and advances0x5a8724. Both accepted branches clear0x5a8720. Rejected/off-screen paths can return before those mutations. Therefore this helper must not be discarded as presentation-only: its seed continuation affects later authoritative simulation. Exact projection/camera inputs and queued audio state remain explicit integration requirements, especially for future multiplayer hosts.

Next: reconstruct positional sound dispatch and execute it inside contiguous reaction comparisons, then complete entry/display/history and later social state handling. No live gameplay behavior changed in this pass.

### Positional sound dispatch: 0x40c1f0–0x40c3d6 (2026-09-11)

`original-positional-sound.js` reconstructs dispatch after the explicit screen-projection boundary. Invisible points return unless the duration argument is-1. That special value clamps screen coordinates to[0,799]×[0,599], resets duration to0 and bypasses zoom rejection. Otherwise zoom doubles coordinates around400, with accepted x in[0,800) and y in[0,500].

The normal branch draws once from the shared RNG with bound600, produces pitch300-draw, derives pan from truncated127*x/800-64, and derives volume from horizontal displacement and the original level gate. Queued playback uses the original36 signed sequence bytes at0x4c1f94 and advances its index without consuming RNG; its volume uses vertical position. Both paths call the explicit playback resolver, then clear the queue while retaining its other returned state changes. Suppressed paths mutate nothing.

`verify-original-positional-sound.py` executes the complete original function with actual clamp/RNG helpers and sequence bytes. Projection and final playback are controlled boundaries.1800 cases match event arguments, seed, queue and sequence index; zoom and queue modes vary independently.144 fixtures and four focused tests pass. The reconstructed dispatcher is not yet wired into reaction selection: original screen projection/camera mapping and browser audio playback remain open.

### Original screen projection: 0x42f270–0x42f6f4 (2026-09-11)

`original-screen-projection.js` reconstructs screen coordinates and visibility for the four original renderer orientations0/2/4/6. It retains fixed-point camera origin/scale,1024/1280 width corrections, signed truncation and32-bit intermediate multiplication. Initial visibility checks precede terrain reads. Terrain flags select no-height, object-height, stored-height or four-corner interpolation in the original order5/7/1/3. Magnification and final bounds follow the height correction.

`verify-original-screen-projection.py` executes the complete original helper with controlled object/corner height query results and actual terrain flags/stored bytes.2400 cases match x/y, visibility and ordered external height queries. Orientations, six terrain-flag combinations and magnification vary independently;144 fixtures retain all48 combinations. Six projection/sound tests pass, including early culling without terrain access. Unused orientation values are rejected rather than inventing the original uninitialized-output behavior.

The projection math is recovered, but authentic camera globals and terrain helper adapters must still be composed with positional sound and reaction processing. It does not replace the approved Three.js renderer or imply that browser camera coordinates already match the original viewport.

### Combined projected-sound query order (2026-09-12)

Extended the existing `verify-original-projected-sound.py` comparison rather than substituting a projected point. The original 0x40c1f0 caller runs directly through the original 0x42f270 projection; object/corner height helpers and final playback remain controlled boundaries. The comparison now records ordered terrain queries as well as sound arguments, random draw count, seed, queue and sequence index. It preserves the original 2,400 generated cases and adds 2,400 more, including explicit equal-corner terrain. All 4,800 cases match the existing `originalPositionalSoundAt` composition.

The 192 retained fixtures cover all 48 orientation/terrain-flag/magnification combinations, both played and suppressed sounds, and immutable inputs. A separate regression demonstrates that a visible sound consumes shared randomness while a point culled by the authoritative camera consumes none and never queries terrain. Eight focused projection/sound tests pass. This strengthens verification of the existing helper; it is not new live browser audio behavior. Actual camera/terrain adapters, integration into the remark pipeline, playback and later reaction effects remain unfinished. Multiplayer must use recorded authoritative projection inputs, not each spectator's viewport, for these simulation-affecting decisions.

### Original remark record dispatch: 0x46737d–0x467408 (2026-09-12)

`original-remark-dispatch.js` fills the stage between entry gating and the later remark-specific branch. The original first calls 0x469330 with request kind, supplied value, signed actor byte0x21 times11 plus signed byte0x22, and actor ID. It then calls 0x406b20 with actor ID and zero. Both calls are explicit synchronous speculative-state dependencies: their implementations are not assumed to be neutral.

After those calls, the stage reloads the redirection flag0x53f8b8 and source actor. A redirected message selects the signed partner index at actor+0xa2; otherwise it selects the original actor. Receiver offsets0x84/0x85/0x86 receive7, request kind, and the current byte0x21*11+byte0x22 packed into a byte. Offset0x9c receives the supplied value's low word. Redirection sets bit0x80 in offset0x85. The per-request table at0x836460 receives the full signed32-bit supplied value. The comparison stops before the request35 conditional branch; it does not claim that branch, history or later processing is composed yet.

`verify-original-remark-dispatch.py` executes the contiguous original instructions, intercepting only the two named external helpers. It compares all bytes of four packed actor records, all65 request-value slots, target identity and ordered call arguments across1200 cases. Cases deliberately mutate source fields/redirection during the helpers, detecting stale-state use. All native cases match;65 fixtures cover every request kind. Eleven related dispatch/entry/history/adjustment/audio tests pass. Remaining work includes reconstructing the external helpers, request35/history branch, full entry-to-outcome composition and live schema integration.

### Remark display priority and prefix: 0x406b20–0x406ce8 (2026-09-12)

`original-remark-display.js` reconstructs the complete display helper. Incoming signed32-bit priority is compared to the sign-extended byte at0x5a8718. Lower priorities return without changes. Accepted priority stores its low byte. Actor-1 copies the source text to the displayed text directly. Other actors preserve the source text, clear the working buffer, invoke explicit name expansion0x466fb0(actor,0), and append ` (hole): ` using the actor's current signed byte0x21. The resulting prefix plus preserved message becomes the displayed text; the underlying source text is restored. State changes from name expansion, including actor fields and priority, are retained.

`verify-original-remark-display.py` executes the complete original helper with controlled name expansion and decimal integer formatting. It compares source/display buffers, priority, all actor bytes and name-call events across1000 cases, including signed priorities, actor-1, suppressed messages and state mutations during name expansion. Eighty fixtures retain this coverage. `originalDisplayedRemarkDispatch` now resolves0x406b20 through this helper, leaving phrase processing0x469330 and name expansion explicit. Ten display/dispatch/entry/history tests pass. The wrapper is covered by ordered-state integration tests; its whole contiguous native comparison and live browser mapping remain open. This does not yet replace live golfer remarks.

### Original actor-name expansion: 0x466fb0–0x467181 (2026-09-12)

`original-actor-name.js` replaces the name-expansion placeholder with the complete original selection rules. The actor status byte selects staff names (including subtype4's profile-zero name), the main club name plus masked generation suffix, a secondary name plus subtype suffix, the two alternating named visitors, or the signed profile-index name. It retains optional comma insertion, original II–X suffix behavior and C-string termination. Name/profile tables and club globals remain explicit inputs from the authoritative snapshot; missing required data rejects rather than inventing a replacement name.

`verify-original-actor-name.py` executes the complete original routine and its jump tables without function stubs.1600 cases match exact text, covering all256 status-byte values, append-comma flags, profile/staff indexes and masked club sequence values.256 retained fixtures and nine naming/display/dispatch tests pass. `originalNamedRemarkDisplay` supplies this recovered helper internally, preserving the source text and original display priority behavior. Live profile/table mapping, phrase selection0x469330 and the rest of the remark pipeline remain unfinished.

### Personal phrase selection entry: 0x469330–0x4694a0 (2026-09-12)

`original-phrase-entry.js` recovers the phrase routine's entry and personal-override path. It derives the hole number by signed division of the supplied combined value by11, clears the redirection flag, and zeroes the special narrator actor152 before routing that actor to standard phrase selection. Actors below152 scan the signed-byte request-code table and their profile-specific50-byte phrase slots (1250-byte profile stride). A nonempty matching phrase is appended to the current working buffer and routes to common postprocessing. Empty matches continue scanning, including later duplicate request codes. The0xff terminator routes unmatched requests to standard selection. Name/phrase records remain explicit input data rather than invented dialogue.

`verify-original-phrase-entry.py` executes the contiguous original instructions with actual memory tables and no helper stubs, stopping at either the standard-case dispatcher or common postprocessing.1600 cases compare the branch, signed division, full retained actor records, narrator clearing, text and redirection.105 retained fixtures and thirteen related phrase/naming/display/dispatch tests pass. This entry does not implement the standard phrase cases or common postprocessing; those are the next reconstruction work before replacing the phrase-processing dependency in the remark dispatcher.

### Common phrase substitution: 0x46bc7e–0x46bf36 (2026-09-12)

`original-phrase-postprocess.js` reconstructs common substitutions after phrase selection. Value-1 returns unchanged. Otherwise it expands the current actor and actor XOR1 names, then derives DATA according to the request:2/3 use the primary terrain name with `under the` for type13 and `in the` otherwise;11/20/28 call the explicit location-description helper;12 uses the primary name;39 selects the label table; other requests select the alternate name for type13 and the primary name otherwise. Actors above152 wrap the data in `DATA(...)`. The original MYNAME, PARTNER and DATA replacements occur once each in that order. Later replacements may match text inserted by an earlier one; repeated same-token occurrences remain untouched.

`verify-original-phrase-postprocess.py` executes the complete native postprocessing and replacement helper, with controlled actor-name/location expansion and standard substring-search behavior.3200 cases compare text and ordered helper calls, including negative location coordinates and exact multiples of50. The recovered actor naming is available through `originalNamedPhrasePostprocess`. Integer quotient/remainder outputs are normalized to signed integers rather than JavaScript negative zero. The earlier phrase-entry oracle now includes near-zero signed division, retaining its prior cases and reaching3200 matches. Fifteen related tests pass.

This does not complete standard phrase selection or the location-description helper0x4074d0, nor does it yet replace live remarks. Those functions and full contiguous entry/selection/postprocessing composition remain the next required steps.

### Original location descriptions: 0x4074d0–0x4078b3 (2026-09-12)

`original-location-description.js` reconstructs the full description helper. It accepts direct terrain IDs, map lookup with type-1, or encoded object-coordinate lookup. Terrain category, actual tile, cell flags/detail, region and selected-world type determine the original descriptive literal. This includes scenic bridges/dolphins, trees/cacti/statues, vineyards, bones/gold, regional five-variant flowerbeds, landmarks and home sites. Object types21/22 distinguish flowerbeds and building references. Building-name expansion0x407270 remains explicit; all other descriptive branches are implemented, including branches that append nothing but still return success1. Original literal spellings are retained in `original-location-strings.js`.

`verify-original-location-description.py` executes the complete native function and jump tables with real map/object memory, intercepting only building-name expansion.6080 cases match output, result and ordered building calls. Additional targeted flower/region cases close coverage gaps found in the initial generated matrix. Retained fixtures include every recovered literal and direct/map/object input routes. Eleven location/phrase tests pass. `originalDescribedPhrasePostprocess` connects these rules to recovered name expansion and phrase substitution using explicit authoritative map inputs. Remaining work includes building-name expansion, standard phrase selection, contiguous full-path comparison and live schema integration.

### Original building descriptions: 0x407270–0x407478 (2026-09-12)

The full helper selects nineteen landmark names in plain or detailed form and appends to the working C-string. IDs outside unsigned 0–18 produce “landmark”; every nonzero detailed argument selects the detailed form. Leading spaces and the leading “n” in some detailed forms are intentional: the caller can prefix “a” to produce the original indefinite article. Type10 retains the same leading-space label in both modes. No other simulation state changes.

`verify-original-building-description.py` executes the complete native helper, actual jump table and original string bytes without stubs. All480 cases match, spanning every ID, out-of-range/signed IDs, five detail arguments and four text prefixes including embedded NUL. Twelve focused building/location/phrase/name tests pass. Location descriptions now use the recovered building helper by default, while retaining the explicit resolver seam for isolated native location comparisons. Composed tests resolve twenty landmark records through actor naming, map lookup, building naming and phrase replacement. Standard phrase selection, full contiguous native phrase comparison and live schema integration remain unfinished; these tests do not establish live gameplay integration.

### Standard phrase dispatch and initial cases (2026-09-12)

`original-standard-phrase.js` reconstructs dispatch at0x469380, including the unsigned1–65 range, default display style0x80006318 and case64's no-append route. Twenty-four fixed cases are recovered:6,8,9,12,14–18,20,21,24,25,27,29,32,33,43,45,46,48,55–57. Their original literals and style assignments are retained, including case20's two successive literals. Conditional cases36,41,47,63,65 add value-zero, actor parity, profile parity, staff-category/actor-modulo-four and signed global-mode selection. Thus30 of65 standard cases are represented; the other35 throw explicit unsupported-case errors. The original mode at0x820344 remains an explicit input rather than a guessed live-game mapping.

`verify-original-standard-phrase.py` executes native dispatch and each supported case through0x46bc7e with the actual jump tables and strings and no helper stubs.4352 cases compare text and display style across128 combinations per request, including out-of-range requests, embedded NUL, actor records and signed mode/value boundaries. Retained fixtures cover distinct text/style results per request. For no-append cases, bytes following a NUL are retained in the JS working buffer; the native C-string observation alone does not expose those bytes.

`originalSelectedPhrase` composes the existing personal-phrase entry with standard fallback. Personal overrides preserve display style; only the standard path resets it. Seventeen related tests pass, including personal precedence, a standard location phrase through actual lighthouse description and explicit unsupported-case rejection. Remaining requirements:35 standard cases, external helper reconstruction for those cases, native full entry/selection/postprocess composition and live remark/state integration. No deployment or live-game fidelity completion is claimed.

### Identity and direct-description standard cases (2026-09-12)

Standard cases31,34,40,42,44 are now reconstructed. Case31 uses the actual nonsequential four-entry jump table indexed by the profile byte. Case34 handles values0–2 and changes style even when no text is appended. Case40 uses signed profile remainder modulo8 (negative nonzero remainders append no opener), and positive values call location description with explicit type. Case42 appends the actor name and sets redirection; case44 appends a direct location description then overrides style. Both helper return states are reloaded, preserving mutations and the original ordering of later writes. `originalDescribedStandardPhrase` resolves these boundaries through the recovered actor/location/building functions. `originalSelectedPhrase` also accepts an explicit helper resolver for standard fallthrough.

The native standard-case oracle now covers14976 cases, retaining the prior128-case input matrix per previously supported request and adding signed profile/mode/value cases and five new requests. Name/location calls are controlled boundaries in this dispatch oracle; their text/style mutations and ordered call arguments are compared. The original variant table exposed and corrected an initially incorrect sequential case31 mapping. Eighteen related tests pass, including actual name/landmark helper composition. Coverage is35 of65 cases. Thirty cases, full native phrase-path composition and live state integration remain open.

### Familiar address and actor-dependent cases2/4/26 (2026-09-12)

The complete0x466e30 helper appends one of four familiar forms of address using actorId&3. `original-affectionate-address.js` preserves the leading comma/space and C-string append behavior. Standard cases2,4,26 now reproduce branching on the original signed actor word at0xae. These branches may name the current actor, append a familiar address, or name the actor referenced by signed word0xa2 with the comma argument enabled. Redirection and display-style writes retain native ordering after helper execution. The described-standard wrapper now forwards the native name helper's comma argument instead of always disabling it.

The expanded standard oracle executes the real0x466e30 helper, including its jump table and strings, while retaining explicit name/location boundaries.16128 comparisons match text, style, redirection and ordered helper calls; this adds signed actor-type and partner-record cases while preserving previous request matrices. Nineteen related tests pass, including composed real current/partner names and familiar address. Standard coverage is38 of65 cases;27 cases, full contiguous phrase verification and live integration remain open. The0xae field's live schema mapping is not inferred from these isolated branch tests.

### Facility feedback and profile voice: cases39/51/52/53 (2026-09-12)

`original-profile-voice.js` reconstructs the complete0x46c140 helper: read the actor's signed profile index at0xb6, then return the inverse of bit7 of profile byte0x21 (profile stride560). It consumes explicit original profile records. Case39 uses the result to choose its original animal-reaction wording, preserving style0x800023e8. No demographic interpretation or live schema mapping is inferred.

Cases51,52,53 describe the driving range, pro shop and putting green. Bit3 of original global0x831828 plus five times the actor ID selects either a facility-quality fragment or the improvement fragment. Quality values1–3 at globals0x5a76a8/0x5a76a0/0x5a7698 select new/upgraded/deluxe; other values append nothing. The alternative fragments refer respectively to distance, accuracy and putting. These are original phrase fragments, not proof of actual live skill increases; the input clock and quality globals remain explicit.

The standard oracle executes the real profile-voice helper with actual profile memory and all256 flag-byte values in its expanded matrix.17664 comparisons match native output, styles, redirection and helper calls. Twenty-one related tests pass, including facility quality/no-append branches and required-profile diagnostics. Coverage is42 of65 standard cases;23 cases, full native phrase-path composition and live integration remain open.

### Profile-record and related-request variants: cases5/13/37/38/49/62 (2026-09-12)

Cases5,37,38 share the original six-entry actor-type jump table, including request-specific openings, parity-dependent text, familiar address, name expansion with the correct comma argument, redirection and final punctuation. Out-of-range signed types preserve the native punctuation-only branch. Case13 names certain actor types and otherwise selects by signed profile remainder modulo3; negative nonzero remainders append nothing but still set the original style.

Case49 appends the original profile-record name, distinct from general actor-name expansion. Unterminated supplied records are rejected instead of guessing bytes beyond the record. Case62 reads signed profile byte0x22, adds20 when the recovered voice helper returns0, then appends the corresponding original68-byte-stride phrase-table entry. Its source phrase table is explicit and required; empty strings remain valid.

The standard native oracle now exercises19968 cases, extending the previous matrices with all six requests. It executes actual familiar-address/profile-voice helpers, original profile records and controlled profile phrase storage; name/location expansion remain declared boundaries. Twenty-three related tests pass, including composed real actor naming, distinct request openings and both profile phrase banks. Coverage is48 of65 standard cases. Remaining cases:1,3,7,10,11,19,22,23,28,30,35,50,54,58,59,60,61. Full contiguous phrase verification and live schema/gameplay integration remain required.

### Terrain/object text and timed variants: cases10/22/35/60 (2026-09-12)

Case10 appends a terrain name with the original singular/plural prefix selected solely by its final lowercase byte‘s’. An empty name reads the preceding table byte in native code; that byte is an explicit required input instead of an assumed zero. Case60 adds terrain-specific wording only for category byte13; other categories do not require a name. Case22 uses its distinct37-byte-stride source-name table. C-string termination and original punctuation/style writes are preserved.

Case35 adds45 times actor ID to global0x831828 with signed32-bit wrap, divides by80 with truncation toward zero, and selects one of four original literals using the low two quotient bits. The native oracle includes signed division boundaries and overflow inputs, actual terrain/source tables, case-sensitive names, embedded NUL and empty-name preceding bytes.21504 standard comparisons and25 related tests pass. Coverage is52 of65 cases. Remaining cases1,3,7,11,19,23,28,30,50,54,58,59,61, full contiguous phrase verification and live integration remain open.

### Current-golfer and partner location wording: cases3/11/28 (2026-09-12)

Case3 preserves special actor-type branches that name the current golfer or use fixed redirected text; other types select an opener by profile low bits. Case28 has three partner-addressed types, including familiar address selected using the partner ID, and otherwise combines the recovered profile-voice result with profile parity. Case11 combines actor-type wording with bit0x100 of the value and optionally appends the partner's familiar address. Unlike case3, cases11/28 do not set redirection; both set the original0x800023e8 style.

Native dispatch comparisons now cover22656 cases with the new type/profile/partner combinations. Actual familiar-address and profile-voice helpers execute inside the oracle; name/location remain explicit boundaries. Twenty-six related tests pass, including actual partner names and preservation of the redirection distinction. Coverage is55 of65 cases. Remaining cases1,7,19,23,30,50,54,58,59,61, full contiguous phrase verification and live integration remain open.

### Hole flags, repeated par and replacement text: cases7/30/58 (2026-09-12)

Case30 reads original520-byte hole records. Matching bit0x20 across current/next flags takes precedence over matching0x40; otherwise it compares the current and preceding par bytes. Equal values append the signed decimal par using the original integer-conversion boundary. `originalSelectedPhrase` now forwards its recovered hole index to standard selection.

Case7 overwrites the source buffer rather than appending. Zero-valued requests choose between two literals using actor flag0x10000 and set style0x800023e8; nonzero values use the third literal and default style. Case58 appends one of three value-selected literals. Their direct0x46bc8c continuation is observed by the oracle; these paths have nonnegative values and therefore agree with normal postprocessing's value-minus-one guard. Other paths stop at0x46bc7e as before.

The expanded oracle covers23808 cases with original hole memory, both flag-priority branches, equal/unequal signed par bytes and overwrite/append behavior. Decimal conversion is a declared controlled CRT boundary with its arguments and output checked; actual familiar-address/profile-voice helpers still execute natively. Twenty-eight related tests pass. Coverage is58 of65 cases. Remaining cases1,19,23,50,54,59,61, full contiguous phrase verification and live integration remain open.

### Club names and original hole-name helper (2026-09-12)

`original-club-name.js` implements complete0x40a6c0 with its fourteen club names and unsigned out-of-range no-append behavior. Case54 now calls it and retains the original mode-dependent style. The standard oracle executes the actual club helper and now covers24192 cases.

`original-hole-description.js` reconstructs0x407050 plus custom-name lookup0x45b2c0. When the following hole-record flags contain0x81, the original signed offset table selects a custom string; even an empty custom string counts as found. Offset-1 falls through to one of three original twenty-entry name tables according to signed par3/4/other. Without those flags, the helper appends “Hole ” and the original index, without adding one. Missing original records, offsets or names fail explicitly.

`verify-original-hole-description.py` executes both full native helpers and actual name tables; only CRT integer conversion is controlled.6400 cases match across all twenty table positions, flags, par variants, signed custom offsets and text prefixes. Thirty-one combined tests pass, including custom-name precedence and real club-name composition. Standard coverage is59 of65 cases. Remaining cases1,19,23,50,59,61, contiguous full-phrase verification and live integration remain open; the new hole-name helper is ready for the unfinished history branch, not yet wired into live gameplay.

### Golfer history and composed hole names: case59 (2026-09-12)

Case59 now consumes original44-byte profile history records and hole flags. A nonzero history byte selects the early style-specific line. Otherwise the following hole flags select fixed wording, a partner-name/hole-name composition, or the signed prior-score text. The profile parity check after name expansion reloads the returned actor state; it does not cache the initial value. The recovered0x407050 hole helper and custom-name lookup execute inside standard selection, preserving ordered nested calls. History indexes must fit the supplied record; data outside that record is not guessed.

The native standard oracle now executes those real hole-name helpers, with name expansion and CRT conversion still declared boundaries.24832 cases include640 history scenarios,256 of which permit name expansion to change actor profile parity. Native/JS outputs, actor records, styles, redirection and helper-call order agree. Thirty-two related tests pass, including real partner/custom names, signed score text and the history-first early branch. Standard coverage is60 of65 cases. Remaining cases1,19,23,50,61, contiguous full-phrase verification and live integration remain open.

### Remaining value and partner-profile branches: cases1/61 (2026-09-12)
...
### Final standard resource remark, case50 (2026-09-12)

Case50 clears the working string, uses signed actor word0xb0 as the resource file ID, splits the signed request value into low-nibble section and arithmetic-right-shifted variant, and uses actor dword0x10 bit20 as mode. It now invokes complete `originalResourcePhrase` internally. Entry and named/location wrappers forward an explicit resource reader without confusing resource reads with location descriptions. Cache state and file events are preserved.

The native standard verifier now executes0x466440 as part of the actual dispatch, with only its CRT file calls supplied externally.2048 new combinations include negative file IDs, signed value extremes, both actor flag modes, prefixes, failed opens, cache hits and section/variant misses. Existing33792 cases remain, for35840 matching comparisons. All65 standard request kinds now have reference coverage;34 related entry, resource, postprocessing and standard tests pass. This verifies reconstructed dispatch, not a complete playable remark subsystem: contiguous full-phrase execution, original resource asset delivery and live integration remain required.

### Contiguous phrase composition (2026-09-12)

`original-complete-phrase.js` now composes personal selection, standard dispatch and final MYNAME/PARTNER/DATA substitutions in one call, retaining helper event order, actor changes, styles and resource cache state. `verify-original-complete-phrase.py` runs native0x469330 continuously to0x46bf36. It executes recovered score, club, hole, profile-voice and resource functions; name/location and CRT operations remain declared controlled boundaries. This verifies full phrase control flow but does not claim full native helper integration or browser resource delivery.

23223 comparisons cover all65 standard kinds, personal override markers, signed/byte record fields, resource failure/hit paths, narrator clearing and substitution bypass for value-1. The matrix uses request values-1 through3 so terrain lookup data is initialized, while earlier dispatch tests retain broader signed-value coverage. Complete comparison caught a category normalization issue: postprocessing now reads terrain type as its original byte, including269 mapping to13. Thirty-four related complete-flow, standard, resource, entry and postprocessing tests pass. Live original actor/resource data and gameplay remark dispatch still need wiring.

### Complete phrases with native actor naming (2026-09-12)

`originalDescribedCompletePhrase` resolves actual packed actor names and location descriptions, including the recovered building-name helper, while threading current state into each map snapshot request. It exposes nested location events separately. The callback boundary remains explicit for resource file reads.

`verify-original-named-complete-phrase.py` now executes actual0x466fb0 inside continuous0x469330–0x46bf36 phrase execution.11255 comparisons match, with all65 standard kinds, personal overrides, actor status name variants, comma prefixes and narrator clearing. This matrix restricts profiles to initialized0–7 and removes the earlier synthetic name-callback mutation; the existing broader controlled-boundary oracle remains unchanged. Location and CRT are still native-oracle boundaries. Twenty-nine related tests pass, including actual personal names plus a lighthouse substitution through the composed location/building path. Live game schema/resource initialization and dispatch remain open.

### Complete remark dispatch and display (2026-09-12)

`originalCompleteRemarkDispatch` composes complete phrase generation with recovered display and recipient updates. `originalDescribedCompleteRemarkDispatch` supplies actual name/location/building helpers. Both preserve phrase/display event groups and the post-helper receiver lookup; priority suppression leaves dispatch pending-state writes intact.

`verify-original-complete-remark-dispatch.py` executes0x46737d continuously through the actual phrase, naming and display routines to0x467408.10763 comparisons match across all65 standard kinds, personal overrides, priority variants, signed request fields, redirection and resource paths. Location and CRT remain controlled boundaries. Fixture actors have valid partner links; an initial missing-partner fixture failed explicitly and was corrected rather than weakening receiver validation. Twelve focused tests pass, including redirected named messages and high-priority suppression. Earlier eligibility, history, adjustment/audio stages and live actor/resource initialization still need composition before the full live remark system is complete.

### Eligibility through completed message dispatch (2026-09-12)

`originalRecordRemarkEntry` now reads the entry gates from packed actors and hole records, including signed completion counts at hole offset0x28, current par/strokes and following-record flags. It preserves conditional reads: blocked or ordinary remark kinds do not demand score data. `originalEligibleRemarkDispatch` composes those gates with complete phrase/display dispatch, carries remapped kind19→23 into request storage and preserves all state when disallowed.

`verify-original-eligible-remark-dispatch.py` runs0x4672d0 continuously to0x467408 or the original rejection exit0x469075, including actual names, phrase parsing and display.10763 comparisons match. This composition matrix uses shot counters0,1,9,10,127; negative signed counters remain covered by the standalone entry oracle, since those combinations can select invalid score-hole memory in the broader synthetic data. All65 standard kinds, suppression, global flags, completion remapping and priorities are covered. Ten related tests pass. History, downstream happiness/social/audio composition, original data initialization and live gameplay wiring remain open.

### Post-display history preparation (2026-09-12)

`original-remark-preparation.js` recovers0x467408–0x467502 and its0x467566 branch. Kind19 exits without history changes. Kind35 resolves profile voice and calls positional speech before checking its latest history kind; repeated35 then exits. Other requests, and nonrepeated35, snapshot the current actor, shift history, and derive the voice offset from profile byte0x23 high nibble plus the recovered voice helper. Speech state changes are reloaded before the repeat check and history snapshot.

`originalPreparedRemarkDispatch` composes this after eligible complete message dispatch.2080 native stage comparisons cover all65 kinds, history patterns, profile bytes, repeated remarks and controlled speech mutations. Seven related tests pass, including pending-message writes present in the pre-history snapshot. The stage oracle executes actual profile voice lookup; speech effects remain explicit. Continuous full entry-through-preparation verification, adjustment/social/audio completion and live integration remain open.

### Continuous eligibility through history preparation (2026-09-12)

`verify-original-prepared-remark-dispatch.py` now runs0x4672d0 continuously through eligibility, complete phrase generation, actual actor naming, display, pending recipient writes and history preparation, stopping at0x467502 or the original early exit.10763 comparisons match. The oracle initializes the profile records required by preparation and records phase-specific helper calls, complete actor/history state, pre-history snapshots and voice offset. Location, CRT and positional speech remain controlled boundaries; speech is inert in this contiguous matrix, while mutation ordering is retained in the standalone preparation oracle.

Seven related tests pass, with all65 request kinds represented, score19 early exit, repeated35 speech ordering, suppression and adjustment continuation state covered. No new live game behavior is claimed. Next work remains mapping prepared actor/profile state into the already recovered adjustment stage, then remaining social/audio logic and live initialization.

### Prepared remarks connected to reaction effects (2026-09-12)

`originalReactedRemark` connects prepared message/history state to the recovered selection and reaction stages. It derives profile flags, voice bytes and per-profile history from original packed records, passes the prepared voice offset and pre-history snapshot, and requests the world counters only after preparation. The returned actor dictionary/array retains its shape; updated actor state and reaction world counters are returned together. Early exits do not request world data or resolve later effects.

Eight related tests pass, including the previously native-verified adjustment matrix, negative happiness/history flags, positive profile-history remarks, pending/history ordering and missing-world-data diagnostics. This adapter's full continuous native verification remains open. Reaction world counters are returned explicitly for an authoritative world writer; they are not yet applied to the live game. Positional speech/effects remain explicit, and downstream social logic still needs composition.

### Packed reaction world read/write (2026-09-12)

`original-remark-world.js` reads original fixed-point x*50+z tile addresses, signed actor hole IDs, hole totals and per-kind counters/values from520-byte records. It returns the outcome context and explicit hole/tile locator. Write-back clones the world, updates the actor, RNG, dirty flag, hole words and tile counters, and preserves unrelated records. Kind64's overlapping hole-total/remark-count word is validated. If the result actor moved to another hole/tile, the writer requires a fresh outcome snapshot instead of applying counters to the wrong location.

Ten related tests pass.48 existing native outcome fixtures round-trip through packed world storage, execute their original reaction and produce exactly the verified native state on read-back. This validates mapping against native outcome evidence, not continuous live integration. The snapshot must be read after external speech/preamble effects; the full reaction adapter still needs that late-read wiring and authoritative world ownership before these writes can be used in live gameplay.

### Late outcome snapshots after reaction effects (2026-09-12)

Response/adjustment now accept an explicit outcome reader after selection and preamble effects, immediately before outcome evaluation. The complete reacted-remark adapter forwards it. Fresh hole/tile/global values replace stale context while the current actor and original request identity remain authoritative. Early returns do not invoke the reader; asynchronous readers are rejected.

Fifteen related tests pass, including existing native response/adjustment fixtures and a connected packed-world test where speech changes the actor's hole, tile and RNG. The outcome updates the new hole/tile, preserves the old counters and advances the effect's seed. Kind40 restoration is observed before snapshotting. Full continuous native verification of the world-backed adapter, audio-wrapper wiring, downstream social stages and live gameplay integration remain unfinished.

### Continuous remark execution through reaction outcome (2026-09-12)
...
### Complete explanation assembly and completion (2026-09-12)

`originalCompleteExplanation` composes eligibility, regenerated quoted remark, actor-name suffix, explanatory text and popup completion. It computes the phrase's combined hole/shot from the current packed actor, preserves the pronoun selected before regeneration, and retains phrase helper events separately. `originalDescribedCompleteExplanation` supplies recovered actor-name and location/building helpers.

1560 uninterrupted native0x46806a–0x469075 comparisons pass across all65 kinds, personal override substitutions, selected-delta/difficulty variants, enabled/disabled messages and popup success/failure. This matrix uses personal phrases, with controlled name/location/popup helpers; it does not claim native standard-phrase or real popup integration. Ten related tests pass, including a complete quoted remark with the actual actor name and successful seen-mask/cooldown update. Remaining work includes standard-phrase/native helper coverage in this composition, the popup helper itself, joining explanations after reactions and live integration.

### Complete original popup acceptance/state helper (2026-09-12)
...
### Continuous full remark verification

`verify-original-complete-remark.py` now executes 0x4672d0 through 0x469075 in one original instruction stream. All 3,120 cases match final actor records, dispatch text/priority/request values, source/style, explanation masks/cooldown, popup fields, intermediate reaction counters and final RNG. The read-only reaction-entry hook captures authentic world input without changing emulated execution. All 65 kinds are represented, with varied difficulty, screen-message eligibility, popup acceptance and shot count. Actual original naming, reaction selection/outcome, popup and RNG helpers run; speech is inert and location/string-search/integer-formatting use controlled boundaries. Personal phrase overrides avoid resource and standard-phrase dependencies in this matrix.

Seven related regression tests pass, including the prior separate reacted-routine native matrix. This establishes the joined routine under these declared boundaries; it does not establish full audio fidelity, standard-phrase coverage of the joined routine, or live game integration. The next integration must apply reaction counters while retaining the final post-popup RNG, and map current explanation globals without consuming additional randomness.

### Complete result write-back

`originalApplyCompleteRemarkWorld` applies outcome counters only when the outcome executed, then copies dispatch/explanation fields and the final RNG into a cloned packed world. It does not replace typed map arrays with scalar reaction snapshots. The caller must provide the actual outcome-boundary snapshot; original location and alias checks remain in force. All 3,120 continuous-native cases now verify write-back actor records, counters, final seed and popup state, with immutable inputs. Eight related tests pass.

A separate regression caught loss of resolver-consumed randomness when selection returned before the outcome/explanation: `originalCompleteRemark` now preserves the reaction seed on that exit. This injected speech-state test does not claim native positional-audio coverage. Actual audio state mapping, authoritative live schema/event integration and joined standard-phrase verification remain required. No production change.
...
### Continuous native audio/remark verification

`verify-original-audible-complete-remark.py` runs the original 0x4672d0–0x469075 instruction stream with actual 0x40c1f0 positional audio, 0x42f270 projection, reaction, naming, popup and RNG routines. All 3,120 cases match playback call parameters, audio draw counts, queue/sequence, final actor and message state, intermediate reaction counters and final post-popup RNG. Camera orientation/scale/position, zoom and terrain flags vary. The matrix includes 1,523 cases with playback, 777 consuming audio RNG, 767 with queued playback and 816 early returns.

Nine related tests pass, including full fixture comparisons and packed-world write-back preserving audio fields. Terrain object/corner reads and actual playback are controlled, as are location and text-library boundaries; personal phrase overrides avoid standard/resource phrase dependencies. This verifies complete routine composition under those boundaries, not browser audio rendering or live schema integration. Full standard-phrase composition and authoritative live gameplay wiring remain required.

### Original fee-unit assessment

Live integration inspection found that the browser's nonnegative provisional happiness/fee model cannot be substituted for the signed original accumulator without recovering settlement. `originalFeeAssessment` now matches 1,152 native executions of 0x426be5–0x426c92. The actor's signed word +0xa4 supplies units; mode 0x542c04==2 doubles them. Flags at hole-index*520+0x574700 (the following packed record's first word) each add two for bits 1 and 2. Global 0x542be8 adds its signed bonus. The profile's low three bits at 0x583432+44*profile add two for level 4, five for levels 5–7; those levels also request sound 25 and reload fee units after playback. No clamp occurs here. Arithmetic wraps to signed 32-bit units.

Two tests pass, including native overflow/negative cases and explicit synchronous speech state. Later 0x426d09–0x426d19 displays units multiplied by 100, but payment suppression, posting, mode identity, global bonus identity and profile tier semantics must be mapped before replacing live fees. The recovered assessment does not itself charge money. This evidence resolves why direct live happiness substitution would be premature.

### Hole fee posting and money notices

`originalFeePosting` matches 576 native executions of 0x426e12–0x426e6b with the actual 0x40c580 helper. It adds fee units to global 0x570a24 and the current hole's signed dword at +0x1fc, then adds their low word to the signed 16-bit ledger at 0x582c60+20*selected-index (index at 0x5a5784). All arithmetic wraps at native width. It calls the floating-money notice with actor coordinates and hole argument -1; that call is not an additional cash booking.

`originalMoneyNotice` reconstructs 0x40c580–0x40c5f5: zero units or global flag 0x01000000 suppress the notice, not the preceding fee ledger writes. Otherwise it writes one of eight slots (coordinates, units, 24 ticks), advances the queue, and for non--1 hole arguments subtracts units from that hole's +0x1f8 counter. Normal initialized queue indices 0–7 are required; malformed negative memory indexing is not emulated.

Five related tests pass, retaining all native posting cases, signed overflow, disabled notices with posted fees, zero notices and explicit-hole notice effects. Earlier settlement gates (including 0x00200000), popup construction and complete assessment-to-posting composition remain to recover. Live economy integration and exactly-once authoritative completion handling are not established by this stage.

### Settlement eligibility and first-fee tutorial

`originalFeeSettlement` reconstructs 0x426c92–0x426e6b. Global flag 0x00200000 skips the entire stage. Otherwise, nonzero selected ledger skips tutorial checks; only ledger zero with a zero balance builds the first-fee message. It stores actorId|0x100 at 0x4c1dfc, names the actor, formats signed wrapped units*100 with the original text, stores 3 at 0x5a8714, and calls popup (1,1,actorId). Posting follows regardless of popup acceptance. Original selected-ledger and fee globals are retained; no invented retry or rollback rule is added.

1,152 native comparisons match source text, events, tutorial fields, actual popup state/RNG, fee ledgers and money notices. Actor naming and integer formatting are controlled native boundaries; the browser helper uses the previously recovered naming implementation. Eight related tests pass, including rejected popups with successful fee posting, suppressed settlement without input reads, and the nonzero-ledger short circuit. Continuous assessment-to-settlement verification, earlier hole-completion gating, live ledger field identities and authoritative integration remain open.

### Continuous fee assessment and settlement

`originalCompleteFee` connects assessment, optional post-speech control-state refresh and settlement. The native oracle executes 0x426be5–0x426e6b without resetting registers or state between stages. All 1,152 cases match complete ledgers, source/popup fields, event order, fee units and final RNG, including explicit speech mutations to fee units and seed. Name, speech playback and integer formatting remain controlled boundaries; popup and money-notice helpers execute original instructions.

Ten related tests pass. A separate regression verifies that changed settlement suppression can be read after speech while preserving already-computed units and RNG; asynchronous snapshots are rejected. This routine assumes the caller has established hole-completion eligibility. It does not infer the live meanings of mode/bonus/ledger globals or supply exactly-once completion ownership. Those remain necessary before browser/server fee integration.

### Fee bonus source: active Airstrip level

Tracing writes establishes 0x542be8 as index 14 of the active-facility table at 0x542bb0. The catalog at 0x4c16a8 identifies type 14 as Airstrip. `originalFacilityLevels` now matches 256 native executions of 0x40f908–0x40f9cf over 256 packed 16-byte building records. Signed types below 6 are skipped; valid types 6–19 aggregate field +8 plus one, with actual clamp semantics. Byte +7 bit 0x40 additionally populates active levels and the facility mask. Type 7 levels are cleared after traversal. Invalid out-of-table types are rejected instead of emulating memory corruption.

Four related tests pass. A level-zero active Airstrip contributes one fee unit ($100), and multiple Airstrips take the highest qualifying level rather than summing. The current live 25% bonus is therefore provisional and inconsistent with the recovered rule. Replacing it requires a versioned economy change preserving historical snapshots and old competitive replay handling. The activation flag's complete connection lifecycle and upgrade mapping remain to integrate. The mode multiplier at 0x542c04 is outside this 20-entry table and remains separately unidentified. Also, earlier property-purchase evidence identifies 0x570a24 as the cash reserve; the fee adapter's `totalFeeUnits` label must be corrected before live mapping.

### Live base-level Airstrip correction

The browser now uses the recovered one-unit ($100) base-level Airstrip addition in `airstripFeeBonus`, replacing its provisional 25%. Existing connected-facility checks remain; this does not claim recovery of every original activation condition or introduce unverified upgrades. Professional practice is still free. New fee snapshots use `airstrip-flat-v1`; untagged historical snapshots retain the old percentage validation and recorded values. Protocol 78 separates new economy commands, and golf-only prior rulesets remain replay-compatible because shot physics did not change. Version-77 tournament replay migration translates only formerly valid commands.

Verification covers a real paid round, displayed browser fee, removal after payment, save restoration, tagged/legacy validation, protocol migration, tournament replay, interrupted visits, authority and deterministic session replay. The build and 74 backend tests pass (one capacity test skipped). Broader live original happiness and facility upgrade mapping remain required. No deployment in this change.
...
### Conditional completion remark before reset
