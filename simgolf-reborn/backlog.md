...
Latest verification: all 952 regression tests passed in 5.8 minutes; production build and deployment dry run passed. A 7,260-second simulation passed 12 save/reload cycles with exact replay and 9 helicopter landings. A separate 7,200-second marina run completed 15 boat visits with 12 successful save/reload cycles and no visitor-pool growth beyond the additional pair. Browser checks showed real disembarked visitors and range practice without JavaScript errors. These checks establish a stable preview baseline, not original-game parity or release completion.
...
| A05 | Trusted server host, authentication/ownership, persistence, reconnect and spectators | Two real clients resume the same server state; unauthorized actions reject; no client-submitted balances/scores | Accounts deployed. Shared construction API implemented locally with D1 persistence, owner/editor/spectator access, optimistic concurrency, durable command receipts, bounded server-time catch-up and per-course Durable Object alarms. Browser adapter and Account → Shared courses lobby implemented; real local Worker/D1 tests verify two authenticated Chrome clients, shared spending, reconnect and spectator downgrade. 18-hole/16-staff profiling found simulation bursts, so read/command simulation now runs through per-course Durable Object RPC. The full 952-test baseline passed before online round integration; the latest 16 local gameplay checks, 48 backend tests and five browser integration checks pass. Spectator controls and browser-history polling recovery are implemented and verified. Hosted lifecycle/resource measurements and two-device production verification remain open; shared construction is not deployed yet |
...
| M02 | Course-building / earnings competitions | Defined shared starting budget/property/rules/time window, server-derived eligible earnings, ranked standings and tie-breaks; no imported wealth or client cash submissions | F02–F08, A03–A06; equal server-created properties/budgets, simultaneous starts, private entrant editing, background simulation, deadline freeze and ledger-derived net-cash standings implemented locally. Qualifying paid play/open holes, ties, deletion and suspension policies included. Hosted capacity, real-time full-event browser verification, regional presets and balancing remain open |
| M03 | Multiplayer tournaments on courses built by different users | Publish/select player courses; pin immutable revisions; multiple authenticated golfers complete the event, with server-validated shots, scorecards, standings, ties and results | F09, A02–A06; isolated server-owned asynchronous rounds, standard profiles, command receipts and completed-round standings now implemented locally. Two authenticated browsers finish two rounds on a one-hole published course with matching server totals and shared ranks. Full hosted event completion, atomic final results and deletion-safe anonymized scorecards are implemented locally; server-enforced playing windows and deadline forfeits are implemented locally; awards and hosted capacity remain open |
| M04 | Tournament lobby, invitations, live/asynchronous scheduling and reconnect/forfeit rules | Multiple-device end-to-end event, disconnect recovery and recorded completion under explicit event rules | M03; authenticated registration lobby, capacity, join/leave, organizer closure/cancellation and independent course pins implemented locally. Isolated round hosting, reload recovery and two-player/two-round real-time local browser completion are verified; explicit permanent self-withdrawal is implemented with server guards and retained scorecards; playing windows and deadline-driven forfeits are implemented locally; copyable tournament links open registration after sign-in without auto-enrollment; hosted completion, scheduled registration/start times and external invitation delivery remain open |
...
F06 original reaction prerequisite: recovered shared remark entry gates and request0x13→0x17 remapping at0x4672d0–0x46737d.1800 executable comparisons and two regression tests pass. Next: reconstruct dispatch/actor selection/history and the reaction-specific state transitions before supplying the automatic planner's resolver. This remains isolated source-backed logic, not completed live happiness/scenery integration.

F06 remark history: recovered the original ten-entry packed history update and full pre-reaction actor snapshot at0x467448–0x4674c7.1000 native comparisons and four combined entry/history tests pass. Live schema mapping, earlier dispatcher/actor selection, per-kind state effects and resolver composition remain required. No provisional happiness rule was replaced with incomplete original logic.

F06 original reaction outcomes: reconstructed0x467e4f–0x46806a, including difficulty/history-based mild-complaint filtering, signed negative adjustment, actor/hole counters, terrain growth flags, positive/negative cell counts and exact RNG continuation. Preserves kind64's aliased hole counter/total.1600 native comparisons and eight combined reaction tests pass. Remaining: actual per-kind delta selection, preceding state transitions, dispatch and subsequent effects, then composition and live schema integration. Current browser happiness values remain provisional.

F06 reaction preamble: recovered0x467d72–0x467e4f including request40 snapshot restoration, zero-delta exits, signed byte accumulation/special resets, and explicit pending0x447a30 effects.1400 native cases match; eleven combined reaction tests pass. The unknown external call is not silently skipped. Continue its implementation and contiguous outcome composition before live resolver integration.

F06 contiguous response: connected original preamble and outcome stages through an explicit synchronous effect-state boundary.1600 contiguous native cases and ten focused tests pass.0x447a30 is identified as sound-object parameter/play dispatch; browser audio adaptation remains open. Full remark entry/history/selection/later-effects composition and live resolver mapping are still required.

F06 per-kind adjustment selection: recovered the complete65-request switch0x467502–0x467d72, covering native delta constants, difficulty/actor/profile conditions and ordered call effects.1320 native comparisons and six selection/response tests pass;264 committed cases cover every request across four difficulties. Continue contiguous selection/response composition, actual external helpers and final social effects before live authoritative integration.

F06 contiguous adjustment pipeline: connected all per-kind selection branches to preamble/outcome, without injecting selected deltas.1320 native contiguous cases and eight related tests pass. Next: supply original profile byte0x4d5061 and integrate the recovered read-only voice query0x46c140; complete earlier record processing, other external helpers and later social effects. Actor relocation/hole changes require refreshing supplied map/hole records before general live integration.

F06 voice query integration: the reaction-selection pipeline now runs the recovered0x46c140 query directly from explicit profile voice bytes.768 exhaustive native byte/index cases,1320 selection cases and1320 contiguous adjustment cases match; ten combined tests pass. This closes the fake voice-result boundary, while real profile mapping and remaining effect helpers/live reaction integration remain open.

F06 secondary action resolved:0x4a0000 is an actual RET in the supplied executable. Removed the mutable resolver for eligible calls;1320 selection and1320 contiguous cases match, eight tests pass. Next positional sound helper0x40c1f0 is not simulation-neutral: screen eligibility controls shared RNG consumption and queued audio state. Preserve those inputs/side effects during reconstruction and authoritative multiplayer design rather than silently dropping audio calls.

F06 sound dispatch: recovered positional sound eligibility, forced-coordinate clamping, zoom rejection, pan/volume/pitch, original36-note sequence, queue writes and shared RNG consumption.1800 native cases and four tests pass. Next integrate this helper inside reaction comparisons with explicit original projection state, then browser playback. Multiplayer must pin/record the authoritative inputs driving these shared-RNG calls; individual spectator cameras must not independently alter accepted simulation history.

F06/F01 original screen projection: recovered fixed-point camera transforms, four orientations, resolution scaling, terrain-height correction/interpolation and visibility gates at0x42f270.2400 native cases and six projection/sound tests pass. Next compose real projection into positional sound and reaction verification; live camera/terrain adapters remain required. Approved browser graphics remain unchanged.
...
M02 regional settings (2026-09-12): earnings events now expose the existing landscape and environment choices, show them before joining, and copy one server-generated starting property to all entrants. Server validation rejects unsupported settings and still rejects custom seeds/state. This completes selection of existing regional generators for competitions; regional art fidelity, generator balancing and hosted capacity remain open.

M01/M02 local concurrent-host evidence (2026-09-12): sixteen populated 18-hole courses, each with twelve visitors and sixteen staff, advance three concurrent six-second batches through actual local Durable Object/D1 RPC with matching complete state and no backlog. Measured batches 0.85–1.58 seconds locally. Added opt-in `test:host-capacity` and structured results; removed redundant scheduler identity rewrites. Hosted capacity/cost and sustained load remain unverified. Cloudflare dashboard requires owner sign-in before plan/email activation review can continue.

F06 combined projection/sound verification (2026-09-12): expanded the existing uninterrupted native comparison from2400 to4800 cases, retaining prior cases and adding equal-corner terrain. Ordered object/corner queries now join sound parameters, RNG and queue state in the comparison;192 retained fixtures cover48 camera/terrain combinations. Eight focused tests pass. This verifies the existing composition more fully, not live reaction integration. Next supply authoritative camera/terrain inputs to remark processing and complete playback/later effects.

F06 record dispatch (2026-09-12): reconstructed0x46737d–0x467408, including ordered external record-processing calls, post-call receiver selection, packed pending-message writes and per-request argument storage.1200 native comparisons and eleven related tests pass. Both unknown helpers remain explicit synchronous state dependencies. Next recover0x469330/0x406b20, request35/history branching and full remark composition before live integration; no provisional live happiness rule was replaced prematurely.

F06 remark display (2026-09-12): recovered complete0x406b20 priority gating, actor/hole prefix, displayed-message copy and working-buffer restoration.1000 native comparisons and ten related tests pass. Connected it inside the recovered record dispatcher, removing that placeholder effect. Phrase selection0x469330, name expansion0x466fb0, full contiguous composition and live actor mapping remain open.

F06 actor-name expansion (2026-09-12): recovered complete0x466fb0 naming rules and connected them to named remark display, replacing the external name-expansion placeholder on that path.1600 native cases cover all256 actor status values; nine related tests pass. Required tables stay explicit. Next reconstruct phrase selection0x469330 and compose the full remark path with live profile/camera/terrain inputs.

F06 personal phrase entry (2026-09-12): recovered0x469330–0x4694a0, including signed hole extraction, redirection reset, narrator clearing and profile-specific dialogue overrides with empty/duplicate matching behavior.1600 native cases and thirteen related tests pass. Standard phrase cases and common postprocessing remain to recover before full live remark integration.

F06 common phrase substitutions (2026-09-12): recovered0x46bc7e–0x46bf36 MYNAME/PARTNER/DATA ordering, terrain/label selection and narrator formatting.3200 native postprocessing cases and3200 phrase-entry cases pass, including signed near-zero arithmetic; fifteen related tests pass. Existing actor naming is connected through a wrapper. Standard phrase cases, location helper0x4074d0 and full live composition remain open.

F06 location descriptions (2026-09-12): recovered complete0x4074d0 map/object/terrain and regional descriptive rules, including every flower variant, and connected them to named phrase substitution.6080 native comparisons and eleven related tests pass; fixtures cover every recovered literal. Building-name helper0x407270, standard dialogue cases and full live integration remain open.

F06 building descriptions: recovered all nineteen original plain/detailed landmark labels, unsigned fallback and C-string append behavior.480 complete native helper comparisons and twelve related tests pass. The location/phrase path now resolves building records without placeholder labels. Standard phrase selection, contiguous full-path verification and live remark integration remain open.
...
F06 standard remarks now cover59 of65 cases. Added complete club-name helper/case54 and recovered the original custom/par-based/numbered hole-name helpers needed by history remarks.24192 standard native cases,6400 hole-name comparisons and31 related tests pass. Six cases (1,19,23,50,59,61), contiguous phrase verification and live integration remain open.
...
Original remark resource loader: complete0x466440 parsing/cache writes now match2912 native executions with explicit CRT file-read inputs. Includes failed-open invalidation, theme selection, section/variant rules, EOF and the201-line cap. Malformed-source stale-buffer behavior is explicit. Case50 dispatch, browser resource loading, full phrase composition and live integration remain unfinished; this helper alone does not complete golfer remarks.

Original standard golfer remarks: case50 now composes the complete resource parser with exact signed file/value and actor-flag inputs. All65 standard request kinds are reconstructed;35840 native comparisons and34 related tests pass. Entry and description wrappers pass resource readers explicitly. Full contiguous phrase verification, original browser resource delivery and live gameplay wiring remain open.

Original complete phrase flow: personal selection, all standard remarks and final substitutions now compose in one API, with23223 matching uninterrupted native executions and34 related tests. Fixed byte-sized terrain category handling discovered by composition. Names/location and CRT remain explicit oracle boundaries; full helper composition, browser resource delivery and live remark dispatch remain unfinished.
...
Reaction-to-explanation adapter now carries the reacted actor, remapped kind, selected (unreduced) delta and RNG seed into the complete popup flow. An explicit synchronous post-reaction snapshot supplies current explanation globals and difficulty. Ten related tests pass, including shared RNG continuation, rejected-popup behavior, early exits and preserving world arrays. This composition still needs uninterrupted native verification across both phases, a live world writer and audio/gameplay integration; it is not a completed live happiness system.

Full entry-through-explanation native verification now passes 3,120 cases across all 65 remark kinds, including actor state, message state, reaction world counters, popup completion and shared RNG. Seven related tests pass. This matrix uses personal phrase overrides and inert audio; standard-phrase full-routine coverage, actual audio composition and authoritative live world/UI integration remain open. Production behavior is unchanged.

Complete remark results now have a packed-world writer that applies outcome counters and then the final popup RNG/message state, preserving map arrays. All 3,120 native cases round-trip through the writer; eight related tests pass. Fixed loss of speech-consumed RNG on an early reaction return. Live schema/event integration, full positional audio composition and standard-phrase continuous verification remain unfinished.

Positional-audio reactions now forward the late outcome snapshot reader. Twelve related tests pass, retaining native audio fixtures and checking destination tile/hole reads after actual positional playback, post-playback RNG continuation, early-return suppression and async rejection. Complete entry/preparation/audio/explanation composition and live integration remain open.

Complete remarks now compose actual positional sound for both preparation and reaction, retaining queue/sequence and shared RNG through outcome and popup. Fourteen related tests pass, including previous native matrices, queued speech, repeated-kind-35 suppression, double speech on a new kind-35 remark and sound-only early returns. Uninterrupted native verification with audio enabled, standard-phrase full-flow coverage and authoritative live integration remain required; no deployment.

Uninterrupted native full-remark verification with positional audio now passes 3,120 cases, including 1,523 with playback and 816 early returns. Nine related tests pass, verifying native playback parameters, queue/sequence, actor/counter changes, final RNG and packed-world write-back. Terrain reads and playback remain controlled boundaries; personal phrase overrides are used. Full standard-phrase coverage and live authoritative gameplay/audio integration remain unfinished.

Live happiness integration uncovered additional original fee adjustments. Recovered fee-unit assessment matches 1,152 native cases; two tests pass. Original signed happiness, mode doubling, hole flags, global bonus and profile-tier additions precede payment; live flat happiness×100 omits these. Settlement gates/posting and authoritative field mappings remain required before replacing live economy calculations. No deployment.

Original hole fee posting now matches 576 native cases including its actual floating-money helper; five related fee tests pass. Signed global/hole/ledger counters and notice queues are preserved, and disabled money notices correctly do not suppress posted fees. Settlement eligibility/popup gates, complete fee-flow composition and live authoritative economy integration remain open.

Original fee settlement gates/tutorial/posting now match 1,152 native cases; eight related tests pass. A rejected first-fee popup still posts fees, while global flag 0x00200000 suppresses settlement entirely. Assessment-to-settlement composition, earlier completion eligibility and live authoritative economy field mapping remain unfinished. Production fees are unchanged.

Complete fee assessment-through-settlement now matches 1,152 uninterrupted native cases; ten fee tests pass. Post-speech fee/RNG changes and current settlement controls are preserved. Original hole-completion eligibility, live mode/bonus/ledger identities and authoritative completion integration remain open. No production fee change.

Identified fee bonus 0x542be8 as active Airstrip level (catalog type 14). Building aggregation matches 256 native cases; four tests pass. A level-zero active Airstrip adds $100 per fee, with highest-level selection rather than stacking. Live 25% behavior needs a versioned correction retaining old scorecards/replays; activation/upgrade mapping remains open. Fee global 0x570a24 is cash reserve, so rename the adapter's provisional totalFeeUnits before live integration.

Live Airstrip fees corrected to the recovered base-level $100 addition, with existing path connectivity and nonstacking behavior. Ruleset/protocol 78 marks the economy change; new scorecards carry an explicit fee rule, legacy percentage snapshots remain valid and unchanged, and golf-only version-77 tournaments migrate without altering shots. UI descriptions updated. Full original activation/upgrade/happiness integration remains open. Not deployed.

Corrected original fee adapter global 0x570a24 from totalFeeUnits to cashUnits, reflecting its verified role as cash reserve. The 1,152-case native full-fee oracle and eight related tests pass. Live ledger mapping remains open.
...
Conditional end-of-hole remark now precedes profile/reset updates through an explicit synchronous resolver. 128 native cases match with controlled remark mutation; five related tests pass. Updated stroke and clock values are read after the remark. The actual full remark/world resolver and the remaining completion branches still need integration; no live replacement or deployment.

Completion prefix now composes score/statistics, fee settlement, conditional remark and reset in one sequence. All 1,152 continuous native cases and the retained matrix test pass. The native comparison preserves shared profile fee-tier storage. Actual full remark integration, caller eligibility and subsequent round progression remain open. No deployment.

Actual audible remark engine now has a packed-world adapter and runs before completion reset. Outcome snapshots are refreshed after speech/preamble; map arrays stay intact while happiness/counters, RNG and messages are written back. Integration tests cover ordinary kind 19 and remapping to kind 23. Continuous native verification of this new joined flow and live browser/server adoption remain required.

Automatic audible packed-world adapter now matches all 3,120 retained native cases across all 65 remark kinds. Verification includes actual actor/counter write-back, final message and RNG state, early returns and unchanged terrain. This closes adapter-level native matrix coverage; uninterrupted completion-with-remark verification and live schema adoption remain open.

Actual conditional end-of-hole remark through reset now matches 192 uninterrupted native cases, including skipped remarks, kind 19 and remapping to kind 23. Three related tests pass. Actual remark/audio/projection/popup helpers execute in the native oracle; terrain/playback/text boundaries remain controlled. Earlier fee-to-remark composition, later progression and live browser/server schema adoption remain open.

Score/statistics and fee settlement now compose with the actual audible remark/reset engine. All 384 uninterrupted native completion-prefix cases match; two retained native matrix tests pass. Cases include payment suppression and disabled notices, mode doubling, bonus units and remapped completion remarks. Later completion branches, caller eligibility and live browser/server adoption remain unfinished; not deployed.

Live integration baseline established: a visitor saved mid-air completes both holes identically after restore, with correct scores/fees displayed at phone size. The new end-to-end test and 16 related live-round tests pass. Evidence: references/release/live-round-baseline.md. Live game.js still uses provisional happiness rules; original packed completion is not yet adopted.

Live signed happiness integration: reactions now clamp to the recovered −10…10 range, and signed fees post/reconcile/save correctly. Protocol 79 and signed-happiness-v1 distinguish new economy behavior; old fee snapshots retain their values, old live happiness above 10 is capped on migration, and version-78 golf-only replays migrate without changing shots. Earnings eligibility now checks an actual positive green-fee ledger entry instead of positive net fees, preserving paid-play eligibility after later negative fees. Initial happiness, incident selection/deduplication and full packed completion adoption remain provisional/open. Build, live full-round/reload tests and backend checks pass; no deployment.

Ruleset 79 authenticated browser/local Worker/D1 suite passes all eight tests in 1.4 minutes: shared edits/reconnect/spectators, phone controls, published courses, two-round tournaments and resumption/final standings, withdrawal and independent earnings spending. Evidence: references/release/ruleset-79-integration.md. Full earnings soak, hosting capacity/provider activation and deployment remain open.

Ruleset 79 full ten-minute earnings soak passed with two authenticated browser accounts and local Worker/D1. Both courses stopped at 12,000 ticks and completed 31 holes; final cash $43,120/$35,900 matched server results and survived reload. Both remained eligible despite negative net fees because they had earlier paid visits. No browser JS errors. Evidence: references/release/ruleset-79-earnings-soak.md. Balance/fidelity, multi-hour stability, production capacity and provider activation remain open; no deployment.
