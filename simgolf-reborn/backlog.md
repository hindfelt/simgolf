...
| A05 | Trusted server host, authentication/ownership, persistence, reconnect and spectators | Two real clients resume the same server state; unauthorized actions reject; no client-submitted balances/scores | Accounts deployed. Shared construction API implemented locally with D1 persistence, owner/editor/spectator access, optimistic concurrency, durable command receipts and bounded server-time catch-up without dropped ticks. Background scheduling, shared-game browser adapter and two-device verification remain open; shared construction is not deployed yet |
