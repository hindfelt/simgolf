...
| M03 | Multiplayer tournaments on courses built by different users | Publish/select player courses; pin immutable revisions; multiple authenticated golfers complete the event, with server-validated shots, scorecards, standings, ties and results | F09, A02–A06; isolated server-owned asynchronous rounds, standard profiles, command receipts and completed-round standings now implemented locally. Full hosted event completion, final sealing, deadlines/forfeits, awards and capacity remain open |
| M04 | Tournament lobby, invitations, live/asynchronous scheduling and reconnect/forfeit rules | Multiple-device end-to-end event, disconnect recovery and recorded completion under explicit event rules | M03; authenticated registration lobby, capacity, join/leave, organizer closure/cancellation and independent course pins implemented locally. Round hosting, schedules, reconnect/forfeit and event completion remain open |
